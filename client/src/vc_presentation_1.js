...
//
// This is where a presentation starts for the wallet: a request arrives — in the
// query string, or as a request_uri the wallet has to fetch, or pasted in from a
// QR code on another device — and before anything is disclosed the wallet has to
...
function clientIdentifier(clientId) {
  ...
  var known = ["redirect_uri", "x509_san_dns", "x509_hash", "did", "verifier_attestation",
               "openid_federation", "decentralized_identifier", "pre-registered", "origin"];
  ...
  if (known.indexOf(prefix) === -1) {
    ...
    return {
      ...
      note: "No Client Identifier Prefix, so this is a pre-registered client: the wallet is expected to know " +
            "it — and its key — out of band. A signed request can therefore be verified, and this workflow " +
  ...
  if (prefix === "redirect_uri") {
    ...
    return {
      ...
      note: "The client_id IS the URL the response goes to. Such a request cannot be signed — there is no key " +
            "the wallet could obtain for it — so what binds the request to the verifier is only that the " +
            "presentation is sent to that same URL. All verifier metadata has to come in client_metadata."
    };
  }
  log.debug("Leaving clientIdentifier(). prefix=" + prefix);
  return {
    prefix: prefix, value: id.slice(colon + 1), signable: true,
    note: "The prefix says how the wallet is meant to authenticate the verifier and find its metadata. This " +
          "workflow implements the pre-registered and redirect_uri cases; for " + prefix +
...
function fetchRequestObject(uri, clientId) {
  ...
  status("vp_request_status", "Fetching the Request Object from " + uri + " …", "vc-pending");
  return fetch(uri, { headers: { "Accept": "application/oauth-authz-req+jwt, application/jwt, application/json" } })
    .then(function (r) {
    ...
    .then(function (text) {
      ...
      if (body.split(".").length === 3) {
        ...
        sdJwtVp.REQUEST_PARAMS.forEach(function (name) {
          ...
          params[name] = typeof payload[name] === "object" ? JSON.stringify(payload[name]) : String(payload[name]);
        });
        adoptParams(params, "request_uri (signed Request Object)");
        request.signed = true;
        request.requestObject = body;
        request.requestObjectHeader = metadataClient.b64uToJson(body.split(".")[0]);
      ...
      Object.keys(parsed).forEach(function (k) {
        flat[k] = typeof parsed[k] === "object" ? JSON.stringify(parsed[k]) : String(parsed[k]);
      });
...
function verifyRequestSignature(compact, clientId) {
  ...
  return metadataClient.fetchJson(jwksUrl)
    .then(function (jwks) {
      return metadataClient.verifyJwsWithJwks(compact, jwks, "the Request Object");
    })
    .then(function (v) {
      request.signatureValid = !!v.valid;
      request.signatureVerdict = (v.valid ? "VALID" : "INVALID") +
        " — checked against the keys at " + jwksUrl + " (alg " + v.header.alg + ", kid " + v.kid + ")." +
        (v.valid ? " The claims being asked for are the ones the verifier signed." : "");
    ...
    .catch(function (e) {
      ...
      log.error("the Request Object signature could not be checked: " + e.message);
      return request;
    });
}
...
function renderRequestedClaims() {
  ...
  if (!queries.length) {
    setHtml("vp_requested_table",
      '<tbody><tr><td>This request carries no <code>dcql_query</code>, so it does not say which credential ' +
      'or which claims it wants. OID4VP requires one (or a <code>scope</code> standing for one).</td></tr></tbody>');
  ...
  queries.forEach(function (q) {
    ...
    rows += "<tr><td>" + esc(q.id || "—") + "</td><td>" + esc(q.format || "—") + "</td>" +
            '<td class="' + (typeOk ? "vc-ok" : "vc-bad") + '">' + esc(vcts || "(any)") + "</td>" +
            "<td>" + (sdJwtVp.requiresKeyBinding(q) ? "required" : "not required") + "</td>" +
            "<td>" + (paths.length
              ? paths.map(function (p) {
                  var have = !!available[p];
                  return '<span class="' + (have ? "vc-ok" : "vc-bad") + '">' + esc(p) +
                         (have ? "" : " (not in the credential you hold)") + "</span>";
                }).join("<br>")
              : "(no claims named — the whole credential)") + "</td></tr>";
  });
  setHtml("vp_requested_table",
    "<thead><tr><th style='width:16%'>Query id</th><th style='width:12%'>Format</th>" +
    "<th style='width:22%'>vct wanted</th><th style='width:12%'>Holder binding</th>" +
...
function renderRequest() {
  ...
  if (!request) {
    status("vp_request_status",
      "No presentation request yet. A verifier sends one — step 0 starts at one — or paste the request URI " +
  ...
  setText("vp_response_mode", p.response_mode || "(fragment, the OAuth 2.0 default)");
  ...
  var problems = [];
  if (!p.nonce) problems.push("no nonce (nothing would stop the presentation being replayed)");
  if (String(p.response_type || "") !== "vp_token") problems.push('response_type is not "vp_token"');
  if (String(p.response_mode || "") === "direct_post" && !p.response_uri) {
    problems.push("response_mode is direct_post but there is no response_uri to post to");
  }
  ...
  if (problems.length) {
    status("vp_request_status", "This request cannot be answered: " + problems.join("; ") + ".", "vc-bad");
    disable("vp_continue_button", true);
  } else {
    status("vp_request_status",
...
function usePastedRequest() {
  ...
  if (!text) {
    status("vp_request_status", "Paste the request URI from the verifier's QR code first.", "vc-bad");
    return false;
  }
  var params = sdJwtVp.parseRequestUri(text);
  if (!Object.keys(params).length) {
    status("vp_request_status",
      "Nothing in that text looks like an OID4VP Authorization Request. It should carry at least a client_id " +
  ...
  if (params.request_uri) {
    fetchRequestObject(params.request_uri, params.client_id)
      .then(renderRequest)
      .catch(function (e) {
        status("vp_request_status", "Could not fetch the Request Object: " + e.message, "vc-bad");
      });
    return false;
  }
  renderRequest();
...
function forgetRequest() {
  ...
  renderRequest();
  status("vp_request_status",
    "The request was discarded. Nothing was presented, and its nonce is now useless to anyone.", "vc-pending");
...
function onload() {
  ...
  if (Object.keys(fromQuery).length) {
    adoptParams(fromQuery, fromQuery.request_uri ? "the verifier's redirect (by reference)"
                                                 : "the verifier's redirect (by value)");
