...
// verifying an assertion. No DOM in here, which is what lets tests/webauthn_decode.js
// drive it in node against artifacts a real browser produced.
//
...
//      not others, at random, specifically to break implementations that compare
//      it against a template. Measured happening here on 2026-08-08.
//
...
function parseClientDataJSON(bytes) {
  ...
  var known = ["type", "challenge", "origin", "crossOrigin", "topOrigin", "tokenBinding"];
  ...
  log.debug("Leaving parseClientDataJSON(). type=" + out.type + " origin=" + out.origin);
  return out;
}
...
function parseAuthenticatorData(bytes) {
  log.debug("Entering parseAuthenticatorData(). bytes=" + (bytes && bytes.length));
  ...
  if (out.flags.AT) {
    if (bytes.length < offset + 18) {
      throw new Error("the AT flag is set but there is no room for attested credential data");
    }
    ...
    if (bytes.length < offset + idLength) {
      throw new Error("the credential ID claims " + idLength + " bytes but only " +
    ...
    // cbor.decodeFirst exists: the extension data, if any, begins where it ends.
...
function parseAttestationObject(bytes) {
  log.debug("Entering parseAttestationObject(). bytes=" + (bytes && bytes.length));
...
function summariseAttStmt(fmt, attStmt) {
  ...
  var out = { format: fmt, recognised: true, alg: null, algName: null, sigBytes: null, x5c: null, note: null };
  var known = ["packed", "tpm", "android-key", "android-safetynet", "apple", "fido-u2f", "none"];
  if (known.indexOf(fmt) === -1) {
    out.recognised = false;
    out.note = "Unrecognised attestation format. The statement is shown raw; nothing here can " +
  ...
  if (fmt === "none") {
    out.note = "No attestation. The authenticator declined to identify itself, which is the " +
  ...
  if (Array.isArray(x5c)) {
    out.x5c = x5c.map(function (c) {
      return { bytes: c instanceof Uint8Array ? c.length : null, base64: c instanceof Uint8Array ? bytesToBase64url(c) : null };
    });
  }
  log.debug("Leaving summariseAttStmt(). alg=" + out.algName + " x5c=" + (out.x5c ? out.x5c.length : 0));
  return out;
}

// --- signature verification -------------------------------------------------

// DER SEQUENCE { INTEGER r, INTEGER s } -> the fixed-width r‖s Web Crypto wants.
// See note 1 in the file header: getting this wrong produces `false` rather than
// an error, so it looks like a bad signature instead of a bad decoder.
function derToRawSignature(der, coordinateBytes) {
  log.debug("Entering derToRawSignature(). der=" + der.length + " coord=" + coordinateBytes);
  if (der[0] !== 0x30) {
    throw new Error("an ECDSA signature should be a DER SEQUENCE (0x30); this starts 0x" +
                    der[0].toString(16));
  }
  ...
  function readInt() {
    ...
    // DER pads with a leading zero to keep the value positive; the raw form does
    // not want it. Conversely a short value is left-padded to the coordinate width.
    while (v.length > 1 && v[0] === 0x00) {
      v = v.subarray(1);
    }
    if (v.length > coordinateBytes) {
      throw new Error("an ECDSA component is " + v.length + " bytes, wider than the curve's " +
                      coordinateBytes);
    }
...
async function verifyAssertion(input) {
  ...
  if (expected.challenge) {
    record("challenge matches the one the RP issued", clientData.challenge === expected.challenge,
           "clientData carries " + clientData.challenge + "; expected " + expected.challenge);
  }
  if (expected.origin) {
    record("origin matches the RP's origin", clientData.origin === expected.origin,
           "clientData carries " + clientData.origin + "; expected " + expected.origin);
  }
  if (expected.rpId) {
    var digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(expected.rpId));
    var ok = equalBytes(authData.rpIdHash, new Uint8Array(digest));
    record("rpIdHash is SHA-256 of the RP ID", ok,
           "authenticator data carries " + authData.rpIdHashHex + "; SHA-256(" + expected.rpId +
  ...
  if (expected.requireUserVerification) {
    ...
    record("user verification (UV) was performed, as required", authData.flags.UV,
           "UV flag " + (authData.flags.UV ? "set" : "clear") +
           (authData.flags.UV ? "" : " — the signature is valid; the ceremony simply did not verify the user"));
  }
  if (typeof expected.previousSignCount === "number") {
    ...
    record("signature counter advanced", advanced,
           "counter is " + authData.signCount + ", previously " + expected.previousSignCount +
           (advanced ? "" : " — a counter that does not advance can mean a cloned authenticator, " +
                            "though authenticators that never increment report 0 throughout"));
  }

  // The signed message: raw authenticator data followed by the HASH of the
  // client data. Note 2 in the header.
  var clientDataHash = new Uint8Array(await crypto.subtle.digest("SHA-256", clientDataBytes));
  ...
  if (spec.derSig) {
    sigForVerify = derToRawSignature(signature, COORDINATE_BYTES[jwk.crv] || 32);
  }

  var key = await crypto.subtle.importKey("jwk", stripAlg(jwk), spec.webCrypto, false, ["verify"]);
  var valid = await crypto.subtle.verify(spec.verify, key, sigForVerify, signedData);
  record("signature verifies against the credential public key", valid,
         algName + " over " + signedData.length + " bytes (authenticatorData ‖ SHA-256(clientDataJSON))");

  var allOk = checks.every(function (c) {
    return c.ok;
  });
  log.info("Leaving verifyAssertion(). valid=" + allOk + " checks=" + checks.length);
  return {
    valid: allOk,
