...
// could only ever be about one DID: the issuer of the credential in hand. Nothing
// about resolving a DID, reading its document, or checking what it is linked to is
// specific to issuance, so it is a page of its own and the wording no longer says
// "issuer".
//
// What it does, in the order the panes appear:
//
//   * RESOLVE a DID. did:jwk and did:key ARE their key — the identifier carries it,
//     so they resolve with no network call at all, and the provenance line says so
//     rather than implying a fetch that never happened. Only did:web has a document
//     to retrieve.
//   * RETRIEVE a document by URL, for one that is not where the method's rules say
//     it should be: a staging host, a tunnel, an unpublished path, or a
//     CORS-friendly proxy. That is a retrieval, not a resolution, and the
//     difference is reported rather than smoothed over.
//   * UPLOAD one from a file, because a host that sends no CORS headers cannot be
//     read by a browser however right the URL is.
//   * verify that a key the document publishes REALLY SIGNED the credential this
//     browser is holding. Resolving a document proves nothing on its own; this is
//     the check that means something.
//   * verify the DOMAIN LINKAGE (DIF Well Known DID Configuration): that an origin
//     and a DID are the same entity. For did:web this is the only non-circular
//     proof, because resolving did:web:example.com means fetching example.com.
//
// It shares the credential in localStorage with the VC workflows and writes none
// of their state, so it can be opened beside them at any point.
// ---------------------------------------------------------------------------
...
//
// It is INDEPENDENT of the VC workflows by design, and that has to be true of the
// storage as much as of the markup. VC Issuance step 1 keeps its own DID pane,
// where a resolved document IS that workflow's configuration (`did_id`,
...
// per-document, they cannot collide, and keeping them identical is what let this
// page reuse the pane's markup and logic unchanged.
//
// The one thing read across the boundary is the CREDENTIAL the workflows stored,
// and that is the point of "Verify Signing Key" — it asks whether a key this
// document publishes signed the credential in this browser. It is read, never
// written.
// ---------------------------------------------------------------------------
var KEY_PREFIX = "didtools_";
function storeKey(name) { return KEY_PREFIX + name; }
...
// Storage keys, namespaced. Never pass one of these to val()/setVal(): they name
// nothing in the document, and the failure is silent — an empty field read back as
// "that is not a DID".
...
// The upload route, through the shared reader so this page and the metadata panes
// cannot disagree about what a JSON document is.
...
function verifyDomainLinkage() {
  ...
  // The origin whose word is in question, taken from the field so any origin can
  // be asked about. It is defaulted (on load) from the credential issuer this
  // browser last discovered, which is the common case, but it is not limited to it:
  // a linkage is a claim between an origin and a DID, and neither has to have
  // anything to do with a credential.
  var issuer = (val(ORIGIN_FIELD) || "").trim();
  if (!issuer) {
    status("did_linkage_status", "Give an origin to check the DID against — the linkage is a claim " +
      "that an ORIGIN and a DID are the same entity, so it takes both.", "vc-bad");
    return false;
  }
...
//
// Built from DID_METADATA in did.js — the same list, with the same descriptions,
// that the resolved document populates — so the page cannot describe a member the
// module does not know about. Editable on purpose: this is where you try a document
// an issuer has not published yet, or break one deliberately to watch the
...
function renderMembersTable() {
  ...
  html += metadataClient.groupRow("DID Document", "W3C DID Core 1.0, from the pane above");
  didLib.DID_METADATA.forEach(function (m) {
    html += metadataClient.fieldRow(didLib.idFor(m.name), m.name, m.desc, m.type);
  });
  html += "</tbody></table>";
  host.innerHTML = html;
  ...
  didLib.DID_METADATA.forEach(function (m) {
    ...
    if (stored !== null && stored !== undefined) setVal(didLib.idFor(m.name), stored);
  });
  log.debug("Leaving renderMembersTable(). " + didLib.DID_METADATA.length + " member(s).");
}

// Save the fields this page owns, so a reload does not lose them. The document
// members are written by populateFromDidDocument() as they are resolved; these two
// are typed by hand.
function saveState() {
...
function renderHeldCredential() {
  ...
  if (!raw) {
    status("did_credential_status", "No credential is held in this browser. Resolving and reading a " +
      "document still works; Verify Signing Key needs one, because it asks whether a key this " +
  ...
  try {
    ...
    status("did_credential_status", "A credential is held but could not be parsed: " + e.message,
           "vc-bad");
  ...
  var named = parsed.format === sdJwtVc.FORMAT_LDP_VC
    ? ((parsed.document || {}).issuer || "")
    : ((parsed.payload || {}).iss || "");
  status("did_credential_status", "Holding a " + parsed.format + " credential issued by " +
    (named || "(no issuer named)") + (didLib.isDid(named) ? " — which is a DID, so it can be resolved above." :
      " — which is not a DID, so this document describes something else."), "vc-ok");
...
function onload() {
  ...
  // Restore what was typed last time, and default the origin from the credential
  // issuer this browser last discovered — the common thing to ask about, without
  // limiting the page to it.
