...
// XML-Encryption, verification, and decryption that back saml_tools.html. No
...
var log = bunyan.createLogger({ name: 'saml_assertion', level: appconfig.logLevel });
...
function renderAttributes() {
  ...
  for (var i = 0; i < attributes.length; i++) {
    ...
    html += '<tr>' +
      ...
      '<td><button type="button" class="saml-copy" onclick="return saml_assertion.removeAttribute(' + i + ');">Remove</button></td>' +
      '</tr>';
  }
  body.innerHTML = html;
}
...
// ---------------------------------------------------------------------------
// UI plumbing (mirrors saml_tools.js)
// ---------------------------------------------------------------------------
...
function viewCertificate(fieldId) {
  ...
  window.open('/saml_cert.html?from=saml_assertion.html', '_blank');
  return false;
}
...
function setReturnLink() {
  ...
  if (link) link.setAttribute('href', '/saml_tools.html');
}
