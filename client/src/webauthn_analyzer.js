...
//
// This is the mode that works with ANY relying party. The debugger is not the RP
// here and performs no ceremony — it decodes bytes. So it needs no secure
...
//   1. `PublicKeyCredential.toJSON()` — the Level 3 serialization, which is what
//      the extension captures and what a modern browser's console gives you from
//      `JSON.stringify(cred)`. Registration and assertion both.
//   2. This workflow's capture envelope ({ v, ceremony, request, response, … }),
//      in which case the REQUEST is shown too — the half no relying party ever
//      shows its user.
...
// textContent so there is no path from pasted bytes to markup. That is a stronger
// guarantee than sanitising, and it costs nothing because none of these panes
// need rich content.
//
// The public key is the one thing an assertion cannot carry: verifying one needs
// the key from the REGISTRATION that created the credential. So a decoded
// registration's key is remembered (localStorage, `webauthn_analyzer_keys`, by
// credential ID) and offered automatically when an assertion for that credential
// turns up. Storing it needs no opt-out checkbox and is not covered by the
// key-material rule in the root CLAUDE.md: a WebAuthn credential's private key
// never leaves the authenticator, so what is kept here is a PUBLIC key — the same
// class of thing as an IdP's certificate, which this debugger has always stored.
// ---------------------------------------------------------------------------
...
var log = bunyan.createLogger({ name: "webauthn_analyzer", level: appconfig.logLevel });
...
function loadKeys() {
  try {
    ...
    // Unreadable store: start again rather than refusing to run. Nothing here is
    // irreplaceable — every entry can be recovered by pasting its registration.
...
function rememberKey(credentialId, jwk) {
  ...
  try {
    ...
    // Quota, or storage disabled. The decode the user just asked for has already
    // happened and is on screen; only the convenience of reuse is lost.
...
function interpret(text) {
  ...
  try {
    ...
    throw new Error("That is not JSON. Paste a PublicKeyCredential (its toJSON() form), or a " +
                    "capture from the browser extension. " + e.message);
  }
  // The capture envelope.
  if (parsed && parsed.v && parsed.ceremony && (parsed.response || parsed.error)) {
  ...
  if (parsed && parsed.response && parsed.id) {
    ...
    log.debug("Leaving interpret(). shape=credential registration=" + isRegistration);
    return { ceremony: isRegistration ? "create" : "get", response: parsed, request: null };
  }
  ...
  throw new Error("Unrecognised shape. Expected a PublicKeyCredential with a `response` member " +
...
function renderRequest(request) {
  ...
  if (!request) {
    pane.appendChild(el("p", "wa-note",
      "No request options in this input. A pasted credential carries only the ANSWER; the options " +
      "the relying party asked with — challenge, userVerification, allowCredentials, attestation, " +
      "extensions, timeout — are visible only to something present when the call was made. That is " +
  ...
  Object.keys(pk).forEach(function (k) {
    var v = pk[k];
    row(t, k, (typeof v === "object" && v !== null) ? JSON.stringify(v) : String(v));
  });
}
...
function analyze() {
  ...
  if (!interpreted.response) {
    ["wa_clientdata_body", "wa_authdata_body", "wa_cose_body", "wa_attestation_body",
    ...
    $("wa_clientdata_body").appendChild(el("p", "wa-bad",
      err
        ? "This ceremony did not complete: " + err.name + " — " + err.message +
          ".  WebAuthn reports one error for several situations on purpose (no matching " +
          "credential, a declined prompt, a timeout), so this does not say which. The request " +
          "above is still the whole story of what was asked for."
        : "This capture carries no response and no error, which should not happen."));
    status("wa_input_status",
      "A refused ceremony. There are no artifacts to decode, but the request options above are " +
  ...
  try {
    clientData = webauthn.parseClientDataJSON(webauthn.base64urlToBytes(response.clientDataJSON));
    renderClientData(clientData);
  } catch (e) {
    status("wa_input_status", "clientDataJSON did not decode: " + e.message, "bad");
  ...
  try {
    if (interpreted.ceremony === "create") {
      ...
      status("wa_input_status",
        "Registration decoded. The credential public key has been kept, so an assertion for this " +
      ...
      $("wa_attestation_body").appendChild(el("p", "wa-note",
        "An assertion carries no attestation object. Attestation happens once, at registration."));
    ...
    status("wa_input_status", "The credential did not decode: " + e.message, "bad");
  ...
  if (interpreted.ceremony === "get") {
    ...
    $("wa_verify_body").appendChild(el("p", "wa-note",
      "Verifying a REGISTRATION means verifying its attestation statement against a trusted " +
      "metadata service, which this page deliberately does not do — see the attestation pane. " +
      "Paste an assertion for this credential to exercise the signature check."));
...
// stored-key lookup silently found nothing and the page reported "cannot verify"
// for a credential whose key it was holding.
function verifyPasted(credential, response, authData) {
  ...
  if (pasted) {
    try {
      ...
      status("wa_verify_status", "The credential public key box does not hold JSON: " + e.message, "bad");
      return;
    }
  } else {
    var keys = loadKeys();
  ...
  if (!jwk) {
    clear($("wa_verify_body"));
    $("wa_verify_body").appendChild(el("p", "wa-note",
      "No public key for this credential. An assertion is a signature and nothing else — the key " +
      "that checks it comes from the registration that created the credential. Paste that " +
      "registration here first, or put the credential's public key (as a JWK) in the box above."));
    status("wa_verify_status", "Cannot verify without the credential public key.", "");
    return;
  }
  ...
  webauthn.verifyAssertion({
    ...
    clear($("wa_verify_body"));
    $("wa_verify_body").appendChild(el("p", "wa-bad", "Verification could not run: " + e.message));
...
function fillExpectationsFromInput() {
  ...
  try {
    ...
    var response = interpreted.response.response || interpreted.response;
    var cd = webauthn.parseClientDataJSON(webauthn.base64urlToBytes(response.clientDataJSON));
    ...
    status("wa_input_status", "Expectations filled in from the artifact itself — correct them to " +
                              "what the relying party actually issued before trusting the verdict.", "");
  } catch (e) {
    status("wa_input_status", e.message, "bad");
  }
...
function clearAll() {
  ...
  ["wa_input", "wa_public_key", "wa_expect_challenge", "wa_expect_origin", "wa_expect_rpid",
   "wa_expect_signcount", "wa_clientdata_raw", "wa_cose_pem", "wa_attestation_raw"].forEach(function (id) {
    if ($(id)) {
      $(id).value = "";
    }
  });
...
// both browsers (externally_connectable is Chrome-only), so this is a namespaced
// window.postMessage handshake answered by a content script the extension puts
// on THIS origin. The same contract the extension's own tests use; if the two
// ever drift, tests/webauthn_extension.js is where it shows.
...
function askBridge(action) {
  return new Promise(function (resolve) {
    ...
    window.postMessage({ channel: BRIDGE_REQ, id: id, action: action }, window.location.origin);
  });
}
...
function renderCaptures(answer) {
  ...
  if (!version) {
    status("wa_ext_status",
      "No extension detected on this page. Modes above work without it; the extension is only needed " +
  ...
  if (!answer) {
    status("wa_ext_status",
      "The extension is installed (version " + version + ") but did not answer. If it was just " +
  ...
  if (!answer.armed) {
    status("wa_ext_status",
      "The extension is installed (version " + version + ") and is observing nothing. Open its popup, " +
      "enter the origin you want to watch, and confirm the permission the browser asks for.", "");
  } else {
    status("wa_ext_status",
      ...
      (drift
        ? "  NOTE: " + unsupported.length + " capture(s) use format v" + unsupported[0].v +
          " and this page understands v" + SUPPORTED_CAPTURE_VERSION + ". The extension " +
          "(version " + (answer.version || "unknown") + ") does not update itself — reload it from " +
          "extension/dist."
  ...
  if (!captures.length) {
    pane.appendChild(el("p", "wa-note",
      "No captures yet. Arm the extension for the origin you want to watch, then perform the " +
...
window.onload = function () {
  ...
  $("wa_ext_done").addEventListener("click", function () {
    ...
    askBridge("disarm").then(function () {
      status("wa_ext_status", "Observation stopped and the extension's capture buffer cleared. " +
                              "Remove the extension entirely from its popup if you want it gone.", "good");
      refreshCaptures();
    });
  });
  refreshCaptures();
  ...
  $("wa_forget_button").addEventListener("click", function () {
    ...
    status("wa_input_status", "Remembered credential public keys have been forgotten.", "");
  });
  try {
    ...
    // Storage unavailable; the page works exactly the same, it just opens empty.
