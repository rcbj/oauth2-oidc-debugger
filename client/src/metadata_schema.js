...
//
// ---------------------------------------------------------------------------
// Checking a metadata document against what its specification actually
// requires.
//
// READ THIS BEFORE LOOKING FOR THE SCHEMA FILE: there isn't one, and not
// because nobody fetched it. Neither specification publishes a machine-readable
// schema. OpenID4VCI 1.0 defines its metadata in prose and tables (section
// 12.2); the spec repository ships thirty-four JSON files and every one of them
// is an EXAMPLE, not a schema. RFC 8414 does the same in section 2, with an
// IANA registry for the member names and no schema either. So "validate against
// the published schema" cannot be done as stated, and pointing a validator at
// some third party's copy would be worse than this: it would check the document
...
// reading the spec rather than by reading this file. Where a rule is a SHOULD
// or a convention rather than a MUST, it is reported as a warning, because a
...
// endpoint is a syntactically valid https URL is checkable here. That it
// answers is not, and pretending otherwise would make this slow and flaky.
// ---------------------------------------------------------------------------
...
function isArray(v) {
  log.debug("Entering isArray().");
  log.debug("Leaving isArray().");
  return Object.prototype.toString.call(v) === "[object Array]";
}
function isString(v) {
  log.debug("Entering isString().");
  log.debug("Leaving isString().");
  return typeof v === "string";
}
function isNonEmptyString(v) {
  log.debug("Entering isNonEmptyString().");
  log.debug("Leaving isNonEmptyString().");
...
function parsedUrl(v) {
  log.debug("Entering parsedUrl().");
  if (!isNonEmptyString(v)) {
    log.debug("Leaving parsedUrl().");
    return null;
  }
  try {
    log.debug("Leaving parsedUrl().");
...
function checkUrl(r, doc, member, cite, opts) {
  ...
  if (!isString(value)) {
    r.error(member, "must be a string containing a URL; this is " +
            typeName(value) + ".", cite);
    log.debug("Leaving checkUrl().");
    return;
  }
  ...
  if ((opts || {}).https && u.protocol !== "https:") {
    ...
    // is how a reader learns to skip the warnings and miss the one that
    // matters. One line at the end names them all.
...
function typeName(v) {
  log.debug("Entering typeName().");
  if (v === null) {
    log.debug("Leaving typeName().");
    return "null";
  }
  if (isArray(v)) {
    log.debug("Leaving typeName().");
...
function checkStringArray(r, doc, member, cite, required) {
  ...
  if (!isStringArray(value)) {
    r.error(member, "must be an array of strings; this is " + typeName(value) +
            ".", cite);
...
function checkBoolean(r, doc, member, cite) {
  log.debug("Entering checkBoolean().");
  if (doc[member] !== undefined && typeof doc[member] !== "boolean") {
    r.error(member, "must be a boolean; this is " + typeName(doc[member]) + ".",
            cite);
  }
  log.debug("Leaving checkBoolean().");
}
...
function foldPlainHttp(r, cite) {
  log.debug("Entering foldPlainHttp().");
  if (!r.plainHttp.length) {
    log.debug("Leaving foldPlainHttp().");
    return;
  }
  r.warn(r.plainHttp.length === 1 ? r.plainHttp[0] : r.plainHttp.length +
         " members",
    (r.plainHttp.length === 1 ? "uses" : "use") +
     " http:// rather than https:// (" +
    r.plainHttp.join(", ") +
        "). The specification requires https for a deployed service; this is " +
...
function checkCredentialConfiguration(r, id, cfg) {
  ...
  if (!isNonEmptyString(cfg.format)) {
    r.error(where + ".format",
            "is REQUIRED and must be a string naming the credential format.",
            VCI);
    log.debug("Leaving checkCredentialConfiguration().");
    return;
  }
  ...
  if (format === "dc+sd-jwt" || format === "vc+sd-jwt") {
    if (!isNonEmptyString(cfg.vct)) {
      r.error(where + ".vct",
        'is REQUIRED for format "' + format +
            '": an SD-JWT VC is identified by its vct.',
        "OpenID4VCI 1.0 Appendix A.3");
    }
    if (cfg.credential_definition !== undefined) {
      r.warn(where + ".credential_definition",
        'is not used by format "' + format +
            '" — that member names a W3C credential; an SD-JWT VC ' +
        "uses vct.", "OpenID4VCI 1.0 Appendix A.3");
    }
  } else if (format === "jwt_vc_json" || format === "jwt_vc_json-ld" ||
             format === "ldp_vc") {
    var def = cfg.credential_definition;
    if (!isObject(def)) {
      r.error(where + ".credential_definition",
        'is REQUIRED for format "' + format + '" and must be an object.',
            "OpenID4VCI 1.0 Appendix A.1");
    } else if (!isStringArray(def.type) || !def.type.length) {
      r.error(where + ".credential_definition.type",
        "is REQUIRED and must be a non-empty array of strings: a W3C " +
            "credential is identified by its " +
      ...
      r.warn(where + ".credential_definition.type",
        'does not include "VerifiableCredential", which every W3C Verifiable ' +
            'Credential carries.',
        "W3C VCDM");
    }
    if (cfg.vct !== undefined) {
      r.warn(where + ".vct",
        'is not used by format "' + format +
            '" — that member identifies an SD-JWT VC.',
        "OpenID4VCI 1.0 Appendix A.1");
    }
    ...
    r.warn(where + ".format",
      'is "' + format +
          '", which is not one of the formats this specification defines ' +
      "(dc+sd-jwt, jwt_vc_json, jwt_vc_json-ld, ldp_vc, mso_mdoc). It may be " +
          "a private extension.", VCI);
  }
  ...
  if (cfg.claims !== undefined && !isArray(cfg.claims)) {
    r.error(where + ".claims",
      "must be an array of claim objects, each with a path. (Earlier drafts " +
          "used an object here.)", VCI);
  }
  log.debug("Leaving checkCredentialConfiguration().");
}
...
function validateVciMetadata(doc) {
  ...
  var r = report();
  if (!isObject(doc)) {
    r.error("(document)", "must be a JSON object; this is " + typeName(doc) +
            ".", VCI);
  ...
  if (configs === undefined) {
    r.error("credential_configurations_supported",
      "is REQUIRED: without it the issuer advertises no credential a wallet " +
          "could ask for.", VCI);
  } else if (!isObject(configs)) {
    r.error("credential_configurations_supported",
      "must be an object keyed by credential_configuration_id; this is " +
          typeName(configs) + ".", VCI);
  } else if (!Object.keys(configs).length) {
    r.error("credential_configurations_supported", "is present but empty.",
            VCI);
  } else {
  ...
  if (doc.authorization_servers !== undefined) {
    if (!isStringArray(doc.authorization_servers)) {
      ...
      doc.authorization_servers.forEach(function (v, i) {
        if (!parsedUrl(v)) r.error("authorization_servers[" + i + "]",
            'is not a valid URL: "' + v + '".', VCI);
      });
    }
  }
  ...
  if (doc.credential_response_encryption !== undefined) {
    ...
    if (!isObject(enc)) {
      ...
      checkStringArray(r, enc, "alg_values_supported", VCI +
                       " (credential_response_encryption)", true);
      checkStringArray(r, enc, "enc_values_supported", VCI +
                       " (credential_response_encryption)", true);
  ...
  // the JWK the wallet chose — and they carry a jwks whose every key MUST have
  // a kid. Checking it against the response side's rules would pass a document
  ...
  if (doc.credential_request_encryption !== undefined) {
    ...
    if (!isObject(req)) {
      ...
      if (!isObject(req.jwks) || !isArray(req.jwks.keys) ||
          !req.jwks.keys.length) {
        r.error("credential_request_encryption.jwks",
          "is REQUIRED and must be a JWK Set with at least one key for the " +
              "wallet to encrypt to.",
          label);
      } else {
        req.jwks.keys.forEach(function (k, i) {
          if (!isObject(k)) {
            r.error("credential_request_encryption.jwks.keys[" + i + "]",
                    "must be an object.", label);
            return;
          }
          if (typeof k.kid !== "string" || !k.kid) {
            r.error("credential_request_encryption.jwks.keys[" + i + "].kid",
              "is REQUIRED: \"Each JWK in the set MUST have a kid (Key ID) " +
                  "parameter that uniquely " +
              "identifies the key\", and the wallet has to echo it in the " +
                  "JWE header.", label);
          }
          if (typeof k.alg !== "string" || !k.alg) {
            r.error("credential_request_encryption.jwks.keys[" + i + "].alg",
              "is REQUIRED for request encryption: \"The alg parameter MUST " +
                  "be present. The JWE alg " +
              "algorithm used MUST be equal to the alg value of the chosen " +
                  "JWK.\" Without it the " +
              "wallet has no algorithm to use, since requests have no " +
                  "alg_values_supported.", label);
          }
        });
        var kids = req.jwks.keys.map(function (k) { return isObject(k) ?
            k.kid : null; })
          .filter(function (k) { return typeof k === "string" && k; });
        if (kids.length !== new Set(kids).size) {
          r.error("credential_request_encryption.jwks",
            "has duplicate kid values; each kid must UNIQUELY identify a " +
                "key, or the issuer cannot " +
      ...
      if (req.alg_values_supported !== undefined) {
        r.warn("credential_request_encryption.alg_values_supported",
          "is not defined for request encryption — that member belongs to " +
              "credential_response_" +
          "encryption. The request algorithm comes from the alg of the " +
              "chosen JWK.", label);
      }
    }
  }
  ...
  if (doc.signed_metadata !== undefined && !isCompactJws(doc.signed_metadata)) {
    r.error("signed_metadata",
        "must be a JWT in compact serialization (three dot-separated parts).",
        VCI);
  }

  foldPlainHttp(r, VCI);
  log.debug("Leaving validateVciMetadata(). " + r.errors.length +
            " error(s), " +
...
function isCompactJws(v) {
  log.debug("Entering isCompactJws().");
  log.debug("Leaving isCompactJws().");
  return isString(v) && v.split(".").length === 3 &&
                  v.split(".").every(function (p) { return p !== ""; });
}
...
function validateAsMetadata(doc) {
  ...
  if (doc.authorization_endpoint === undefined) {
    r.warn("authorization_endpoint",
      "is absent. RFC 8414 requires it unless no supported grant type uses " +
          "the authorization endpoint.", AS);
  } else {
    checkUrl(r, doc, "authorization_endpoint", AS, { https: true });
  }
  if (doc.token_endpoint === undefined) {
    r.warn("token_endpoint",
      "is absent. RFC 8414 requires it unless only the implicit grant is " +
          "supported.", AS);
  } else {
  ...
  if (doc.jwks_uri === undefined) {
    r.warn("jwks_uri",
      "is absent. It is RECOMMENDED, and without it a client cannot verify a " +
          "signed token or " +
      "signed_metadata.", AS);
  } else {
  ...
  ["registration_endpoint", "revocation_endpoint", "introspection_endpoint",
   "device_authorization_endpoint", "userinfo_endpoint",
       "service_documentation",
   "op_policy_uri", "op_tos_uri"].forEach(function (member) {
    checkUrl(r, doc, member, AS, { https: true });
  });

  ["grant_types_supported", "response_modes_supported",
   ...
   "ui_locales_supported", "claims_supported",
  ...
  // A cross-member rule worth stating:
  // token_endpoint_auth_signing_alg_values_supported MUST NOT include "none"
  // (RFC 8414 §2).
  if (isStringArray(doc.token_endpoint_auth_signing_alg_values_supported) &&
      doc.token_endpoint_auth_signing_alg_values_supported.indexOf(
          "none") !== -1) {
...
function summarize(result, label) {
  ...
  if (!e && !w) {
    ...
    return label + " matches " + (result.spec || "its specification") + ".";
  }
  var parts = [];
...
// document that LOOKS right fail somebody else's verifier, and they are the
// reason this is worth checking at all:
//
...
//     nothing beyond iss/sub/nbf/exp/vc — and both are things a JWT library
//     adds for you unless told otherwise. This is the single most likely way to
//     publish a document that reads correctly and is refused.
//
// What is NOT checked here, because it is not a schema question: whether the
// signature verifies, whether the DID resolves, and whether the origin matches
// the one the document was served from. did.js's verifyDomainLinkage() does all
// three, and the pane runs it alongside this.
// ---------------------------------------------------------------------------
...
var DIDCFG_LD_MEMBERS = ["@context", "issuer", "issuanceDate", "expirationDate",
    "type",
                         "credentialSubject", "proof"];

function isDidString(v) {
  log.debug("Entering isDidString().");
  log.debug("Leaving isDidString().");
  return isNonEmptyString(v) && /^did:[a-z0-9]+:/.test(v);
}

// One entry of linked_dids, in whichever of the two forms it takes. `label`
// names it for the report, since a resource may carry several.
function checkLinkedDid(r, entry, label) {
  log.debug("Entering checkLinkedDid().");
  var where = DIDCFG + " §Domain Linkage Credential";
  // A string is the JWT form; an object is the Linked Data Proof form. Anything
  // else is neither.
  if (isString(entry)) {
    var parts = entry.split(".");
    if (parts.length !== 3) {
      r.error(label, "a JWT-form entry must be a three-part JWS; this has " +
              parts.length +
              " dot-separated part(s).", where);
      log.debug("Leaving checkLinkedDid().");
      return;
    }
    var header, claims;
    try {
      ...
      r.error(label, "the JWT's header or payload is not base64url JSON: " +
              e.message, where);
      log.debug("Leaving checkLinkedDid().");
      return;
    }
    if (!isNonEmptyString(header.alg)) r.error(label + ".alg",
        "REQUIRED in the JWT header.", where);
    if (!isNonEmptyString(header.kid)) {
      r.error(label + ".kid",
              "REQUIRED in the JWT header: it names the verification method.",
              where);
    }
    if (Object.prototype.hasOwnProperty.call(header, "typ")) {
      r.error(label + ".typ",
              "MUST NOT be present in the JWT header. Most JWT libraries add " +
              'typ:"JWT" unless told not to, so this is the usual way a ' +
                  'correct-looking document is ' +
              "refused.", where);
    }
    ...
    if (extraHeader.length) {
      r.error(label + " header",
              "no members beyond alg and kid are permitted; this carries " +
    ...
    if (extraClaims.length) {
      r.error(label + " claims",
          "no members beyond iss, sub, nbf, exp and vc are permitted; this " +
              "carries " + extraClaims.join(", ") +
                  ". iat is the usual culprit — most libraries add " +
              "it unless told noTimestamp.", where);
    }
    if (!isDidString(claims.iss)) r.error(label + ".iss",
        "REQUIRED, and MUST be a DID.", where);
    if (claims.sub !== claims.iss) {
      r.error(label + ".sub", "MUST equal iss: a Domain Linkage Credential " +
              "is self-issued, because " +
              "nobody but the DID's controller can say which origin it " +
                  "controls.", where);
    }
    if (!isObject(claims.vc)) {
      r.error(label + ".vc",
          "REQUIRED: the credential, in the Linked Data form minus its proof.",
          where);
      log.debug("Leaving checkLinkedDid().");
      return;
    }
  ...
  if (isObject(entry)) {
    ...
    if (!isObject(entry.proof)) {
      r.error(label + ".proof", "REQUIRED in the Linked Data Proof form.",
              where);
    }
    log.debug("Leaving checkLinkedDid().");
    return;
  }
  r.error(label, "each linked_dids entry is either a JWT string or a " +
          "credential object; this is " +
          typeName(entry) + ".", where);
  log.debug("Leaving checkLinkedDid().");
}

// The credential itself, shared by both forms — in the JWT form it is the `vc`
// claim with no proof, which is why `embedded` decides whether a proof is
// expected.
function checkLinkageCredential(r, vc, label, issuer, embedded) {
  log.debug("Entering checkLinkageCredential().");
  var where = DIDCFG + " §Domain Linkage Credential";
  var contexts = isArray(vc["@context"]) ? vc["@context"] : [];
  if (contexts.indexOf(VC_V1_CONTEXT) === -1 ||
      contexts.indexOf(DIDCFG_CONTEXT) === -1) {
    r.error(label + ".@context", "MUST contain both " + VC_V1_CONTEXT +
            " and " + DIDCFG_CONTEXT + ".",
            where);
  }
  ...
  if (types.indexOf("VerifiableCredential") === -1 ||
    ...
    r.error(label + ".type",
            "MUST contain VerifiableCredential and DomainLinkageCredential.",
            where);
  }
  if (Object.prototype.hasOwnProperty.call(vc, "id")) {
    r.error(label + ".id", "MUST NOT be present at the credential root. This " +
            "credential is not an " +
            "addressable thing to be fetched or revoked; it is an assertion " +
                "about one pair.", where);
  }
  if (!isDidString(vc.issuer)) r.error(label + ".issuer",
      "REQUIRED, and MUST be a DID.", where);
  if (!isNonEmptyString(vc.issuanceDate)) r.error(label + ".issuanceDate",
      "REQUIRED.", where);
  if (!isNonEmptyString(vc.expirationDate)) {
    r.warn(label + ".expirationDate",
           "REQUIRED by the specification. A credential with no expiry " +
  ...
  if (!subject) {
    ...
    log.debug("Leaving checkLinkageCredential().");
    return;
  }
  if (!isDidString(subject.id)) {
    r.error(label + ".credentialSubject.id",
            "REQUIRED, and MUST be the DID this credential links.",
            where);
  } else if (issuer && subject.id !== issuer) {
    r.error(label + ".credentialSubject.id", "MUST equal the issuer (" +
            issuer + "): the subject and " +
            "the issuer of a Domain Linkage Credential are the same DID.",
                where);
  }
  if (!isNonEmptyString(subject.origin)) {
    r.error(label + ".credentialSubject.origin",
            "REQUIRED: the origin this DID claims.", where);
  } else {
    var parsed = parsedUrl(subject.origin);
    if (!parsed) {
      r.error(label + ".credentialSubject.origin",
              "MUST be an origin (scheme, host and optional " +
              'port); "' + subject.origin + '" does not parse as one.', where);
    } else {
      foldPlainHttp(r, where);
      if (parsed.pathname && parsed.pathname !== "/") {
        r.warn(label + ".credentialSubject.origin",
               "an ORIGIN is scheme, host and port — this carries " +
               "the path " + parsed.pathname +
                   ", which a verifier comparing origins will not match.",
               where);
      }
    }
  }
  if (embedded && Object.prototype.hasOwnProperty.call(vc, "proof")) {
    r.error(label + ".proof",
        "MUST NOT be present inside the JWT's vc claim: the JWS IS the proof.",
            where);
  }
  log.debug("Leaving checkLinkageCredential().");
}
...
function validateDidConfiguration(doc) {
  ...
  if (doc["@context"] !== DIDCFG_CONTEXT) {
    r.error("@context", 'REQUIRED, and MUST be exactly "' + DIDCFG_CONTEXT +
            '"; this says ' +
            JSON.stringify(doc["@context"]) + ".", DIDCFG +
                           " §DID Configuration Resource");
  }
  if (!isArray(doc.linked_dids)) {
    r.error("linked_dids",
            "REQUIRED, and MUST be an array of Domain Linkage Credentials.",
            DIDCFG + " §DID Configuration Resource");
  } else if (!doc.linked_dids.length) {
    r.warn("linked_dids",
           "present but empty, so this origin vouches for no DID at all.",
           DIDCFG + " §DID Configuration Resource");
  } else {
  ...
  if (extra.length) {
    r.error("(document)", "no members beyond @context and linked_dids are " +
            "permitted; this carries " +
            extra.join(", ") + ".", DIDCFG + " §DID Configuration Resource");
  }
  log.debug("Leaving validateDidConfiguration(). " + r.errors.length +
            " error(s), " +
            r.warnings.length + " warning(s).");
  return r;
}
