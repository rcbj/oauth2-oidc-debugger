...
//             most other wallets interoperate on, and what walt.id advertises
//             in cryptographic_binding_methods_supported.
...
// no base58 anywhere. did:key uses "z", and the alphabet is Bitcoin's — it
// omits 0, O, I and l precisely so the four characters that look alike cannot
// be confused when a DID is read aloud or retyped.
// ---------------------------------------------------------------------------
...
function base58Decode(str) {
  ...
  for (var k = 0; k < bytes.length; k++) out[zeros + k] = bytes[bytes.length -
       1 - k];
  log.debug("Leaving base58Decode().");
  return out;
}
...
var CODECS = [
  ...
  { code: 0xe7, name: "secp256k1-pub", kty: "EC", crv: "secp256k1", length: 33,
   compressed: true },
  { code: 0x1200, name: "p256-pub", kty: "EC", crv: "P-256", length: 33,
   compressed: true },
  { code: 0x1201, name: "p384-pub", kty: "EC", crv: "P-384", length: 49,
   compressed: true },
  { code: 0x1202, name: "p521-pub", kty: "EC", crv: "P-521", length: 67,
   compressed: true }
];

function codecFor(match) {
  log.debug("Entering codecFor().");
  for (var i = 0; i < CODECS.length; i++) {
    if (match(CODECS[i])) {
      log.debug("Leaving codecFor().");
      return CODECS[i];
    }
  }
  log.debug("Leaving codecFor().");
...
function varintEncode(value) {
  log.debug("Entering varintEncode().");
  var out = [];
  var n = value;
  while (n > 0x7f) {
  ...
  out.push(n);
  log.debug("Leaving varintEncode().");
  return out;
}
...
function varintDecode(bytes) {
  ...
  for (; i < bytes.length; i++) {
    ...
    if ((b & 0x80) === 0) {
      ...
      return { value: result >>> 0, length: i + 1 };
    }
    shift += 7;
    if (shift > 28) break;
  }
...
// `+`, and P-384's and P-521's were each NINE DIGITS SHORT — a truncation that
// is invisible in a 116-digit decimal number, and which no test caught because
// the only curve anything here uses in practice is P-256, whose constants were
// right. The consequence was not a wrong answer but a refusal: decompressing
// any P-384 or P-521 did:key threw "no square root exists", because with the
// wrong field nothing is a square. Hex is how FIPS 186-4 and SEC 2 publish
// these, so a digit can be checked against the source by eye, and a truncated
// hex string is caught by the byte-length check in tests/did_document.js.
var CURVES = {
  "P-256": {
    p: BigInt(
...
function compressPoint(crv, xBytes, yBytes) {
  log.debug("Entering compressPoint().");
  ...
  out.set(xBytes, 1);
  log.debug("Leaving compressPoint().");
  return out;
}
...
function decompressPoint(crv, compressed) {
  ...
  if (!curve) throw new Error("no parameters for curve " + crv +
      ", so its point cannot be decompressed.");
  var prefix = compressed[0];
  if (prefix !== 0x02 && prefix !== 0x03) {
    throw new Error("a compressed point starts with 0x02 or 0x03; this " +
                    "starts with 0x" +
                    prefix.toString(16) + ".");
  }
  ...
  if (modPow(y, BigInt(2), p) !== alpha) {
    throw new Error("this x is not on curve " + crv +
...
function parse(did) {
  ...
  log.debug("Leaving parse().");
  return { did: "did:" + m[1] + ":" + m[2], method: m[1], id: m[2],
          fragment: fragment };
}

function isDid(value) {
  log.debug("Entering isDid().");
  log.debug("Leaving isDid().");
  return !!parse(value);
}

// --- did:jwk ---------------------------------------------------------------
function jwkToDidJwk(jwk) {
  log.debug("Entering jwkToDidJwk().");
  var pub = publicPart(jwk);
  log.debug("Leaving jwkToDidJwk().");
  return "did:jwk:" + metadataClient.utf8ToB64u(JSON.stringify(pub));
}

function didJwkToJwk(did) {
  log.debug("Entering didJwkToJwk().");
  var parsed = parse(did);
  if (!parsed || parsed.method !== "jwk") {
    log.debug("Leaving didJwkToJwk().");
    return null;
  }
  try {
    log.debug("Leaving didJwkToJwk().");
...
function jwkToDidKey(jwk) {
  ...
  if (!codec) {
    throw new Error("did:key has no multicodec for a " + jwk.kty + " key on " +
                    jwk.crv + ".");
  }
  var keyBytes;
  if (codec.compressed) {
    if (!jwk.x || !jwk.y) throw new Error("an EC did:key needs both x and y " +
        "to compress the point.");
    keyBytes = compressPoint(jwk.crv, metadataClient.b64uToBytes(jwk.x),
        metadataClient.b64uToBytes(jwk.y));
  } else {
...
function didKeyToJwk(did) {
  ...
  var id = parsed.id;
  if (id.charAt(0) !== "z") {
    throw new Error('a did:key is multibase base58btc, so it begins "z"; ' +
                    'this begins "' +
                    id.charAt(0) + '".');
  }
  ...
  if (!codec) {
    throw new Error("multicodec 0x" + head.value.toString(16) +
                    " is not a public key type this " +
                    "supports (" +
                        CODECS.map(function (c) { return c.name; }).join(", ") +
                        ").");
  }
  var keyBytes = bytes.slice(head.length);
  if (keyBytes.length !== codec.length) {
    throw new Error("a " + codec.name + " key is " + codec.length +
                    " bytes; this carries " +
                    keyBytes.length + ".");
  }
  if (!codec.compressed) {
    log.debug("Leaving didKeyToJwk().");
    return { kty: codec.kty, crv: codec.crv,
            x: metadataClient.bytesToB64u(keyBytes) };
  }
...
function didWebToUrl(did) {
  log.debug("Entering didWebToUrl().");
  var parsed = parse(did);
  if (!parsed || parsed.method !== "web") {
    log.debug("Leaving didWebToUrl().");
    return "";
  }
  ...
  var path = segments.length ? "/" + segments.join("/") +
      "/did.json" : "/.well-known/did.json";
  log.debug("Leaving didWebToUrl().");
...
function didWebToUrlInsecure(did) {
  log.debug("Entering didWebToUrlInsecure().");
  var url = didWebToUrl(did);
  log.debug("Leaving didWebToUrlInsecure().");
...
function resolveLocally(did) {
  ...
  if (parsed.method === "jwk") {
    ...
    log.debug("Leaving resolveLocally().");
    return jwk ? { document: documentForKey(parsed.did, jwk),
        from: "the did:jwk itself (no network call)" }
               : null;
  }
...
function resolve(did, opts) {
  ...
  if (!parsed) {
    ...
    return Promise.reject(new Error('"' + did +
                          '" is not a DID: it must begin "did:".'));
  }
  if (parsed.method !== "web") {
    ...
    if (!local) {
      log.debug("Leaving resolve().");
      return Promise.reject(new Error("did:" + parsed.method +
                            " is not a method this debugger " +
                                      "resolves (did:jwk, did:key and " +
                                          "did:web are)."));
    }
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (vm.publicKeyJwk) {
      ...
      try {
        ...
        log.debug("verificationMethods(): undecodable publicKeyMultibase on " +
                  vm.id);
      }
    }
    out.push({ id: vm.id || "", type: vm.type || "",
             controller: vm.controller || "", jwk: jwk,
...
function assertionKeys(doc) {
  ...
  refs.forEach(function (ref) {
    if (ref && typeof ref === "object") {
      out.push({ id: ref.id || "", type: ref.type || "",
               controller: ref.controller || "",
                 jwk: ref.publicKeyJwk || null,
                     publicKeyMultibase: ref.publicKeyMultibase || "" });
      return;
    }
  ...
  if (kid) {
    for (var i = 0; i < keys.length; i++) {
      ...
      if (keys[i].id && keys[i].id.indexOf("#") !== -1 &&
          keys[i].id.split("#")[1] === String(kid).replace(/^#/, "")) {
            log.debug("Leaving keyForKid().");
            return keys[i];
          }
    }
...
// verifyDomainLinkage() below verifies a Domain Linkage Credential against
// them. Both are answering "did a key this DID authorises to ASSERT sign these
// bytes", and two implementations of that would be two chances to answer
// differently.
//
// `unusable` is not an error: a document may legitimately publish a key with no
// JOSE representation beside one that has — the mock issuer's carries its BBS
// key as a Multikey next to its RS256 JsonWebKey2020 — and a caller that found
// no usable key should be able to say WHICH keys it passed over rather than
...
function assertionJwks(doc) {
  ...
  assertionKeys(doc).forEach(function (method) {
    if (!method) return;
    if (!method.jwk) {
      unusable.push((method.id || "(unnamed)") + (method.type ? " (" +
                    method.type + ")" : ""));
      return;
    }
    var jwk = {};
    Object.keys(method.jwk).forEach(function (name) { jwk[name] =
                method.jwk[name]; });
    // The verification method's id is this key's name within the document, and
    // it is what a DID-URL kid names. Only filled in when the key carries no
    // kid of its own: that one is the issuer's own choice and is what a JWS
    // header will match, so overwriting it would leave a kid lookup nothing to
    // find.
    if (!jwk.kid && method.id) jwk.kid = method.id;
    keys.push(jwk);
  });
  log.debug("Leaving assertionJwks(). " + keys.length + " usable, " +
            unusable.length + " not.");
...
function publicPart(jwk) {
  log.debug("Entering publicPart().");
  if (!jwk) {
    log.debug("Leaving publicPart().");
    return {};
  }
  var out = { kty: jwk.kty };
...
//
// A Data Integrity proof's verificationMethod is "where my key is", and it may
// be either kind of URL. This project's ldp_vc credentials named an https URL
// that dereferences straight to a key document; a credential from a DID-named
// issuer names a DID URL instead, which is NOT fetchable — resolving it means
// resolving the DID and picking out the method its fragment names.
//
// Both are handled here rather than at the call site because the call site is a
// presentation page whose failure mode is a fetch of "did:web:…" that reports
// the issuer's key as unreachable. The two forms are one question with two
// answers.
//
// There is deliberately NO fallback to "the first key in the document": the
// proof named a specific method, and quietly verifying against a different one
// would turn "this proof was made by another key" into a confusing pass or fail
// somewhere else.
function resolveVerificationMethod(url, opts) {
  log.debug("Entering resolveVerificationMethod(). url=" + String(url).slice(0,
            80));
  opts = opts || {};
  var wanted = String(url || "");
  if (!wanted) {
    log.debug("Leaving resolveVerificationMethod().");
    return Promise.reject(new Error("the proof names no verificationMethod."));
  }
  if (!isDid(wanted)) {
    var doFetch = opts.fetch || (typeof fetch === "function" ? fetch : null);
    if (!doFetch) {
    ...
    // — an HTML error page from a misconfigured host is the usual one — says
    // that rather than surfacing a bare SyntaxError from somewhere inside a
    // promise.
    log.debug("Leaving resolveVerificationMethod().");
    return doFetch(wanted).then(function (r) {
      return r.text().then(function (text) {
        ...
        try {
          ...
          throw new Error("the verification method at " + wanted +
                          " is not JSON: " + e.message);
        }
        log.debug("Leaving resolveVerificationMethod(). Fetched an https " +
                  "verification method.");
  ...
  return resolve(parsed.did, opts).then(function (res) {
    ...
    for (var i = 0; i < methods.length; i++) {
      ...
      if (fragment && methods[i].id &&
          methods[i].id.split("#")[1] === fragment) {
        found = methods[i];
        break;
      }
    }
    if (!found) {
      throw new Error('the DID document for ' + parsed.did +
                      ' has no verification method "' + wanted +
                      '". It publishes ' +
                      (methods.length ?
                       methods.map(function (m) { return m.id; }).join(", ")
                                      : "none") + ".");
    }
    log.debug("Leaving resolveVerificationMethod(). Found " + found.id +
              " in the DID document.");
...
//
// Every requirement below is from the specification's own verification steps
// and from its "no additional members permitted" rules, which is why the
// negatives have their own checks rather than one pass/fail — a document that
// fails for a reason nobody can see teaches nothing.
// ---------------------------------------------------------------------------
var DID_CONFIGURATION_PATH = "/.well-known/did-configuration.json";
var DID_CONFIGURATION_CONTEXT =
    "https://identity.foundation/.well-known/did-configuration/v1";
var VC_V1_CONTEXT = "https://www.w3.org/2018/credentials/v1";
// The specification permits exactly these, in each of the two places it says
// so.
var LINKAGE_JWT_CLAIMS = ["iss", "sub", "nbf", "exp", "vc"];
var LINKAGE_JWT_HEADER = ["alg", "kid"];

function didConfigurationUrl(origin) {
  log.debug("Entering didConfigurationUrl().");
  var trimmed = String(origin || "").replace(/\/+$/, "");
  log.debug("Leaving didConfigurationUrl().");
...
function extraMembers(object, permitted) {
  log.debug("Entering extraMembers().");
  log.debug("Leaving extraMembers().");
...
function verifyDomainLinkage(entry, expectedOrigin, opts) {
  ...
  var fail = function (name, detail) {
    ...
    return Promise.resolve({ valid: false, did: "", origin: "",
                           checks: checks });
  };

  // The specification allows either form. Only the JWT one is verified here,
  // and an LD-proof entry is reported as unverified rather than as invalid: it
  // is a legitimate document this code cannot check, which is a different thing
  // from a document that fails its checks.
  if (entry && typeof entry === "object") {
    log.debug("Leaving verifyDomainLinkage().");
    return fail("Credential form",
                "this entry is a Linked Data Proof credential. That form is " +
                    "valid per the " +
                "specification, but verifying it needs URDNA2015 " +
                    "canonicalization, which this " +
                "debugger does not implement — only the JWT form is " +
                    "checked here.");
  }
  ...
  if (parts.length !== 3) {
    log.debug("Leaving verifyDomainLinkage().");
    return fail("Credential form",
                "a linked_dids entry must be a JWT or a credential object; " +
                                   "this has " + parts.length +
                                       " dot-separated part(s).");
  }
  var header, payload;
  try {
    ...
    log.debug("Leaving verifyDomainLinkage().");
    return fail("Credential form",
                "the JWT's header or payload is not base64url JSON: " +
                e.message);
  }
  ...
  checks.push({
    name: "JWT header",
    ok: !!header.alg && !!header.kid && !("typ" in header) &&
        !headerExtra.length,
    detail: "alg " + (header.alg || "(absent)") + ", kid " + (header.kid ||
        "(absent)") +
            '. The specification requires alg and kid, forbids typ — which a ' +
                'JWT library will add ' +
            'unless told not to — and permits nothing else' +
            (headerExtra.length ? "; this carries " +
             headerExtra.join(", ") : "") + "."
  });
  ...
  checks.push({
    ...
    detail: payloadExtra.length
      ? "iat is the usual culprit, added by default by most libraries: " +
          payloadExtra.join(", ") +
        " " + (payloadExtra.length === 1 ? "is" : "are") +
            " not permitted here."
  ...
  checks.push({
    ...
    detail: "iss " + (payload.iss || "(absent)") + ", sub " + (payload.sub ||
        "(absent)") +
            ", vc.issuer " + (vc.issuer || "(absent)") +
  ...
  checks.push({
    ...
    ok: contexts.indexOf(VC_V1_CONTEXT) !== -1 &&
                         contexts.indexOf(DID_CONFIGURATION_CONTEXT) !== -1,
    detail: "must contain both " + VC_V1_CONTEXT + " and " +
  ...
  checks.push({
    ...
    ok: types.indexOf("VerifiableCredential") !== -1 &&
                      types.indexOf("DomainLinkageCredential") !== -1,
    detail: "must contain VerifiableCredential and " +
        "DomainLinkageCredential; got " +
            (types.length ? types.join(", ") : "(none)") + "."
  });
  // The specification says the credential MUST NOT have an id at its root. It
  // is an odd-looking rule and it is deliberate: this credential is not an
  // addressable thing to be fetched or revoked, it is an assertion about a
  // pair.
  checks.push({
    name: "No credential id",
    ok: !("id" in vc),
    detail: ("id" in vc) ? 'the credential carries id "' + vc.id +
             '" at its root, which is forbidden.'
                         : "absent, as required."
  });
  ...
  checks.push({
    ...
    detail: 'the credential names origin "' + (claimedOrigin || "(absent)") +
            '" and was read from "' + wanted +
                '". These must be the same origin, or the ' +
  ...
  checks.push({
    ...
    ok: (!payload.exp || payload.exp > now) && (!payload.nbf ||
         payload.nbf <= now),
    detail: "nbf " + (payload.nbf || "—") + ", exp " + (payload.exp || "—") +
        ", now " + now + "."
  });
  ...
  if (!isDid(did)) {
    log.debug("Leaving verifyDomainLinkage().");
    return fail("Issuer signature", 'the issuer "' + did +
                '" is not a DID, so there is nothing to ' +
  ...
  return resolve(did, { fetch: opts.fetch, allowHttp: opts.allowHttp })
    .then(function (res) {
      ...
      if (!converted.jwks.keys.length) {
        throw new Error("the DID document publishes no assertion key that " +
                        "can verify a JWS" +
        ...
        .then(function (verdict) {
          checks.push({
            ...
            detail: (verdict.valid ? "verified" : "DOES NOT verify") +
                    " against the assertionMethod keys of " + did + " (" +
                        res.from +
          ...
          return { valid: valid, did: did, origin: claimedOrigin,
                  checks: checks };
        });
    })
    .catch(function (e) {
...
// Fetch an origin's DID Configuration and verify every entry in it, returning
// the verdict for the DID asked about.
//
// `wantedDid` matters: an origin may link several DIDs, and the question a
// wallet has is not "does this site link any DID" but "does it link THIS one" —
// the DID its credential named. Without that, a site linking its own DID would
// appear to vouch for somebody else's.
function verifyOriginLinkage(origin, wantedDid, opts) {
  log.debug("Entering verifyOriginLinkage(). origin=" + origin + ", did=" +
            wantedDid);
  opts = opts || {};
  var url = didConfigurationUrl(origin);
  if (!url) {
    log.debug("Leaving verifyOriginLinkage().");
    return Promise.reject(new Error("no origin to fetch a DID " +
                          "Configuration from."));
  }
  var doFetch = opts.fetch || (typeof fetch === "function" ? fetch : null);
  if (!doFetch) {
  ...
  return doFetch(url).then(function (r) {
    return r.text().then(function (text) {
      ...
      if (doc["@context"] !== DID_CONFIGURATION_CONTEXT) {
        throw new Error('the resource\'s @context must be "' +
                        DID_CONFIGURATION_CONTEXT +
                        '"; it is "' + doc["@context"] + '".');
      }
      ...
      // Every entry is verified, not just the first: the wanted DID may be any
      // of them, and an entry that fails is worth reporting beside one that
      // passes.
      return Promise.all(entries.map(function (entry) {
        return verifyDomainLinkage(entry, origin, opts);
      })).then(function (results) {
        var matched =
            results.filter(function (r) { return r.did === wantedDid; });
        log.debug("Leaving verifyOriginLinkage(). " + results.length +
                  " entry(ies), " +
...
//
// The resource itself has only two members, so a section listing just those
// would say very little. The rest are DERIVED from the credential it carries —
// the DID it links, the origin it claims, which of the two forms it is in, its
// validity window and the verification method its signature names — because
// those are the facts a reader has come to this pane for, and every one of them
// is buried inside a JWT that a table of two members would show as one
// unreadable line.
//
// Marked as derived in their descriptions rather than being hidden: this pane
// is editable like the others, and someone changing `linked_dids` by hand
// should be able to see that the fields below came from it and will not change
// until it is applied again.
// ---------------------------------------------------------------------------
var DID_CONFIGURATION_METADATA = [
  { name: "@context", type: "string", dflt: DID_CONFIGURATION_CONTEXT,
    desc: "REQUIRED. DIF Well Known DID Configuration: the resource's " +
        "@context MUST be exactly \"" +
          DID_CONFIGURATION_CONTEXT +
              "\". No other value is permitted, and no members beyond this " +
          "and linked_dids." },
  { name: "linked_dids", type: "json", dflt: "",
    desc: "REQUIRED. The Domain Linkage Credentials this origin publishes, " +
        "each either a JWT string " +
          "or a credential object with an embedded proof. An origin may link " +
              "more than one DID." },
  { name: "linked_did", type: "string", dflt: "",
    desc: "Derived from linked_dids: the DID this origin claims to be the " +
        "same entity as. It is both " +
          "the issuer and the subject of the credential — a domain linkage " +
              "is self-issued, because " +
          "nobody but the DID's controller can say which origin it controls." },
  { name: "origin", type: "string", dflt: "",
    desc: "Derived: the origin the credential names " +
        "(credentialSubject.origin). A verifier MUST " +
          "check this against the origin it fetched the resource from; if " +
              "they differ the credential " +
          "belongs to a different site." },
  { name: "credential_form", type: "string", dflt: "",
    desc: "Derived: JWT or Linked Data Proof. The specification allows " +
        "either. The JWT form is the " +
          "one where the header MUST NOT carry typ and the payload permits " +
              "nothing beyond " +
          "iss/sub/nbf/exp/vc — both things a JWT library adds unless " +
              "told not to." },
  { name: "verification_method", type: "string", dflt: "",
    desc: "Derived: the kid the signature names, which is the verification " +
        "method in the DID's own " +
  ...
  { name: "valid_until", type: "string", dflt: "",
    desc: "Derived from expirationDate (exp in the JWT form). The " +
        "specification requires one: a " +
          "linkage with no expiry asks a verifier to trust it indefinitely." }
];

var DID_CONFIGURATION_PREFIX = "didcfg_";
function didConfigurationIdFor(name) {
  log.debug("Entering didConfigurationIdFor().");
  log.debug("Leaving didConfigurationIdFor().");
  return DID_CONFIGURATION_PREFIX + name;
}

// The derived facts above, read off a resource. Returns the same member names
// so the pane can populate its fields without knowing which are derived.
//
// It reads the FIRST credential, and says so: an origin may link several DIDs,
// and a two-column table cannot show a set. The pane names how many were found;
// a reader who needs another one has linked_dids in front of them.
function didConfigurationDetails(resource) {
  ...
  var out = { "@context": (resource || {})["@context"] || "", linked_dids: "",
              linked_did: "", origin: "", credential_form: "",
                  verification_method: "",
  ...
  if (typeof entry === "string") {
    ...
    if (parts.length === 3) {
      try {
        ...
        vc = claims.vc || null;
        // nbf/exp are the JWT's own form of the credential's dates; the vc
        // claim carries them too, and the ISO strings read better in a field.
        if (!vc || !vc.issuanceDate) out.valid_from = claims.nbf ?
            String(claims.nbf) : "";
        if (!vc || !vc.expirationDate) out.valid_until = claims.exp ?
            String(claims.exp) : "";
      } catch (e) {
        ...
        log.debug("didConfigurationDetails(): the JWT form did not decode: " +
                  e.message);
      }
    }
...
var DID_METADATA = [
  { name: "id", type: "string", dflt: "",
    desc: "REQUIRED. The DID this document describes. DID Core: a resolved " +
        "document's id MUST equal the DID that was resolved, so a mismatch " +
        "here means the document belongs to somebody else." },
  ...
  { name: "verificationMethod", type: "json", dflt: "",
    desc: "The public keys this DID publishes, each with an id, a type and " +
        "the key itself (publicKeyJwk, or publicKeyMultibase for a key with " +
        "no JOSE representation such as BBS)." },
  { name: "authentication", type: "json", dflt: "",
    desc: "OPTIONAL. Which verification methods may authenticate as this DID " +
        "— either ids referencing verificationMethod, or embedded methods." },
  { name: "assertionMethod", type: "json", dflt: "",
    desc: "OPTIONAL. Which verification methods may ASSERT — i.e. which keys " +
        "may sign a credential. This is the relationship that matters when " +
        "verifying an issuer signature." },
  { name: "service", type: "json", dflt: "",
    desc: "OPTIONAL. Service endpoints this DID publishes. Not used by " +
        "either credential format here; shown because a real document often " +
        "carries them." }
];
...
module.exports = {
  ...
  // published values. Nothing in the pages reads them: a truncated constant
  // here was a real bug, and the round trip alone could only report it as "this
