...
var STARTS = {
  "same-device": { verifierPath: "/oid4vp/verifier",
                  cta: "Go to the verifier's web page" },
...
// verifier's invention (sts/server.js). A real verifier has no such thing,
// which is why walt.id's does not answer them. So the base is taken from
// explicit configuration first, and only guessed as a last resort.
//
// The guess used to be the credential issuer URL, whole, on the reasoning that
// the mock verifier shares an origin with the mock issuer. True for the mock,
// and wrong in two separate ways as soon as issuance has been run against
// walt.id: its Credential Issuer Identifier carries a PATH
// (http://localhost:7005/openid4vci), so appending /oid4vp/verifier produced
...
function verifierBase() {
  ...
  if (stored) {
    ...
    return { base: trimBase(stored), source: "configured" };
  }

  var configured = appconfig.oid4vpVerifierUrlDefault || "";
  if (configured) {
    log.debug("Leaving verifierBase().");
    return { base: trimBase(configured), source: "configured" };
  }
  ...
  if (origin) {
    log.debug("Leaving verifierBase().");
    return { base: origin, source: "guessed from the credential issuer" };
  }
...
function trimBase(v) {
  log.debug("Entering trimBase().");
  log.debug("Leaving trimBase().");
  return String(v).replace(/\/+$/, "");
}

// The scheme://host:port of a URL, dropping any path. Falls back to the trimmed
// string when it will not parse, so a hand-typed value is not silently
// discarded.
...
function configStatus(text, cls) {
  log.debug("Entering configStatus().");
  var e = el("config_status");
  if (!e) {
    log.debug("Leaving configStatus().");
    return;
  }
  e.textContent = text;
...
function restoreDefaults() {
  ...
  if (!base) {
    configStatus("This deployment configures no default verifier " +
                 "(oid4vpVerifierUrlDefault is empty), so " +
                 "there is nothing to restore \u2014 enter the " +
                     "verifier by hand.", "vc-bad");
...
function heldFormat() {
  ...
  try {
    ...
    // verifier for a format derived from a broken credential would be worse
    // than letting it use its default.
    log.debug("heldFormat(): the held credential could not be parsed: " +
              e.message);
    log.debug("Leaving heldFormat().");
    return "";
  }
...
function buttonHtml(uc, current) {
  ...
  var html =
    '<button type="button" class="' + classes.join(" ") + '" id="vp_usecase_' +
        esc(uc.id) + '"' +
    (uc.available ? ' onclick="return vcpresentation0.choose(\'' + esc(uc.id) +
     '\');"' : ' disabled="disabled"') +
    '>' +
      ...
      '<span class="vc-usecase-cta">' + esc(start.cta || "Choose this") +
          ' &rarr;</span>' +
    '</button>';
...
// show a verifier, and saying so here is more use than letting step 2 discover
// it.
function renderCredentialState() {
  ...
  if (!host) {
    log.debug("Leaving renderCredentialState().");
    return false;
  }
  if (!raw) {
    host.className = "vc-note vc-status vc-bad";
    host.innerHTML = "This wallet is not holding a credential yet, so there " +
  ...
  try {
    ...
    host.className = "vc-note vc-status vc-bad";
    host.textContent = "The credential in storage could not be parsed: " +
        e.message;
    log.debug("Leaving renderCredentialState(). Unparseable.");
    return false;
  }
  var hasKey = !!sdJwtVc.getJson(sdJwtVc.KEYS.HOLDER_PRIVATE_JWK);
  // Absent-by-choice is not the same as absent-and-lost: the holder key pair
  // can be deliberately kept out of localStorage (issuance step 2), in which
  // case it is pasted in on step 2 of this workflow and the credential is
  // perfectly presentable. Saying "cannot be presented" for that case would be
  // wrong.
  var optedOut = !hasKey && !sdJwtVc.holderPrivateKeyMayBeStored();
  host.className = "vc-note vc-status " + (hasKey ? "vc-ok" : (optedOut ?
      "vc-pending" : "vc-bad"));
  ...
  host.textContent = "Holding a " + sdJwtVc.credentialLabel(parsed) +
    (parsed.format === sdJwtVc.FORMAT_JWT_VC_JSON
      ...
      : " with " + parsed.disclosures.length +
          " selectively-disclosable claim(s)") +
    (hasKey
      ? ", and the holder key it is bound to — so it can be presented."
      : optedOut
        ? ". The holder key is not kept in this browser, by the choice made " +
            "on issuance step 2 — you " +
          "will be asked to paste it when the presentation is assembled."
        : ", but NOT the private half of the key it is bound to, so no Key " +
            "Binding JWT can be signed for it.");
...
function render() {
  ...
  // wrong — the start pages below exist only on this debugger's mock verifier,
  // so a base guessed from, say, walt.id's issuer will 404 — and saying so here
  // is the difference between an obvious misconfiguration and an unreachable
  // link.
  var b = verifierBase();
  var where = b.base
    ? "The verifier is " + b.base +
      (b.source === "configured"
        ? "."
        : " (guessed from the credential issuer — these start pages exist " +
            "only on this " +
          "debugger's own mock verifier, so set oid4vpVerifierUrlDefault, or " +
              "paste a request " +
...
function choose(id) {
  ...
  if (!url) {
    status("There is no verifier configured to ask for a presentation. Set " +
           "one in step 1, or paste a " +
           "request there by hand.", "vc-bad");
    window.setTimeout(function () { window.location.href = sdJwtVp.STEP1_URL; },
                      2500);
    log.debug("Leaving choose().");
    return false;
  }
