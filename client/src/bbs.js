...
// whose `alg` must be a registered JOSE algorithm, and BBS is not one. BBS needs
// a pairing-friendly curve and produces a signature that a verifier can turn
// into a *derived proof* over a chosen subset of the signed messages — which is
// a different shape from a signature entirely.
//
...
// presentations of the same credential are trivially linkable by that signature.
// With BBS the holder derives a FRESH proof each time, disclosing only the
// statements asked for, and the proofs are unlinkable to each other. That is
// selective disclosure plus unlinkability, which neither of the other formats
// offers.
//
...
// implementation and checked by a different one. The two share @noble/curves for
// field and curve arithmetic, so the independence is at the BBS PROTOCOL layer,
// not all the way down — worth being precise about rather than overclaiming.
//
...
//
// Two constants that are easy to get wrong, and both were, in the first draft of
// this file — with a self-test that passed anyway, because sign() and verify()
// shared the mistake. Only cross-checking against a different implementation
// found them.
//
...
function hashToScalar(message, dst) {
  var uniform = hashToCurve.expand_message_xmd(message, dst, EXPAND_LEN, sha256Fn());
  return Fr.create(nobleUtils.bytesToNumberBE(uniform));
}

// @noble exposes sha256 through its hashes package; resolved once and kept, so a
// missing export fails at load with a clear message rather than inside a signing
// operation.
var sha256Impl = null;
function sha256Fn() {
  if (sha256Impl) return sha256Impl;
  try {
...
function createGenerators(count) {
  if (generatorCache.length >= count) return generatorCache.slice(0, count);
  ...
  for (var i = 1; i <= count; i++) {
    v = hashToCurve.expand_message_xmd(concatBytes(v, i2osp(i, 8)), seedDst, seedLen, sha256Fn());
...
function calculateDomain(pk, q1, generators, header) {
  var dom = concatBytes(
    ...
    q1.toRawBytes(true),
    concatBytes.apply(null, generators.map(function (g) { return g.toRawBytes(true); })),
    te(API_ID),
...
function computeB(domain, msgScalars, q1, generators) {
  ...
  msgScalars.forEach(function (m, i) {
    if (m !== 0n) b = b.add(generators[i].multiply(m));
  });
  return b;
}
...
function verify(pk, signature, header, messages) {
  ...
  var e = Fr.create(nobleUtils.bytesToNumberBE(signature.slice(OCTET_POINT_LENGTH)));
  var msgScalars = messagesToScalars(messages);
  var all = createGenerators(msgScalars.length + 1);
  var q1 = all[0];
  var generators = all.slice(1);
...
function randomScalar() {
  ...
  // that loads this module. Measured: it added 40 elliptic references and ~1 MB.
  // See the "Keeping elliptic out of the bundles" note in CLAUDE.md.
  var webcrypto = (typeof globalThis !== "undefined" && globalThis.crypto) ||
                  (typeof window !== "undefined" && window.crypto);
  if (!webcrypto || !webcrypto.getRandomValues) {
    throw new Error("BBS proof generation needs Web Crypto's getRandomValues, which this context " +
      "does not provide. In the containerized suite the page must be on a secure origin — see " +
...
// zero-knowledge proof over a chosen subset of the messages: the verifier learns
// the disclosed messages and that a valid signature exists over all of them,
// and nothing else. Because the randomness is fresh each time, two proofs from
// the same credential are unlinkable — which is the property an SD-JWT cannot
// offer, since there the issuer's signature is reused verbatim every time.
function proofChallenge(initRes, disclosedIndexes, disclosedScalars, ph) {
  ...
  var arr = [disclosedIndexes.length].concat(zipped, [
    initRes.Abar, initRes.Bbar, initRes.D, initRes.T1, initRes.T2, initRes.domain]);
...
function proofGen(pk, signature, header, ph, messages, disclosedIndexes) {
  ...
  var q1 = all[0];
  var H = all.slice(1);
  var disclosed = disclosedIndexes.slice().sort(function (x, y) { return x - y; });
  var undisclosed = [];
  for (var i = 0; i < L; i++) if (disclosed.indexOf(i) === -1) undisclosed.push(i);
  ...
  undisclosed.forEach(function (j, idx) { T2 = T2.add(H[j].multiply(mTilde[idx])); });

  var initRes = { Abar: Abar, Bbar: Bbar, D: D, T1: T1, T2: T2, domain: domain };
  ...
  return serializeArray([Abar, Bbar, D, eHat, r1Hat, r3Hat].concat(mHat, [challenge]));
}

function parseProof(proof) {
  var at = 0;
  var take = function (n) { var out = proof.slice(at, at + n); at += n; return out; };
  ...
  var scalars = [];
  while (at < proof.length) scalars.push(Fr.create(nobleUtils.bytesToNumberBE(take(OCTET_SCALAR_LENGTH))));
...
function proofVerify(pk, proof, header, ph, disclosedMessages, disclosedIndexes) {
  ...
  var T1 = p.Bbar.multiply(p.challenge).add(p.Abar.multiply(p.eHat)).add(p.D.multiply(p.r1Hat));
  var Bv = P1().add(q1.multiply(domain));
  disclosedScalars.forEach(function (m, idx) { Bv = Bv.add(H[disclosed[idx]].multiply(m)); });
  var T2 = Bv.multiply(p.challenge).add(p.D.multiply(p.r3Hat));
  undisclosed.forEach(function (j, idx) { T2 = T2.add(H[j].multiply(p.mHat[idx])); });

  var initRes = { Abar: p.Abar, Bbar: p.Bbar, D: p.D, T1: T1, T2: T2, domain: domain };
  var expected = proofChallenge(initRes, disclosed, disclosedScalars, ph);
  if (!Fr.eql(expected, p.challenge)) return false;

  // e(Abar, W) == e(Bbar, G2): the pairing check that ties the proof back to the
  // issuer's key.
  var w = G2.fromHex(pk);
  return bls.fields.Fp12.eql(bls.pairing(p.Abar, w), bls.pairing(p.Bbar, G2.BASE));
}
