...
// under Holder of Key the key being proved is the DPoP key and a wallet may have
// chosen RS256 for that. Signing an RSA key's proof with the ECDSA parameters
// fails in Web Crypto with an unhelpful DataError, so the two are matched here
...
function proofAlgFor(key) {
  if (key && PROOF_ALGS[key.alg]) return key.alg;
  ...
  // proof_signing_alg_values_supported.
  if (key && key.publicJwk && key.publicJwk.kty === "RSA") return "RS256";
  return PROOF_ALG;
}
...
//
// One implementation for every call the wallet makes, because DPoP's whole value
// is in being on EVERY call: an endpoint whose call site forgot the proof is the
// one an attacker uses, and this workflow has six such call sites across two
// pages (token, credential, deferred, notification, and the refresh page's
// copies of the last two). A `"Bearer " + token` written by hand at each of them
// is how one of six ends up wrong.
//
...
// is nothing to attach, and the request goes out exactly as it did before any of
// this existed.
// ---------------------------------------------------------------------------

// The headers for one attempt. Returns what was built as well as the headers, so
// a pane can SHOW the proof it sent — which is the point of a debugger, and the
// reason this returns an object rather than just setting headers.
function dpopHeadersFor(opts) {
  log.debug("Entering dpopHeadersFor(). method=" + opts.method + ", withToken=" +
            (opts.accessToken ? "yes" : "no"));
  var context = opts.context;
  var accessToken = opts.accessToken || "";
  if (!context || !context.key) {
    log.debug("Leaving dpopHeadersFor(). No DPoP context; this is a Bearer call.");
  ...
  return dpopLib.proof({
    ...
    // sending an `ath` there would be a claim about a token that does not exist.
    ...
    if (accessToken) headers.Authorization = dpopLib.authorizationHeader(accessToken);
    log.debug("Leaving dpopHeadersFor(). A DPoP proof was attached.");
    return {
      headers: headers,
      authorization: accessToken ? dpopLib.authorizationHeader(accessToken) : "",
      proof: made.proof, decoded: { header: made.header, payload: made.payload },
      scheme: "DPoP"
    };
  });
}
...
// request, which is the difference between nonce mode being a nuisance and being
// invisible.
function fetchProtected(opts) {
  ...
  var attempt = function (nonce, isRetry) {
    return dpopHeadersFor({
      ...
      return fetch(opts.url, { method: method, headers: headers, body: opts.body })
  ...
  return attempt(undefined, false).then(function (first) {
    ...
    var headersObject = {};
    // fetch()'s Headers is iterable but not indexable, and nonceRequested() takes
    // a plain object so it can be unit-tested without one.
    ...
    var asked = dpopLib.nonceRequested(first.response.status, first.body, headersObject);
...
function describeCall(opts) {
  ...
  // The DPoP proof is a header of the request like any other, and leaving it out
  // of the assembled call would describe a request this wallet does not make. A
  // pane that shows `Authorization: DPoP <token>` with no `DPoP:` line beside it
  // is showing a request every conforming server refuses.
