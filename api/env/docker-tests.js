var config = {
  ...
  // STS, the DCR endpoint). A NUMBER, not a string. axios has no default of its
  // own, so without this a host that accepts the connection and then never
  // answers holds the request open for minutes. Omit it to use the code default
  // of 10000; a value that is not a positive number is logged and ignored.
  callTimeout: 10000,
  ...
  // different deadline from callTimeout rather than a smaller copy of one: a dead
  // or firewalled address fails inside connectionTimeout, while a host that has
  // answered still gets the whole callTimeout to produce a response. Keep it
  // BELOW callTimeout; omit it for the code default of 5000. A value that is not
  // a positive number is logged and ignored.
  ...
  // of megabytes, though a single IdP's descriptor is far below this. A value that
  // is not a positive number is logged and ignored (0 would refuse every response
  // that has a body).
  ...
  // sends this service somewhere private ("302 Location: http://127.0.0.1:8080/"),
  // which the SSRF guard refuses on the hop itself.
  //
  // 0 is legal and means follow none: axios then hands the 3xx back as the
  // response, and since a non-2xx is a failure to these endpoints, the caller sees
  // the upstream's status (302) with the endpoint's error body and no Location.
  // That suits a deployment that expects no redirects at all; it is not a general
  // "be careful" setting. A negative or fractional value is logged and ignored,
  // because axios would silently fall back to its own 21 rather than to anything
  // intended here.
  maxRedirects: 5,
  // The User-Agent sent on every outbound call. `{{VERSION}}` is replaced with the
  // build version (M.N.O), the same placeholder the client's footer uses.
  //
  // Without this axios announces itself as "axios/<its version>", which tells the
  // operator of an identity provider nothing about who is calling — and this
  // service appears in other people's access logs by design. A value with no
  // placeholder in it is sent verbatim.
  userAgent: "Identity Protocol Debugger/{{VERSION}}",
  // Whether outbound connections are pooled and reused (the agents' keepAlive).
  // A BOOLEAN; only an explicit false turns it off, so a missing or misspelled key
  // stays on. A quoted "false" is refused and logged, since it is truthy.
  //
  // On, a debugger session that calls the token endpoint, then introspection, then
  // userinfo on the same host stops paying for a TCP connection and a TLS
