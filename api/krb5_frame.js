...
// What a Kerberos-protected service is presented with. Almost always the
// GSS-wrapped form: a service receives an RFC 2743 InitialContextToken, not a
// bare AP-REQ.
...
// The DER-encoded Kerberos v5 mechanism OID, 1.2.840.113554.1.2.2. Written out
// here rather than imported from common/krb5/krb5_gss.js for the same reason
...
// request with a large PAC can be bigger, so the ceiling is generous — but it
// is a ceiling, because this value bounds what one call can make this service
// send.
...
// lengths of at most four bytes are accepted: an indefinite length is BER
...
// name on success.
//
// It checks the SHAPE and refuses to guess: a payload whose declared length
// does not match its actual length is either truncated or padded, and relaying
  ...
  if (!name) {
    ...
    throw new Error('this is not a request a KDC answers: the first byte is ' +
      ...
      (isServiceRequest
        ...
        : looksBase64
          ? '. It looks like text — was it decoded from base64 before being ' +
              'sent?'
          : '.'));
  }
...
function assertServiceRequest(bytes) {
  ...
  // The GSS-wrapped form. Every field is checked, because "0x60 then anything"
...
function readTcpFrame(buffer, maxBytes) {
  ...
  if (declared > maxBytes) {
    ...
    throw new Error('the reply announces ' + declared + ' bytes, over the ' + 
      ...
      'legitimately answers with more (a large PAC can make a reply big, but ' +
          'not this big).');
  }
