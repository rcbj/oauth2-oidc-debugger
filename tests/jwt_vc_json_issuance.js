...
//
// ---------------------------------------------------------------------------
// OID4VCI issuance in the jwt_vc_json format, against the mock Credential Issuer
// the STS service hosts.
//
// The sibling of sd_jwt_vc_issuance.js, which covers the same workflow in
// dc+sd-jwt. Kept separate rather than folded in as a flag, because most of that
// suite is about Disclosures — choosing them, hashing them, the decoy digest,
// the sd_hash that commits to which ones went — and none of that exists in this
// format. A flag would leave those sections skipped and the run would read as
// though selective disclosure had been declined rather than being unavailable.
//
// What this asserts is the format's own contract and the wallet's handling of
// it:
//
//   * the issuer ADVERTISES jwt_vc_json properly — credential_definition.type in
//     place of a vct, since a W3C VC has no vct;
...
//   * the wallet's own pages read the credential correctly: step 3 shows it, and
//     the presentation workflow's entry point describes it as a credential that
...
async function metadataAdvertisesTheFormat() {
  ...
  assert.ok(found.id,
    ...
    Object.keys((found.meta || {}).credential_configurations_supported || {}).join(", "));
  ...
  assert.ok(entry.credential_definition && Array.isArray(entry.credential_definition.type),
    "a jwt_vc_json configuration identifies its credential with credential_definition.type — there is no " +
    "vct in this format. Got: " + JSON.stringify(entry.credential_definition));
  assert.ok(entry.credential_definition.type.indexOf("VerifiableCredential") >= 0,
    "and that type array starts from VerifiableCredential. Got: " +
    JSON.stringify(entry.credential_definition.type));
  assert.ok(!entry.vct,
    "a jwt_vc_json configuration should NOT carry a vct: that is the SD-JWT VC way of naming a credential, " +
...
async function issuesARealVcJwt(found) {
  ...
  const key = ((jwks || {}).keys || []).filter(function (k) { return k.kid === header.kid; })[0];
  assert.ok(key, "the issuer should publish the key it signed with (kid " + header.kid + "). Published: " +
    ((jwks || {}).keys || []).map(function (k) { return k.kid; }).join(", "));
  const parts = held.credential.split(".");
  const verified = crypto.verify("sha256", Buffer.from(parts[0] + "." + parts[1]),
    crypto.createPublicKey({ key: key, format: "jwk" }), Buffer.from(parts[2], "base64url"));
  assert.ok(verified, "the credential's signature should verify against the issuer's published JWKS.");

  const binding = common.holderBindingOf(parsed.payload);
  assert.notStrictEqual(binding.kind, "none",
    "an issued credential should say who may present it — cnf.jwk or a subject DID in " +
    "credentialSubject.id. It carries neither.");
  assert.strictEqual(binding.kind, "cnf.jwk",
    "the MOCK issuer binds with cnf.jwk (walt.id's own profiles use a subject DID instead, which is what " +
...
async function theWalletPagesReadIt(driver, held) {
  ...
  assert.ok(String(raw).indexOf("~") === -1 || String(raw).trim() === "",
    "step 3 should be showing the VC-JWT it was given, not something with Disclosures in it.");
  ...
  assert.ok(/no selective disclosure|cannot be withheld/.test(state),
    "and say that nothing in it can be withheld — the one thing this format changes for a holder. Said: " +
    state);
...
async function test() {
  ...
  assert.ok(found.meta,
    "no credential issuer metadata at " + issuerBase + ". Start the STS mock and set " +
    "WSTRUST_STS_URL / OID4VCI_ISSUER_URL.");
  assert.ok(found.id,
    "this issuer offers no jwt_vc_json credential configuration, so the format is untested. Offered: " +
    Object.keys(found.meta.credential_configurations_supported || {}).join(", "));
