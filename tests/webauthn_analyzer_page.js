...
// exactly as `tests/jwk_pem.js` collided with `client/src/jwk_pem.js`, where the
// module silently replaced the test and the job passed having run nothing. Same
// trap, avoided by name. See tests/jwk_pem_encoding.js.
//
// What this covers that tests/webauthn_decode.js cannot: that the PAGE puts the
// decoded values on the screen. The node test proves the arithmetic; this proves
// the arithmetic reaches the user, which is a different failure — a pane that
// stays empty because an element id was renamed decodes perfectly and shows
// nothing.
//
...
function asCredentialJson(vector, ceremony) {
  const response = ceremony === "create"
    ? { clientDataJSON: vector.clientDataJSON, attestationObject: vector.attestationObject,
        transports: vector.oracle ? vector.oracle.transports : undefined }
    : { clientDataJSON: vector.clientDataJSON, authenticatorData: vector.authenticatorData,
        signature: vector.signature, userHandle: vector.userHandle || undefined };
...
// Wait for CONTENT rather than for an element: every pane on this page is static
// markup that exists during parsing and says nothing about whether the decode
// has run. See tests/wait_for.js for why this rule exists.
...
async function test() {
  ...
  // This page needs no secure context of its own, but it does run Web Crypto for
  // the signature check, and on the containerized origin crypto.subtle is
  ...
  try {
    ...
    await section("a registration decodes onto the page", async () => {
      await setValue(driver, "wa_input", asCredentialJson(VECTORS.es256_registration, "create"));
      await driver.findElement(By.id("wa_analyze_button")).click();

      const authData = await waitForText(driver, "wa_authdata_body",
        (t) => /sign count/i.test(t), "the authenticator data pane never filled");
      assert.ok(/AAGUID/.test(authData), "the AAGUID should be shown for a registration");
      ...
      assert.ok(/ES256/.test(cose), "the algorithm should be named: " + cose.slice(0, 120));
      ...
      assert.ok(/http:\/\/localhost:3199/.test(cd), "the origin should be shown");
      return "flags, AAGUID, COSE key, PEM, attestation and client data all on screen";
    });

    await section("an assertion verifies against the key kept from that registration", async () => {
      await setValue(driver, "wa_input", asCredentialJson(VECTORS.es256_assertion, "get"));
      // The expectations are the relying party's; supply the real ones.
      await setValue(driver, "wa_expect_challenge",
        Buffer.from(Uint8Array.from([0xca, 0xfe, 0xba, 0xbe].concat(Array(28).fill(0x22))))
      ...
      const verdict = await waitForText(driver, "wa_verify_status",
        (t) => /VALID|NOT VALID/.test(t), "the verification pane never reported a verdict");
      assert.ok(/^VALID/.test(verdict), "the assertion should verify: " + verdict);

      const checks = await textOf(driver, "wa_verify_body");
      assert.ok(/signature verifies/.test(checks), "the signature check should be listed");
      ...
      assert.ok(/"kty"/.test(key),
        "the credential public key should have been recalled from the registration, not pasted");
      return verdict;
    });

    await section("a tampered signature is reported as exactly that", async () => {
      ...
      bad.response.signature = sig.slice(0, 8) + (sig[8] === "A" ? "B" : "A") + sig.slice(9);
      await setValue(driver, "wa_input", JSON.stringify(bad, null, 2));
      await driver.findElement(By.id("wa_analyze_button")).click();

      const verdict = await waitForText(driver, "wa_verify_status",
        (t) => /NOT VALID/.test(t), "a tampered signature should have produced a NOT VALID verdict");
      const checks = await textOf(driver, "wa_verify_body");
      assert.ok(/✗.*signature verifies/.test(checks),
        "the signature check should be the one marked failed: " + checks.slice(0, 200));
      assert.ok(!/✗.*challenge matches/.test(checks),
        "and nothing else should be marked failed — the pane must name the right cause");
      return verdict;
    });

    await section("a UV-clear assertion fails on the flag, with the signature still valid", async () => {
      // The registration first, so its key is remembered.
      await setValue(driver, "wa_input", asCredentialJson(VECTORS.uv_clear_registration, "create"));
      await driver.findElement(By.id("wa_analyze_button")).click();
      await waitForText(driver, "wa_cose_body", (t) => /jwk\.kty/.test(t),
        "the UV-clear registration did not decode");

      await setValue(driver, "wa_input", asCredentialJson(VECTORS.uv_clear_assertion, "get"));
      ...
      await waitForText(driver, "wa_verify_status", (t) => /NOT VALID/.test(t),
        "UV was required and not performed, so the verdict should be NOT VALID");
      const checks = await textOf(driver, "wa_verify_body");
      assert.ok(/✗.*user verification/.test(checks),
        "the UV check should be the failure: " + checks.slice(0, 200));
      assert.ok(/✓.*signature verifies/.test(checks),
        "and the SIGNATURE must still be shown as valid — reporting a bad signature here would " +
        "send the user after the wrong problem entirely");
      return "UV ✗, signature ✓";
    });

    await section("nonsense input is refused by name, not by silence", async () => {
      await setValue(driver, "wa_input", "this is not json");
      await driver.findElement(By.id("wa_analyze_button")).click();
      const st = await waitForText(driver, "wa_input_status", (t) => t.length > 0,
        "pasting nonsense produced no message at all");
      assert.ok(/not JSON/i.test(st), "the message should say what was wrong: " + st);
      return st.slice(0, 60);
    });

    await section("no pane overflows its box", async () => {
      await setValue(driver, "wa_input", asCredentialJson(VECTORS.rs256_registration, "create"));
      await driver.findElement(By.id("wa_analyze_button")).click();
      ...
      assert.ok(m.worst <= 2,
        "content escapes its pane by " + m.worst + "px (" + m.id + "). A table holding base64url " +
        "sizes itself to its longest line unless it is table-layout: fixed with overflow-wrap.");
...
program
  ...
  .description("Drive the WebAuthn Analyzer page against real ceremony artifacts.")
