...
// The module reaches Web Crypto through globalThis rather than
// require("crypto"), which is what keeps `elliptic` out of the browser bundles.
// Assert it rather than discover a TypeError six functions deep.
assert.strictEqual(typeof globalThis.crypto.subtle, "object", "this node has " +
    "no Web Crypto");
...
function nfoldMatchesRfc3961() {
  ...
  eq("192-fold(\"MASSACHVSETTS…\")", prim.nfold(ascii("MASSACHVSETTS " +
...
async function ctsMatchesRfc3962() {
  ...
  const input = ascii("I would like the General Gau's Chicken, please, and " +
      "wonton soup.");
  ...
  const cases = [
    {
      iter: 1,
      pass: "password",
      salt: ascii("ATHENA.MIT.EDUraeburn"),
      ...
      // cases either side of the key-shortening boundary in HMAC.
    {
      iter: 1200,
      pass: "X".repeat(64),
      salt: ascii("pass phrase equals block size"),
    ...
    },
    {
      iter: 1200,
      pass: "X".repeat(65),
      salt: ascii("pass phrase exceeds block size"),
...
async function arcfourBehavesLikeRfc4757() {
  ...
  assert.strictEqual(kcrypto.translateArcfourUsage(3), 8, "usage 3 must " +
      "translate to 8");
  assert.strictEqual(kcrypto.translateArcfourUsage(9), 8, "usage 9 must " +
      "translate to 8");
  assert.strictEqual(kcrypto.translateArcfourUsage(23), 13, "usage 23 must " +
      "translate to 13");
  assert.strictEqual(kcrypto.translateArcfourUsage(11), 11, "usage 11 must " +
      "pass through");
  ...
  for (const len of [0, 1, 16, 63]) {
    ...
    // A 16-byte checksum, an 8-byte confounder, and a stream cipher: no
    // padding.
    assert.strictEqual(ct.length, 16 + 8 + len, "arcfour ciphertext length " +
        "for " + len);
...
async function refusesWhatItMustRefuse() {
  ...
  assert.strictEqual(kcrypto.etypeName(4242), "etype-4242", "an unknown " +
      "etype must render as something");
...
function keyUsageNumbersArePinned() {
  ...
  Object.keys(expected).forEach(function (name) {
    assert.strictEqual(U[name], expected[name],
      "key usage " + name + " must be " + expected[name] + " (RFC 4120 " +
          "section 7.5.1)");
  });
  ...
  assert.strictEqual(pref[pref.length - 1], 23, "arcfour-hmac-md5 must be " +
      "offered last if at all");
  log.debug("Leaving keyUsageNumbersArePinned().");
}

async function test() {
  log.debug("Entering test().");
  log.info("Starting Test run. Verifying common/krb5 crypto against RFC " +
      "1320, 1321, 2202, 3961, 3962, 8009.");
...
program
  ...
  .description("Verify the Kerberos v5 encryption framework against the " +
      "RFCs' own published vectors.")
