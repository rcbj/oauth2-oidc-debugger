...
// readAuthorizationData takes a PARSED TLV, not raw bytes. Getting that wrong
// throws from inside the DER reader with a message about a missing tag, which
// reads as a broken encoder rather than as a caller handing it the wrong kind
// of thing.
...
function sampleLogonInfo(overrides) {
  log.debug("Entering sampleLogonInfo().");
  var base = {
...
async function buildSample(overrides) {
  log.debug("Entering buildSample().");
  var spec = {
...
async function theHeaderMatchesTheSpecByteForByte() {
  ...
  // The buffer table: ulType, cbBufferSize, then an EIGHT-byte offset. A reader
  // that treated Offset as 4 bytes would find every entry after the first
  // shifted, so this asserts the stride explicitly.
  var seenTypes = [];
  ...
  [pac.TYPE.LOGON_INFO, pac.TYPE.CLIENT_INFO, pac.TYPE.SERVER_CHECKSUM,
   pac.TYPE.KDC_CHECKSUM].forEach(function (t) {
    assert.ok(seenTypes.indexOf(t) !== -1,
      "a PAC in a ticket MUST carry " + pac.bufferTypeName(t) + " ([MS-PAC] " +
          "section 2.4)");
  });
...
async function theLogonInfoSitsWhereTheIdlSaysItDoes() {
  ...
  // [MS-RPCE] section 2.2.6's sixteen bytes: version 1, little-endian/ASCII
  // 0x10, common header length 8, 0xCC filler; then ObjectBufferLength and a
  // zero filler.
  assert.strictEqual(hex(b.subarray(0, 8)), "01100800cccccccc",
    "the type-marshalling common header should be 01 10 08 00 cc cc cc cc, " +
        "got " +
    hex(b.subarray(0, 8)));
  assert.strictEqual(u32At(b, 12), 0, "the private header's filler should be " +
      "zero");
  ...
  assert.strictEqual(u32At(b, S + 104), 513, "PrimaryGroupId belongs at " +
      "struct offset 104");
  assert.strictEqual(u32At(b, S + 108), 3, "GroupCount belongs at struct " +
      "offset 108");
  assert.notStrictEqual(u32At(b, S + 112), 0, "the GroupIds referent id " +
      "belongs at struct offset 112");
  assert.strictEqual(u32At(b, S + 116), pac.USER_FLAG_EXTRA_SIDS,
    "UserFlags belongs at struct offset 116, and should carry D (EXTRA_SIDS) " +
        "= 0x20 because this " +
    "PAC has extra SIDs. 0x40 there would mean the bit diagram was read from " +
        "the wrong end");

  // EffectiveName's own header at struct offset 48: "alice" is 5 characters, so
  // its Length and MaximumLength are 10 BYTES — not 5, and not 12.
  assert.strictEqual(u16At(b, S + 48), 10,
    "EffectiveName.Length is in BYTES, so 'alice' is 10");
  assert.strictEqual(u16At(b, S + 50), 10, "EffectiveName.MaximumLength " +
      "should match");
  ...
  assert.strictEqual(hex(b.subarray(S + 8, S + 16)), "ffffffffffffff7f",
    "LogoffTime should be the 0x7FFFFFFFFFFFFFFF 'never' sentinel, " +
        "little-endian");
...
async function theReaderRecoversEveryField() {
  ...
  assert.strictEqual(info.userSid, DOMAIN + "-1104",
    "the account's SID is LogonDomainId + UserId; a PAC never carries it " +
        "whole");
  ...
  assert.strictEqual(info.groups[1].name, "Domain Admins",
    "RID 512 should be named, because 'S-1-5-21-…-512' is work the reader " +
        "should not have to do");
  assert.ok(info.groups[0].attributeNames.indexOf("ENABLED") !== -1,
    "the default group attributes should include ENABLED");

  // TWO extra SIDs — the case that distinguishes a correct deferred-pointer
  // reader.
  assert.strictEqual(info.extraSids.length, 2,
    "both extra SIDs should be read. One would pass even with the pointers " +
        "followed in " +
  ...
  assert.strictEqual(info.extraSids[1].text, "S-1-5-32-544",
    "the SECOND extra SID is the one a reader that follows each pointer as " +
        "it goes gets wrong");
  assert.strictEqual(info.extraSids[0].name, "Authentication authority " +
      "asserted identity",
  ...
  assert.strictEqual(info.logoffTime.never, true,
    "0x7FFFFFFFFFFFFFFF means 'never', and rendering it as a date in the " +
        "year 30828 would be " +
  ...
  assert.strictEqual(upn.samName, "alice",
    "the SAM name is only present when the S flag is set, and must be read " +
        "at ITS offset");
  ...
  assert.strictEqual(attrs.flagsLength, 2, "FlagsLength is a count of BITS, " +
      "not bytes");
...
// Resource groups — the OTHER group list, and the one nothing else here
// reaches.
//
// It has its own domain SID, its own array and its own UserFlags bit (H), and
// the attributes on its entries carry SE_GROUP_RESOURCE. This section exists
// because without it a wrong value for that flag is invisible: the ordinary
// group list never sets it, so `0x20000000` could be `0x10` and every other
// assertion in this file would still pass. It is also the list that makes a
// domain-local group grant access on a resource in another domain, so getting
...
async function resourceGroupsAreReadWithTheirOwnDomainSid() {
  ...
  var built = await buildSample({
    logonInfo: {
      ...
      // Two again, and with DIFFERENT attributes, so the array is walked
      // properly and the flags are not read off the first entry for all of
      // them.
      resourceGroups: [
  ...
  assert.strictEqual(info.resourceGroupDomainSid.text, RESOURCE_DOMAIN,
    "the resource groups' domain SID is a separate field from LogonDomainId " +
        "— they are groups " +
    "in the RESOURCE's domain, not the account's");
  assert.strictEqual(info.resourceGroups.length, 2, "both resource groups " +
      "should be read");
  assert.deepStrictEqual(info.resourceGroups.map(function (g) { return g.relativeId; }),
    [1200, 1201]);
  assert.ok(info.resourceGroups[0].attributeNames.indexOf("RESOURCE " +
      "(domain-local)") !== -1,
    "SE_GROUP_RESOURCE is 0x20000000 — bit 2 of [MS-PAC]'s diagram, counting " +
        "from the LEFT " +
    "where the rightmost bit is the least significant. Reading that diagram " +
        "the other way " +
    "gives 0x10, which is USE_FOR_DENY_ONLY, an almost opposite meaning. " +
        "Got: " +
  ...
  assert.ok(info.userFlagNames.indexOf("H RESOURCE_GROUPS") !== -1,
    "resource groups require the H flag ([MS-PAC] section 2.5), and the " +
  ...
  // is what makes the value above load-bearing rather than incidental.
  ...
  assert.ok(pac.groupAttributeNames(0x20000000).indexOf("RESOURCE " +
      "(domain-local)") !== -1,
    "and 0x20000000 is");

  // SE_GROUP_LOGON_ID is TWO bits (0xC0000000), so a bitwise-any test would
  // report it for either half alone.
  ...
  assert.ok(!/LOGON_ID/.test(pac.groupAttributeNames(0x80000000).join(",")),
    "but half of it is not — SE_GROUP_LOGON_ID needs both bits, so it must " +
        "be matched exactly");
...
async function eachSignatureIsCheckedAgainstItsOwnKeyAndItsOwnBytes() {
  ...
  var all = await pac.verifySignatures(parsed, {
    serverKey: serverKey,
    kdcKey: kdcKey
  });
  [pac.TYPE.SERVER_CHECKSUM, pac.TYPE.KDC_CHECKSUM, 
    ...
    function (t) {
      ...
      assert.strictEqual(r.verified, true,
        pac.bufferTypeName(t) + " should verify against the key it was made " +
            "with: " + r.note);
    });
  ...
  assert.strictEqual(byType(after, pac.TYPE.KDC_CHECKSUM).verified, true,
    "but the KDC signature STILL verifies, because it covers only the server " +
        "signature's " +
    "bytes and those were not touched. This is exactly why the extended KDC " +
        "signature was " +
    "added (CVE-2022-37967) — a verifier that checks the KDC signature alone " +
        "sees nothing wrong");

  // A key of the wrong etype cannot verify, and the reason must say so rather
  // than reading as a bad signature.
  ...
  assert.ok(/belongs to etype 18.*supplied is etype 17/.test(s.note),
    "an etype mismatch should be named as such — it is what a KDC and a " +
        "service disagreeing " +
    "about encryption types looks like from outside. Got: " + s.note);

  // The ticket signature: asked for without the bytes it covers, the builder
  // must refuse rather than sign the wrong thing.
  var refused = null;
  try {
    await pac.buildPac({
      serverKey: serverKey,
      kdcKey: kdcKey,
      includeTicketSignature: true,
  ...
  assert.ok(refused && /ad-data replaced by a single zero byte/.test(refused),
    "asking for a ticket signature without an EncTicketPart should be " +
        "refused with an " +
...
// The ticket signature end to end: it binds a PAC to ONE ticket, so the bytes
// it covers have to be built the way [MS-PAC] section 2.8.2 says — the
// EncTicketPart's DER with this PAC's ad-data replaced by a single zero byte.
async function theTicketSignatureBindsThePacToOneTicket() {
  ...
  function encTicketPartWith(adData, endtime) {
    log.debug("Leaving encTicketPartWith().");
    log.debug("Entering encTicketPartWith().");
  ...
  var signedOver = encTicketPartWith([{
    type: pac.AD_TYPE.WIN2K_PAC,
  ...
  assert.strictEqual(ticketResult.verified, true,
    "the ticket signature should verify over the EncTicketPart with the " +
        "ad-data blanked: " +
    ticketResult.note);
...
async function thePacIsFoundInsideAdIfRelevant() {
  ...
  assert.strictEqual(ad.length, 1, "the wrapper is a single AD-IF-RELEVANT " +
      "element");
  assert.strictEqual(ad[0].type, pac.AD_TYPE.IF_RELEVANT,
    "the outer element must be ad-type 1. A PAC placed at the top level as " +
        "ad-type 128 is " +
    "what a non-Windows client will refuse outright, because AD-IF-RELEVANT " +
        "is the wrapper " +
  ...
  assert.strictEqual(found[0].path, "AD-IF-RELEVANT → AD-WIN2K-PAC",
    "the path should say where it was found, so 'no PAC' can be told from 'a " +
        "PAC somewhere odd'");

  // A search that only looked at the top level would find nothing here — assert
  // the negative directly, so the unwrapping cannot be removed without this
  // failing.
  var topLevelOnly = reread.filter(function (e) { return e.type === pac.AD_TYPE.WIN2K_PAC; });
  assert.strictEqual(topLevelOnly.length, 0,
    "there is deliberately no ad-type 128 at the top level: that is what " +
        "makes the previous " +
    "assertion mean something");

  // Nested one deeper (AD-MANDATORY-FOR-KDC around AD-IF-RELEVANT), which real
  // KDCs do.
  var deeper = [{
  ...
  assert.strictEqual(deepFound.length, 1, "a PAC two containers deep should " +
      "still be found");
  assert.strictEqual(deepFound[0].path, "AD-MANDATORY-FOR-KDC → " +
      "AD-IF-RELEVANT → AD-WIN2K-PAC");

  // An unopenable container must not lose the PAC beside it, and must not
  // throw.
  var mixed = readAd(msg.encAuthorizationData([
    {
      type: pac.AD_TYPE.IF_RELEVANT,
      data: new Uint8Array([0x30, 0x05, 0xff, 0xff, 0xff])
    },
    ad[0]
  ]));
  var mixedFound = pac.findPacs(mixed);
  assert.strictEqual(mixedFound.length, 1,
    "a container this code cannot parse must not cost us the PAC in the next " +
        "element");
...
async function malformedPacsAreRefusedOrReported() {
  ...
  throwsWith(corrupt(function (b) { setU32(b, 0, 5000); }),
    /declares 5000 buffers|table alone would need/, "a PAC claiming 5000 " +
        "buffers");
  throwsWith(corrupt(function (b) { setU32(b, 0, 0); }), /zero buffers/,
    "a PAC claiming no buffers");

  // A non-zero Version is a MUST violation that still parses: it must be
  // REPORTED, not thrown, because the rest of the structure is still worth
  // reading.
  ...
  // A buffer offset that is not a multiple of 8: reported, and the buffer still
  ...
  assert.ok(misaligned.problems.some(function (p) { return /not a multiple of 8/.test(p); }),
    "a buffer offset that is not 8-aligned should be reported — Windows " +
        "rejects the PAC for it: " +
    misaligned.problems.join(" | "));

  // A buffer that claims to run past the end: recorded against that buffer,
  ...
  // which is precisely why it has to be counted: a second buffer is somewhere
  // to hide a different set of groups.
...
// The internal MUSTs of KERB_VALIDATION_INFO. Each of these produces a PAC that
// reads perfectly and behaves differently on a real Windows service, which is
// the only reason to check them deliberately.
async function theConsistencyRulesAreReported() {
  ...
  assert.ok(/GroupCount is 9 but the array's own conformant count is 3/
    ...
    "a GroupCount that disagrees with the array's own count must be refused, " +
        "got: " +
...
function theNdrReaderRefusesHostileInput() {
  ...
  throwsWith(function () {
    // 0xFFFFFF as a count: not credible, and the message should point at byte
    // order.
  ...
  assert.strictEqual(readBack.text, "S-1-5-21-1-2-3-1104",
    "a SID's identifier authority is the one BIG-endian field in the " +
        "structure");
  ...
  [["never", true], [null, false]].forEach(function (pair) {
    ...
    assert.strictEqual(!!ft.never, pair[1], "the 'never' sentinel should " +
        "round-trip");
  });
...
async function everySignatureEtypeWorks() {
  ...
  // 18/17 are the two [MS-PAC] section 2.8 defines for AES; 23 is RC4's -138,
  // which is still what an older domain will hand you.
  var cases = [
  ...
  for (var i = 0; i < cases.length; i++) {
    ...
    var results = await pac.verifySignatures(parsed, {
      serverKey: key,
      kdcKey: key
    });
  ...
  // -138 is 0xFFFFFF76 on the wire. Read unsigned it is 4294967158 and matches
  // nothing, so assert the signed reading explicitly.
  ...
  assert.strictEqual(hex(rc4Pac.subarray(at, at + 4)), "76ffffff",
    "KERB_CHECKSUM_HMAC_MD5 is -138, which is 0xFFFFFF76 little-endian on " +
        "the wire");
  ...
  assert.ok(/not one this code can compute/.test(r.note),
    "and the verifier should say it cannot compute that type rather than " +
        "reporting a bad " +
...
program
  .name("krb5_pac")
  .description("Verify the Windows PAC codec: the NDR layout at hand-derived " +
      "offsets, all four " +
