...
//
// That distinction is why this file exists. The pane is the easiest part of this
// feature to get convincingly wrong: it can display a key pair, a thumbprint and a
// decoded proof while the access token still arrives as an ordinary Bearer token —
// the wallet would LOOK as though it had DPoP and have none. So the assertions
// that carry this file are the ones about what came back:
//
...
//   * under Holder of Key the CREDENTIAL's cnf.jwk is that same key, which is the
//     entire claim of the second checkbox;
...
// needs a DPoP key to reuse, so it cannot take effect with DPoP off — and a ticked
// box that silently does nothing is the sort of quiet lie this suite exists to
// catch.
//
// Driven with the pre-authorized code grant (OID4VCI Appendix H.2), so it needs
// the STS mock and the client and NO identity provider — which is why it is a job
// of its own rather than a section of sd_jwt_vc_issuance.js.
...
async function txt(driver, id) {
  return await driver.executeScript(
    "var e=document.getElementById(arguments[0]); return e?e.textContent:'(missing)';", id);
}
...
// Waits on CONTENT, not on an element: every field read here is static markup that
// exists during parsing, so elementLocated would succeed while saying nothing about
// whether the page has filled it in. Same reasoning as tests/wait_for.js.
async function waitFor(driver, id, re, what) {
  ...
  try {
    ...
    throw new Error(what + " (last: " + JSON.stringify(String(last).slice(0, 260)) + ")");
  }
  return last;
}

// An assertion rather than a running tally: a failure here means the wallet is not
// doing DPoP, and a run that carried on would report later successes about a wallet
// whose token is not bound.
...
async function paneBeforeAnythingIsSent(driver) {
  ...
  await driver.executeScript(
    ...
    "localStorage.setItem('credential_endpoint', arguments[0] + '/oid4vci/credential');" +
    ...
    "localStorage.setItem('vci_credential_configuration_id', 'IdentityCredential');" +
    "localStorage.setItem('client_id', 'dpop-workflow-client');" +
    "localStorage.setItem('dpop_signing_alg_values_supported', 'ES256, RS256');", STS);
  await driver.navigate().refresh();
  await driver.wait(until.elementLocated(By.id("vc_dpop_enabled")), 20000);
  await driver.sleep(1200);

  check("the DPoP pane is on issuance step 2",
    (await driver.findElements(By.id("pane_dpop"))).length === 1, "pane_dpop is missing");
  ...
  check("DPoP is OFF by default",
    ...
    "it defaults to ON, which would change the workflow's default path — the Bearer flow is what " +
    "the specifications describe first and what the rest of the suite exercises");
  ...
  check("Holder of Key with DPoP off says Proof of Possession is what will happen",
    /Proof of Possession is what will actually happen/.test(
      await txt(driver, "vc_dpop_binding_note")),
    "a ticked box that silently does nothing is the failure this checks for. Note: " +
    await txt(driver, "vc_dpop_binding_note"));
  ...
  check("the public JWK is shown and carries no private half",
    ...
    "this JWK travels in every proof header, so a private member here would be published: " +
  ...
  // implementation written here — so a wrong member order or a stray member in the
  // page's own thumbprint is caught rather than agreed with.
  ...
  check("the displayed jkt is the RFC 7638 thumbprint of the displayed key",
    recomputed === jkt, "page says " + jkt + ", recomputed independently as " + recomputed);
  log.debug("Leaving paneBeforeAnythingIsSent().");
}

// The mock issuer's cross-device screen carries the offer URI and the Transaction
// Code; step 1's Receive pane takes the offer; step 2 redeems it with the
// pre-authorized code grant, which needs no identity provider.
async function boundTokenThroughTheOfferFlow(driver) {
  ...
  var screen = await driver.executeScript(
    "return { offerUri: document.getElementById('offer_uri').textContent.trim()," +
  ...
  check("the issuer's cross-device screen offers a credential-offer URI",
    screen.offerUri.indexOf("openid-credential-offer://") === 0, screen.offerUri.slice(0, 100));
  check("...and a Transaction Code", /^[0-9]{4,8}$/.test(screen.txCode), screen.txCode);
  ...
  // A pre-authorized offer needs no authorization request, so Start Issuance goes
  // straight to step 2.
  ...
  await driver.executeScript("document.getElementById('start_issuance_button').click();");
  ...
  check("DPoP survived the hand-off from step 1 — it is stored, not per-page",
    ...
    "the checkbox came back unchecked, so the preference did not survive a navigation");
  var jkt = (await txt(driver, "vc_dpop_jkt")).trim();
  ...
  check("step 2 is in the pre-authorized state",
    ...
    "the offer did not put step 2 into the pre-authorized Token Request state, so the rest of " +
    "this test would prove nothing");
  await driver.executeScript(
    "document.getElementById('vc_tx_code').value = arguments[0]; vcissuance2.onTxCodeChange();",
    screen.txCode);
  await driver.executeScript("document.getElementById('vc_token_request_button').click();");
  var tokenStatus = await waitFor(driver, "vc_token_status", /issued|refused|failed/i,
  ...
  check("the access token ITSELF carries cnf.jkt equal to the wallet's thumbprint",
    cnf && cnf.jkt === jkt,
    "this is what the pane cannot fake: the binding is inside the signed token. cnf=" +
  ...
  check("the DPoP proof that was sent is shown decoded",
    /dpop\+jwt/.test(proof) && /"htm": "POST"/.test(proof) && /oauth2\/token/.test(proof),
    proof.slice(0, 240));
  check("...and the Token Request proof carries no ath — there was no token yet",
    !/"ath"/.test(proof),
    "an ath here would be a claim about a token that does not exist: " + proof.slice(0, 300));

  var call = await val(driver, "vc_approval_request");
  check("the assembled Credential Request shows Authorization: DPoP and a DPoP header line",
    /Authorization: DPoP /.test(call) && /^DPoP: /m.test(call),
    "a pane showing `Authorization: DPoP` with no DPoP header beside it describes a request every " +
    "conforming server refuses. First lines: " + String(call).split("\n").slice(0, 5).join(" | "));
...
async function credentialIsBoundToTheDpopKey(driver) {
  ...
  await driver.executeScript("document.getElementById('vc_approve_button').click();");
  var approved = "";
  try {
    await driver.wait(async function () {
    ...
    throw new Error("approving produced neither a status nor a navigation (last: " +
  ...
  var dpopPublic = await driver.executeScript(
    "return JSON.parse(localStorage.getItem('sdjwtvc_dpop_public_jwk') || 'null');");
  check("the credential's cnf.jwk IS the DPoP key",
    ...
    "this is the whole claim of the second checkbox — one key as the access token's cnf.jkt and " +
    "as the credential's cnf.jwk. credential cnf.jwk=" + JSON.stringify(credentialCnf) +
    "  dpop public=" + JSON.stringify(dpopPublic));
  log.debug("Leaving credentialIsBoundToTheDpopKey().");
}

// The consequence of Holder of Key that reaches the OTHER workflow. A wallet that
// reached for the holder key here would produce a presentation the verifier
// refuses, and the complaint would be about the Key Binding JWT's signature rather
// than about the wrong key having been chosen.
async function presentationSideKnowsWhichKey(driver) {
  ...
  check("...and recognises that this credential was issued under Holder of Key",
    ...
    "the note is built from the credential in hand, so it should recognise that its cnf.jwk is " +
...
async function turningItOffDiscardsTheKey(driver) {
  ...
  check("turning DPoP off discards the key pair",
    ...
    "the private key survived being switched off — a key nothing will use again, which a later " +
    "session could pick up and bind a credential to by accident");
  check("and the jkt display is cleared",
    (await txt(driver, "vc_dpop_jkt")).trim() === "—", await txt(driver, "vc_dpop_jkt"));
...
async function test() {
  ...
  // Web Crypto and the private-network flags. Every proof on this page is signed
  // with crypto.subtle, which does not exist on the containerized suite's
...
program
  .name("dpop_workflow")
  .description("Verify DPoP (RFC 9449) through the VC Issuance pages: the pane, the binding, Holder of Key.")
