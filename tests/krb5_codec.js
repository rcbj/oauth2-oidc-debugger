...
// encoding — that is what catches a field moving to the wrong context tag,
// which a round trip cannot see. The outer LENGTH byte is not part of that: it
// is arithmetic, and hand-arithmetic in an oracle is how this test file
// acquired three wrong expectations in a row, each of which cost a round of
// suspecting correct code. So the tags are written out and the length is
// computed.
...
function structuresEncodeToTheExpectedBytes() {
  ...
  const krbtgt = msg.encPrincipalName({
    type: msg.NAME_TYPE.SRV_INST,
  ...
  eq("Checksum type -138",
     msg.encChecksum({
       type: -138,
       checksum: unhex("00112233445566778899aabbccddeeff")
     }),
  ...
  const pa = msg.readPaData(asn1.readTlv(msg.encPaData({
    type: 19,
    value: unhex("00")
  }), 0));
  assert.strictEqual(pa.type, 19, "padata-type must be read from tag [1]");
  assert.strictEqual(pa.typeName, "ETYPE_INFO2", "a known padata type must " +
      "be named");
...
function kdcRequestsRoundTrip() {
  ...
  // carrying them could be decoded and never re-encoded. Populate everything,
  // or the round trip only covers the fields somebody happened to think of.
  const req = {
    msgType: msg.MSG_TYPE.AS_REQ,
    padata: [{
      type: msg.PA_TYPE.PAC_REQUEST,
    ...
    reqBody: {
      ...
      // Addressless tickets are the norm and AD issues them, which is exactly
      // why this is here: the rarely-used field is the one that rots.
  ...
  // The outermost tag identifies the message before anything is parsed, which
  // is what the api's relay uses to decide a payload is Kerberos at all.
  assert.strictEqual(bytes[0], 0x6a, "an AS-REQ must be tagged [APPLICATION " +
      "10] (0x6a)");
  assert.deepStrictEqual(msg.identify(bytes), {
  ...
  assert.strictEqual(minimal.reqBody.from, null, "an absent OPTIONAL must " +
      "read as null");
  ...
  assert.strictEqual(minimal.reqBody.additionalTickets, null, "absent " +
      "additional-tickets");
  assert.deepStrictEqual(minimal.padata, [], "absent padata reads as an " +
      "empty list");
  ...
  assert.strictEqual(tgs[0], 0x6c, "a TGS-REQ must be tagged [APPLICATION " +
      "12] (0x6c)");
  ...
  assert.ok(tgsBack.reqBody.raw && tgsBack.reqBody.raw.length > 0,
    "the KDC-REQ-BODY's own bytes must be retained — a checksum is computed " +
        "over them");
  eq("a body relayed by its raw bytes is unchanged",
     msg.encKdcReq({
       msgType: msg.MSG_TYPE.TGS_REQ,
       padata: tgsBack.padata,
       reqBody: tgsBack.reqBody
     }),
     hex(tgs));
...
async function kdcRepliesRoundTripAndTolerateTheIrregularTag() {
  ...
  assert.strictEqual(asRep[0], 0x6b, "an AS-REP must be tagged [APPLICATION " +
      "11] (0x6b)");

  const response = msg.readKdcResponse(asRep);
  assert.strictEqual(response.kind, "AS-REP", "readKdcResponse must classify " +
      "an AS-REP");
  ...
  assert.strictEqual(part.nonce, 3735928559, "the nonce must match the " +
      "request's");
  assert.deepStrictEqual(msg.ticketFlagNames(part.flags),
    ["forwardable", "initial", "pre-authent"], "ticket flags must render by " +
        "name");
...
function relayedTicketsAreByteExact() {
  ...
  assert.notStrictEqual(hex(msg.encTicket({
    ...
    "building the same ticket from FIELDS should normalise the header — " +
        "which is why raw is kept");
...
function krbErrorsCarryTheSalt() {
  ...
  assert.strictEqual(err[0], 0x7e, "a KRB-ERROR must be tagged [APPLICATION " +
      "30] (0x7e)");

  const response = msg.readKdcResponse(err);
  assert.strictEqual(response.kind, "KRB-ERROR",
    "a KDC's error must be classified as an error, not treated as a parse " +
        "failure");
  const e = response.error;
  assert.strictEqual(e.errorCode, 25, "error-code");
  assert.strictEqual(e.error.name, "KDC_ERR_PREAUTH_REQUIRED", "the code " +
      "must be named");
  assert.ok(/salt/i.test(e.error.meaning),
    "the description for PREAUTH_REQUIRED must say it carries the salt, " +
        "since that is the point");
  assert.strictEqual(e.stime.getTime(), stime.getTime(), "stime — which is " +
      "how clock skew is measured");
  ...
  assert.strictEqual(e.eDataPaData[0].type, 19, "e-data carries " +
      "PA-ETYPE-INFO2");
  ...
  assert.strictEqual(info[1].salt, null, "arcfour's entry has no salt, and " +
      "that is meaningful");

  // An error whose e-data is NOT PA-DATA must not throw — on other error codes
  // it is something else entirely, and a decoder that dies there loses the
  // error message the user actually needed.
  ...
  assert.strictEqual(other.eDataPaData, null, "unparseable e-data must read " +
      "as null");
  ...
  assert.ok(/RC4|AES/.test(other.error.meaning),
    "ETYPE_NOSUPP's description should name the 2026 cause, since that is " +
        "the whole diagnosis");
...
async function apExchangeRoundTrips() {
  ...
  assert.strictEqual(apReq[0], 0x6e, "an AP-REQ must be tagged [APPLICATION " +
      "14] (0x6e)");
  ...
  assert.strictEqual(repPart.ctime.getTime(), ctime.getTime(),
    "the AP-REP echoes the authenticator's ctime — that echo IS the mutual " +
        "authentication");
  assert.strictEqual(repPart.seqNumber, 987654321, "the acceptor's sequence " +
      "number");
...
function paForUserRoundTrips() {
  ...
  assert.strictEqual(back.cksum.type, -138, "KERB_CHECKSUM_HMAC_MD5 is " +
      "negative");
...
function theTreeViewDescribesUnknownBytes() {
  ...
  assert.strictEqual(nodes[0].tagName, "SEQUENCE", "the outer element is a " +
      "SEQUENCE");
  ...
  assert.strictEqual(opaque[0].text, "deadbeef", "an OCTET STRING renders as " +
      "hex");

  // A flags BIT STRING names the bits that are set.
  const flagTree = asn1.tree(asn1.encFlags([1, 8]));
  assert.ok(/1, 8/.test(flagTree[0].text), "a KerberosFlags renders which " +
      "bits are set: " + flagTree[0].text);
...
function refusesMalformedAndHostileInput() {
  ...
  mustThrow("a Kerberos message of the wrong type",
    () => msg.readKdcReq(msg.encApRep({ encPart: {
      etype: 18,
      cipher: unhex("00")
    } })),
    /expected AS-REQ .* or TGS-REQ/);
  mustThrow("an AP-REP read as a KDC response",
    () => msg.readKdcResponse(msg.encApRep({ encPart: {
      etype: 18,
      cipher: unhex("00")
    } })),
  ...
  // for the TREE view the limit is a display cap, not a refusal: bytes pasted
  // into the decoder page should render as far as they go. What must not happen
  // is a silent cap, where a constructed element renders as a hex leaf and so
  // reads as opaque data. It has to say it stopped.
...
program
  .name("krb5_codec")
  .description("Verify the Kerberos v5 DER codec: byte-exact encodings, " +
      "round trips, and the compatibility cases.")
