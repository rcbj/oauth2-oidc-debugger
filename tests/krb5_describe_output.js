...
function allRows(doc) {
  ...
  (function walk(sections) {
    (sections || []).forEach(function (s) {
      (s.rows || []).forEach(function (r) { out.push(Object.assign({ section: s.title }, r)); });
      walk(s.sections);
    });
  })(doc.sections);
  return out;
...
// Sections nest — a decrypted EncTicketPart sits inside the Ticket section, which
// sits inside the AS-REP — so an assertion that only looks at doc.sections misses
...
// Every value handed to the renderer must already be a string: the page puts them
// through textContent and never formats. A number or an object leaking through is
// how a renderer acquires formatting logic, which is how two pages start
// disagreeing.
function everyValueIsAString(doc, what) {
  allRows(doc).forEach(function (r) {
    assert.strictEqual(typeof r.name, "string", what + ": a row name is not a string");
    assert.strictEqual(typeof r.value, "string",
      what + ": the value of row '" + r.name + "' is a " + typeof r.value + ", not a string");
    if (r.note !== null && r.note !== undefined) {
      assert.strictEqual(typeof r.note, "string", what + ": the note on row '" + r.name + "' is not a string");
    }
  });
}
...
async function acceptsEveryShapeACaptureArrivesIn() {
  ...
  const forms = [
    ...
    ["base64url", b64(message).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "")],
    ["hex", hex(message)],
  ...
  for (const [label, text] of forms) {
    ...
    assert.strictEqual(doc.kind, "AS-REQ", "input as " + label + " must decode to an AS-REQ");
    assert.strictEqual(doc.input.byteLength, message.length, label + ": byte length");
  }

  // The TCP framing, which a capture very often includes.
  const framed = prim.concat([
    new Uint8Array([(message.length >>> 24) & 255, (message.length >>> 16) & 255,
                    (message.length >>> 8) & 255, message.length & 255]),
    message]);
  const framedDoc = await describe.describe(b64(framed));
  assert.strictEqual(framedDoc.kind, "AS-REQ", "a TCP-framed message must still decode");
  assert.ok(/TCP length prefix/.test(framedDoc.input.framing || ""),
    "the stripped framing must be REPORTED, not silently removed: " + framedDoc.input.framing);
  assert.strictEqual(framedDoc.input.byteLength, message.length, "the prefix is not part of the message");

  // A length prefix that does not match the body is NOT framing, and treating it
  // as such would silently discard four bytes of a real message.
  ...
  assert.strictEqual(wrongDoc.input.framing, null,
    "a prefix whose length does not match the body must not be treated as framing");
...
async function describesAnAsReqAndNamesWhatIsWrongWithIt() {
  ...
  const good = await describe.describe(b64(msgs.encKdcReq({
    msgType: msgs.MSG_TYPE.AS_REQ,
    padata: [{ type: msgs.PA_TYPE.PAC_REQUEST, value: msgs.encPaPacRequest(false) }],
    reqBody: {
      kdcOptions: [msgs.KDC_OPTION.FORWARDABLE, msgs.KDC_OPTION.RENEWABLE, msgs.KDC_OPTION.CANONICALIZE],
      cname: { type: 1, name: ["alice"] },
      realm: "EXAMPLE.COM",
      sname: { type: 2, name: ["krbtgt", "EXAMPLE.COM"] },
      till: new Date(Date.UTC(2026, 7, 14)),
  ...
  assert.ok(/forwardable/.test(rowNamed(good, /^kdc-options$/).value), "options rendered by name");
  ...
  assert.ok(/aes256-cts-hmac-sha1-96/.test(rowNamed(good, /^etype$/).value), "etypes named, not numbered");
  assert.ok(/case-sensitive/.test(rowNamed(good, /^realm$/).note || ""),
    "the realm row must warn that it is case-sensitive");
  // PA-PAC-REQUEST of false is a legitimate diagnostic and must be shown as such.
  assert.strictEqual(rowNamed(good, /include-pac/).value, "false", "include-pac must be shown");
  assert.deepStrictEqual(good.problems, [], "a well-formed AS-REQ has no problems: " + good.problems);
  ...
  const noPreauth = await describe.describe(b64(msgs.encKdcReq({
    msgType: msgs.MSG_TYPE.AS_REQ,
    reqBody: { kdcOptions: [], cname: { type: 1, name: ["alice"] }, realm: "EXAMPLE.COM",
               sname: { type: 2, name: ["krbtgt", "EXAMPLE.COM"] },
  ...
  assert.ok(/PREAUTH_REQUIRED/.test(padataSection.note || ""),
    "an AS-REQ with no padata must explain what the KDC will answer: " + padataSection.note);
  ...
  assert.ok(hasProblem(bad, /not upper case/), "a lower-case realm must be a finding");
  assert.ok(hasProblem(bad, /RC4/) && hasProblem(bad, /Windows Server 2025/),
    "an RC4-only etype list must be a finding that names the 2026 cause: " + bad.problems.join(" | "));
  assert.ok(hasProblem(bad, /NEVER_VALID/), "till before from must be a finding");

  const des = await describe.describe(b64(msgs.encKdcReq({
    msgType: msgs.MSG_TYPE.AS_REQ,
    reqBody: { kdcOptions: [], realm: "EXAMPLE.COM", sname: { type: 2, name: ["krbtgt", "EXAMPLE.COM"] },
               till: new Date(Date.UTC(2026, 7, 14)), nonce: 1, etypes: [3, 18] }
  })));
  assert.ok(hasProblem(des, /DES/), "a DES etype must be a finding even alongside a good one");
  ...
  const tgsNoTgt = await describe.describe(b64(msgs.encKdcReq({
    ...
    reqBody: { kdcOptions: [], realm: "EXAMPLE.COM", sname: { type: 3, name: ["HTTP", "web.example.com"] },
               till: new Date(Date.UTC(2026, 7, 14)), nonce: 1, etypes: [18] }
  })));
  assert.strictEqual(tgsNoTgt.kind, "TGS-REQ", "kind");
  assert.ok(hasProblem(tgsNoTgt, /no PA-TGS-REQ/),
    "a TGS-REQ without its TGT must be called out: " + tgsNoTgt.problems.join(" | "));
...
async function describesAnAsRepAndDecryptsWhenGivenKeys() {
  ...
  const serviceKey = await e.stringToKey("krbtgtpw", prim.utf8("EXAMPLE.COMkrbtgt"), null);
  ...
  const asRep = msgs.encKdcRep({
    ...
    ticket: {
      realm: "EXAMPLE.COM",
      sname: { type: 2, name: ["krbtgt", "EXAMPLE.COM"] },
      encPart: { etype: 18, cipher: await e.encrypt(serviceKey, kcrypto.KEY_USAGE.KDC_REP_TICKET,
        msgs.encEncTicketPart({
          flags: [msgs.TICKET_FLAG.FORWARDABLE, msgs.TICKET_FLAG.INITIAL, msgs.TICKET_FLAG.PRE_AUTHENT],
    ...
    encPart: { etype: 18, cipher: await e.encrypt(clientKey, kcrypto.KEY_USAGE.AS_REP_ENCPART,
      msgs.encEncKdcRepPart({
        key: { etype: 18, key: sessionKey }, lastReq: [{ type: 0, value: now }], nonce: 4242,
        flags: [msgs.TICKET_FLAG.FORWARDABLE, msgs.TICKET_FLAG.INITIAL],
        authtime: now, endtime: end,
        srealm: "EXAMPLE.COM", sname: { type: 2, name: ["krbtgt", "EXAMPLE.COM"] }
  ...
  assert.ok(ticketEnc.length >= 2,
    "both the ticket and the enc-part must report that they were not decrypted");
  ...
  assert.strictEqual(keyRow.value.indexOf("aes256-cts-hmac-sha1-96"), 0, "the session key's etype is named");
  assert.ok(/^EncASRepPart/.test(rowNamed(withClient, /tagged as/).value), "the enc-part tag is reported");
  assert.strictEqual(rowNamed(withClient, /^nonce$/).value, "4242", "the nonce is read out of the enc-part");
  assert.ok(/must equal the request/.test(rowNamed(withClient, /^nonce$/).note || ""),
    "and the nonce row must say why it matters");

  // With the SERVICE's key as well: now the ticket opens too, which is the whole
  // point of accepting a keytab.
  ...
  assert.ok(/krbtgt key/.test(ticketPart.note || ""),
    "the section must say WHICH key opened it, so a reader with several keys knows: " + ticketPart.note);
  assert.ok(/SESSION key/.test(allRows(withBoth).filter(function (r) {
    return r.section === ticketPart.title && r.name === "key";
  })[0].note || ""), "and must flag that the session key inside is itself a credential");
...
async function describesErrorsAndMeasuresSkew() {
  ...
  const etypeInfo2 = msgs.encEtypeInfo2([
    { etype: 18, salt: "EXAMPLE.COMhostweb.example.com", s2kparams: unhex("00008000") },
  ...
  assert.ok(/KDC_ERR_PREAUTH_REQUIRED/.test(preauth.summary), "the summary names the code");
  ...
  assert.ok(/not guessable/.test(saltRow.note || ""),
    "and the note must say the salt cannot be guessed, which is why this error matters");
  assert.deepStrictEqual(preauth.problems, [],
    "KDC_ERR_PREAUTH_REQUIRED is NOT a problem — it is where the salt comes from");
  ...
  const nosupp = await describe.describe(b64(msgs.encKrbError({
    stime: new Date(), susec: 0, errorCode: 14, realm: "EXAMPLE.COM",
    sname: { type: 2, name: ["krbtgt", "EXAMPLE.COM"] }
  })));
  assert.ok(hasProblem(nosupp, /ETYPE_NOSUPP/), "a real error must be a problem");
  assert.ok(/RC4|msDS-SupportedEncryptionTypes/.test(rowNamed(nosupp, /^meaning$/).value),
    "the meaning must diagnose, not restate: " + rowNamed(nosupp, /^meaning$/).value);
  ...
  const skewed = await describe.describe(b64(msgs.encKrbError({
    stime: new Date(Date.now() - 20 * 60 * 1000), susec: 0, errorCode: 37, realm: "EXAMPLE.COM",
    sname: { type: 2, name: ["krbtgt", "EXAMPLE.COM"] }
  })));
  ...
  const fine = await describe.describe(b64(msgs.encKrbError({
    stime: new Date(Date.now() - 5000), susec: 0, errorCode: 6, realm: "EXAMPLE.COM",
    sname: { type: 2, name: ["krbtgt", "EXAMPLE.COM"] }
  })));
  assert.ok(!hasProblem(fine, /clock difference/), "a five-second skew is not a finding");
  assert.ok(hasProblem(fine, /C_PRINCIPAL_UNKNOWN/), "but the error itself still is");
...
async function fallsBackToStructureRatherThanRefusing() {
  ...
  assert.strictEqual(notKrb.kind, "unrecognised", "kind for non-Kerberos ASN.1");
  assert.ok(notKrb.tree && notKrb.tree.length === 1, "the ASN.1 tree must be shown instead");
  assert.strictEqual(notKrb.tree[0].tagName, "SEQUENCE", "and be structurally right");
  assert.ok(/ASN.1 structure/.test(notKrb.summary), "the summary must explain the fallback");
  ...
  const truncated = msgs.encKdcReq({
    msgType: msgs.MSG_TYPE.AS_REQ,
    reqBody: { kdcOptions: [], realm: "EXAMPLE.COM", sname: { type: 2, name: ["krbtgt", "X"] },
  ...
  assert.ok(/does not parse/.test(bad.kind), "a self-declared message that fails must say so: " + bad.kind);
  assert.ok(bad.problems.length > 0, "and record why");

  // Not decodable as either encoding.
  await assert.rejects(() => describe.describe("this is definitely not base64 or hex!!"),
...
function buildKeytab(version, entries) {
  ...
  function u16(n) { return big ? new Uint8Array([(n >> 8) & 255, n & 255]) : new Uint8Array([n & 255, (n >> 8) & 255]); }
  function u32(n) {
    const b = [(n >>> 24) & 255, (n >>> 16) & 255, (n >>> 8) & 255, n & 255];
    return new Uint8Array(big ? b : b.reverse());
  }
  function i32(n) { return u32(n < 0 ? (n >>> 0) : n); }
  function counted(text) { return prim.concat([u16(text.length), prim.utf8(text)]); }
...
function readsKeytabsInBothByteOrders() {
  ...
  const entries = [
    { realm: "EXAMPLE.COM", components: ["HTTP", "web.example.com"], nameType: 3, kvno: 5,
      etype: 18, key: key18, timestamp: 1755000000 },
    { deleted: 24 },
    { realm: "EXAMPLE.COM", components: ["HTTP", "web.example.com"], nameType: 3, kvno: 5,
      etype: 17, key: key17, timestamp: 1755000000 }
  ];
  ...
  [0x02, 0x01].forEach(function (version) {
    ...
    assert.strictEqual(parsed.bigEndian, version === 0x02, label + ": byte order");
    ...
    assert.strictEqual(parsed.deletedSlots, 1, label + ": the deleted slot must be counted");
    assert.strictEqual(parsed.entries.length, 2,
      label + ": the entry after a deleted slot must still be read");
    assert.strictEqual(parsed.entries[0].principal, "HTTP/web.example.com@EXAMPLE.COM",
      label + ": principal");
    assert.strictEqual(parsed.entries[0].kvno, 5, label + ": kvno");
    assert.strictEqual(parsed.entries[0].etype, 18, label + ": etype");
    assert.strictEqual(parsed.entries[0].etypeName, "aes256-cts-hmac-sha1-96", label + ": etype name");
    assert.strictEqual(hex(parsed.entries[0].key), hex(key18), label + ": the key itself");
    assert.strictEqual(hex(parsed.entries[1].key), hex(key17), label + ": the second key");

    const keys = keytab.keysFromKeytab(parsed);
    assert.strictEqual(keys.length, 2, label + ": both keys offered for decryption");
    assert.ok(/HTTP\/web.example.com/.test(keys[0].label),
      label + ": a key's label must name its principal so the page can say which one opened a ticket");
  });
  ...
  mislabelled[1] = 0x02;
  let threw = null;
  try { keytab.parseKeytab(mislabelled); } catch (e) { threw = e; }
  assert.ok(threw, "a keytab whose declared byte order is wrong must be refused");
  assert.ok(/byte order|not credible/i.test(threw.message),
    "and the message must point at the byte order: " + threw.message);
  // Not just name it — say what the bytes mean the other way round, which is the
  // fact that turns the diagnosis into a fix.
  ...
  const cases = [
    ...
    [unhex("0503"), /too short/],                      // the length check comes first
    [unhex("05030000"), /format version/],             // long enough; an unknown version
    [unhex("05040000"), /format version/],
    [prim.utf8("BQIAAAA="), /not a keytab/],           // a base64 keytab pasted as text
    [new Uint8Array(keytab.MAX_KEYTAB_BYTES + 1).fill(5), /refusing to parse/]
  ];
  cases.forEach(function (c) {
    let e = null;
    try { keytab.parseKeytab(c[0]); } catch (err) { e = err; }
    assert.ok(e, "expected a refusal for " + hex(c[0].subarray(0, 4)));
    assert.ok(c[1].test(e.message), "refusal must explain: " + e.message + " (wanted " + c[1] + ")");
  });
...
async function aKeytabOpensATicket() {
  ...
  const e = kcrypto.etypeById(18);
  const serviceKey = await e.stringToKey("Passw0rd!", prim.utf8("EXAMPLE.COMHTTPweb.example.com"), null);
  const sessionKey = kcrypto.randomBytes(32);
  const now = new Date(Date.UTC(2026, 7, 13, 12, 0, 0));
  ...
  const kt = buildKeytab(0x02, [{
    realm: "EXAMPLE.COM", components: ["HTTP", "web.example.com"], nameType: 3,
    kvno: 3, etype: 18, key: serviceKey, timestamp: 1755000000
  }]);
  ...
  assert.strictEqual(hex(prim.fromHex(rowNamed(doc, /^key$/).value.split(", ")[1])), hex(sessionKey),
    "the session key inside the ticket must be revealed by the keytab");
  assert.deepStrictEqual(rowNamed(doc, /^cname$/).value.indexOf("alice") >= 0, true,
    "and the client the ticket was issued to");
  assert.ok(allRows(doc).some(function (r) { return /ok-as-delegate/.test(r.value); }),
    "ok-as-delegate must be visible — it is what tells a client this service may be delegated to");
  assert.ok(doc.sections.some(function (s) {
    return (s.sections || []).some(function (c) { return /keytab HTTP/.test(c.note || ""); });
  }) || allRows(doc).length > 0, "the section must say which keytab entry opened it");
...
//
// The codec itself is tested in krb5_pac.js. What is tested HERE is the presentation:
// that the PAC is found two containers deep in the authorization-data, that the group
// memberships reach the screen as SIDs with names rather than as bare RIDs, and that
// the signatures are reported by name with the key that opened the ticket used as the
// service key — which it is, by definition, since it decrypted this very ticket.
//
// This section exists because a decoded structure that no test renders is a structure
// that is broken: everything below the ticket's authorization-data was a single row of
// hex until phase 4, and nothing would have noticed if the new panes threw.
async function aTicketsPacIsDecodedAndItsSignaturesReported() {
  ...
  const pacBytes = await kpac.buildPac({
    ...
    logonInfo: {
      ...
      logonServer: "DC01", logonDomainName: "EXAMPLE", logonDomainId: DOMAIN,
      userId: 1104, primaryGroupId: 513,
  ...
  const pacSection = sectionTitled(doc, /^PAC \(Privilege Attribute Certificate\)$/);
  ...
  assert.ok(/AD-IF-RELEVANT → AD-WIN2K-PAC/.test(pacSection.note),
    "the note should say where the PAC was found, because 'no PAC' and 'a PAC somewhere odd' " +
  ...
  assert.strictEqual(sidRow.value, DOMAIN + "-1104",
    "the account's SID has to be assembled from LogonDomainId + UserId — a PAC never carries " +
    "it whole, and the SID is the thing a Windows service checks");
  assert.ok(/never carries it whole/.test(sidRow.note || ""), "and the note should say so");

  // Groups, as SIDs with their well-known names.
  const groupsRow = rowNamed(doc, /^groups \(2\)$/);
  assert.ok(groupsRow, "the group list must be rendered, with its count. Rows: " +
    allRows(doc).map(function (r) { return r.name; }).join(", "));
  ...
  assert.ok(/Domain Admins/.test(groupsRow.value),
    "and the well-known ones should be named — 'S-1-5-21-…-512' is work the reader should not " +
    "have to do: " + groupsRow.value);
  assert.ok(/ENABLED/.test(groupsRow.value), "with the SE_GROUP attributes shown");
  ...
  assert.ok(/USER_ACCOUNT codes, not the LDAP/.test(rowNamed(doc, /UserAccountControl/).note || ""),
    "the UserAccountControl note should warn which of the two tables these bits come from");
  ...
  assert.ok(/^verified/.test(serverSig.value),
    "the key that decrypted this ticket IS the service's long-term key, so the server signature " +
    "must verify with it and no further input: " + serverSig.value + " — " + serverSig.note);
  ...
  assert.strictEqual(kdcSig.value, "not checked — HMAC_SHA1_96_AES256",
    "the KDC signature needs the krbtgt key, which a service does not have. It must read as " +
    "NOT CHECKED rather than as a failure, or every ticket looks forged: " + kdcSig.value);
  assert.ok(/expected result rather than a sign of tampering/.test(kdcSig.note || ""),
    "and the note must say that this is EXPECTED, not suspicious. The service key is in the pool " +
    "and is the same etype, so it gets tried and fails — reporting that as a failed signature " +
    "would put a scary red line on every correct PAC anyone ever pastes in: " + kdcSig.note);
  ...
  assert.ok(!hasProblem(doc, /PAC/),
    "a well-formed PAC should raise no problems: " + JSON.stringify(doc.problems));
  everyValueIsAString(doc, "a ticket carrying a PAC");

  // Now the negative: alter the PAC inside the ticket and confirm the page SAYS so.
  // Signature failure is the finding a debugger exists to surface.
  const parsedPac = kpac.parsePac(pacBytes);
  const tamperedPac = new Uint8Array(pacBytes);
  tamperedPac[kpac.bufferOfType(parsedPac, kpac.TYPE.LOGON_INFO).offset + 40] ^= 0xff;
  ...
  assert.ok(hasProblem(bad, /Server checksum does not verify against the service key/),
    "an altered PAC must be reported as a problem, not merely rendered: " +
    JSON.stringify(bad.problems));

  // A ticket with authorization-data but no PAC in it: worth saying, and not a problem.
  const noPac = await describe.describe(b64(msgs.encTicket({
    ...
    encPart: { etype: 18, cipher: await e.encrypt(serviceKey, kcrypto.KEY_USAGE.KDC_REP_TICKET,
      msgs.encEncTicketPart({
        ...
        crealm: "EXAMPLE.COM", cname: { type: 1, name: ["alice"] },
        authtime: now, endtime: new Date(Date.UTC(2026, 7, 13, 22, 0, 0)),
        authorizationData: [{ type: kpac.AD_TYPE.SERVICE_TARGET, data: prim.utf8("x") }]
      })) }
  })), { keys: [{ etype: 18, key: serviceKey, label: "the service's password" }] });
  ...
  assert.ok(/USER_NO_AUTH_DATA_REQUIRED/.test(rowNamed(noPac, /^PAC$/).note || ""),
    "and should name a reason that would explain it: " + rowNamed(noPac, /^PAC$/).note);
  assert.ok(!hasProblem(noPac, /PAC/),
    "but an absent PAC is not by itself WRONG — this file's rule is that problems are for what " +
...
// A pasted KRB-CRED — a DELEGATED CREDENTIAL, which is the one structure this page renders
// that is a capability rather than a claim. Somebody who has captured one is holding
// somebody else's ability to act as them, and the page should say so rather than listing
// fields neutrally.
//
// This section exists because nothing else opens that pane: the workflow never pastes one,
// so a `ReferenceError` in it would go unnoticed indefinitely — which is exactly how the
// WebAuthn Analyzer's extension-capture pane stayed broken.
async function aDelegatedCredentialIsDescribedAsACapability() {
  ...
  // Without the key: the structure is visible and the enc-part is not, and the page must
  // say WHICH key would open it — usage 14 under the AP exchange's subkey is not something
  // a reader guesses.
  const closed = await describe.describe(b64(cred));
  assert.strictEqual(closed.kind, "KRB-CRED", "a pasted KRB-CRED must be recognised");
  assert.ok(/unconstrained delegation/.test(closed.summary),
    "and the summary should say what it IS, not just name the message: " + closed.summary);
  const section = sectionTitled(closed, /^KRB-CRED$/);
  assert.ok(/tickets to ANYTHING as that client/.test(section.note),
    "the note must explain the capability rather than naming the structure: " + section.note);
  assert.ok(/krbtgt\/EXAMPLE.COM/.test(rowNamed(closed, /^tickets$/).value),
    "the forwarded ticket should be named: " + rowNamed(closed, /^tickets$/).value);
  assert.ok(/key usage 14/.test(rowNamed(closed, /^decryption$/).note || ""),
    "and an unopened enc-part must say which key opens it — usage 14 under the AP exchange's " +
    "subkey is not guessable: " + rowNamed(closed, /^decryption$/).note);

  // With the key: the forwarded ticket's own session key appears, which is the thing that
  // makes the credential usable.
  ...
  assert.ok(keyRow.value.indexOf(hex(forwardedSession)) !== -1,
    "the forwarded ticket's session key must be shown — that key plus the ticket IS being that " +
  ...
  assert.ok(/handed over rather than presented by/.test(rowNamed(open, /^flags$/).note || ""),
    "the `forwarded` flag is the record a service has that these were delegated, and the page " +
...
async function test() {
  ...
  log.info("Starting Test run. Verifying common/krb5/krb5_describe.js and krb5_keytab.js.");
...
program
  ...
  .description("Verify what the Kerberos decoder page shows, and the keytab reader behind it.")
