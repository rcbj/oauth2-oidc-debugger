...
//
// Everything here is checkable without a browser or a service, and every part of
// it fails SILENTLY when it is wrong: a wrong thumbprint, a wrong `htu` or a
...
async function thumbprintMatchesPublishedVectors() {
  ...
  assert.strictEqual(ec, RFC9449_JKT,
    "the EC key from RFC 9449's own worked example must hash to the jkt that same document " +
    "prints for the token bound to it. Got " + ec + ", expected " + RFC9449_JKT + ".");
  var rsa = await dpop.thumbprint(RFC7638_JWK);
  assert.strictEqual(rsa, RFC7638_JKT,
    "the RSA key from RFC 7638 section 3.1 must hash to the thumbprint printed beside it. " +
    "Got " + rsa + ", expected " + RFC7638_JKT + ".");
  log.info("[thumbprint] OK — the RFC 9449 EC vector and the RFC 7638 RSA vector both match.");
  ...
  assert.strictEqual(canonical,
    '{"crv":"P-256","kty":"EC","x":"l8tFrhx-34tV3hRICRDY9zCkDlpBhF42UQUfWVAWBFs",' +
    '"y":"9VE4jf_Ok_o64zbTTlcuNJajHmt6v9TDVrU0CdvGRDA"}',
    "the thumbprint input must be the required members, lexicographic, with no whitespace.");
...
async function thumbprintIgnoresEverythingItShould() {
  ...
  assert.strictEqual(await dpop.thumbprint(decorated), RFC9449_JKT,
    "kid/alg/use/key_ops/ext are NOT thumbprint input — a key that gains any of them must " +
    "still match the token bound to it. This is what Web Crypto's own export produces.");
  ...
  assert.strictEqual(await dpop.thumbprint(shuffled), RFC9449_JKT,
    "the members are ordered lexicographically by the specification, not by the order the " +
    "key object happens to hold them in.");

  // 3. A DIFFERENT key must give a different answer — the control that stops the
  //    two assertions above from passing on a constant.
  var other = Object.assign({}, RFC9449_JWK, { x: "l8tFrhx-34tV3hRICRDY9zCkDlpBhF42UQUfWVAWBFt" });
  ...
  log.info("[thumbprint] OK — extras and ordering ignored, a changed coordinate is not.");
  ...
  assert.throws(function () { dpop.canonicalJwk({ kty: "NOPE", x: "a" }); }, /kty NOPE/,
    "an unknown kty must be refused, not hashed.");
  assert.throws(function () { dpop.canonicalJwk({ kty: "EC", crv: "P-256", x: "a" }); }, /missing y/,
  ...
  log.info("[thumbprint] OK — an unusable key is refused, naming what is wrong.");
...
function htuDropsQueryAndNormalizes() {
  ...
  var cases = [
    ...
    ["https://issuer.example:443/oid4vci/credential",
     "https://issuer.example/oid4vci/credential", "a default https port is normalized away"],
    ...
    ["HTTPS://Issuer.EXAMPLE/oid4vci/credential",
     "https://issuer.example/oid4vci/credential", "scheme and host are case-insensitive"],
    ["https://issuer.example", "https://issuer.example/", "an empty path is /"]
  ];
  cases.forEach(function (c) {
    assert.strictEqual(dpop.htuFor(c[0]), c[1],
      "htu: " + c[2] + ". " + c[0] + " -> " + dpop.htuFor(c[0]) + ", expected " + c[1]);
  });
  log.info("[htu] OK — " + cases.length + " cases, query/fragment dropped and RFC 3986 " +
  ...
  assert.strictEqual(dpop.htuFor("https://a.example:443/x"), dpop.htuFor("https://a.example/x"),
    "the same endpoint written with and without its default port must produce ONE htu, or a " +
...
async function athIsTheHashOfTheToken() {
  ...
  var expected = crypto.createHash("sha256").update(token, "ascii").digest("base64url");
  assert.strictEqual(await dpop.athFor(token), expected,
    "ath must be base64url(SHA-256(ASCII(token))). This is the claim that stops a proof " +
...
function jtiIsUnpredictableAndLongEnough() {
  ...
  assert.ok(shortest >= 96,
    "RFC 9449 asks for at least 96 bits of pseudorandom data in jti so the server can detect " +
    "a replay; the shortest of 500 was " + shortest + " bits.");
  log.info("[jti] OK — 500 distinct values, at least " + shortest + " bits each.");
...
async function proofIsWellFormedAndVerifies() {
  ...
  for (const alg of dpop.ALGS) {
    ...
    assert.strictEqual(key.alg, alg, "generateKeyPair(" + alg + ") reported alg " + key.alg);
    assert.ok(key.publicJwk && !key.publicJwk.d,
      "the public half must carry no private material — it travels in every proof header.");
    ...
    ["d", "p", "q", "dp", "dq", "qi", "key_ops", "ext"].forEach(function (m) {
      assert.strictEqual(key.publicJwk[m], undefined,
        "the public JWK must not carry " + m + "; it would travel in every proof.");
    });
    ...
    assert.strictEqual(parts.length, 3, "a DPoP proof is a compact JWS with three parts.");
    var header = jsonFromB64u(parts[0]);
    var payload = jsonFromB64u(parts[1]);

    // Section 4.2's required header parameters, and section 4.3's checks on them.
    assert.strictEqual(header.typ, "dpop+jwt",
      "the proof must be explicitly typed dpop+jwt (RFC 8725 section 3.11). Without it a " +
      "receiver may accept some other JWT the client signed with the same key.");
    assert.strictEqual(header.alg, alg, "the header must name the signing algorithm.");
    assert.notStrictEqual(header.alg, "none", "alg must not be none.");
    assert.ok(header.jwk && header.jwk.kty, "the header must carry the public key as a JWK.");
    assert.strictEqual(header.jwk.d, undefined, "the header's jwk must not contain a private key.");
    assert.strictEqual(header.kid, undefined,
      "a kid in the header would be pointless here — the key itself is present — and would " +
    ...
    assert.strictEqual(payload.htm, "POST",
      "htm is the HTTP method, upper case as RFC 9110 writes it — a lower-case method fails " +
    ...
    assert.strictEqual(payload.ath, await dpop.athFor("an-access-token"),
      "ath must be present and correct when the proof accompanies an access token.");
    ...
    if (alg === "ES256") {
      ...
      ok = crypto.verify("sha256", signingInput, { key: publicKey, dsaEncoding: "ieee-p1363" },
                         signature);
      assert.strictEqual(signature.length, 64,
        "an ES256 JWS signature is the 64-byte r||s pair, not DER. Got " + signature.length +
        " bytes, which is what a DER signature would be.");
    } else {
      ok = crypto.verify("sha256", signingInput, publicKey, signature);
    }
    assert.ok(ok, alg + ": the proof's signature does not verify against the key in its own " +
                  "header, checked with node rather than with the module that made it.");
    ...
    assert.strictEqual(jkt, await dpop.thumbprint(key.publicJwk),
      "the thumbprint of the header's jwk must equal the thumbprint of the wallet's own key — " +
      "that equality is the whole binding.");

    // Tampering must break it. A JWS whose payload can be edited proves nothing.
    ...
    var stillOk = alg === "ES256"
      ? crypto.verify("sha256", tinput, { key: publicKey, dsaEncoding: "ieee-p1363" },
                      b64uDecode(tparts[2]))
    ...
    log.info("[proof] OK — " + alg + ": typed, all required claims, verifies under node, and " +
  ...
  assert.strictEqual(jsonFromB64u(atTokenEndpoint.proof.split(".")[1]).ath, undefined,
    "the proof for a Token Request carries no ath: there is no access token yet. RFC 9449 " +
    "requires ath only when the proof accompanies one.");
  assert.strictEqual(jsonFromB64u(atTokenEndpoint.proof.split(".")[1]).nonce, undefined,
    "and no nonce until the server has asked for one.");
  log.info("[proof] OK — a Token Request proof carries neither ath nor nonce.");

  // Two proofs for the same request must differ: a reusable proof is a replayable one.
  var again = await dpop.proof({ key: key2, htm: "POST", htu: "http://localhost:8081/oauth2/token" });
  assert.notStrictEqual(atTokenEndpoint.proof, again.proof,
    "two proofs for the same method and URI must differ — each carries its own jti, which is " +
...
function nonceHandshakeIsRecognised() {
  ...
  var fromAs = dpop.nonceRequested(400, { error: "use_dpop_nonce" },
                                   { "dpop-nonce": "eyJ7S_zG" });
  assert.strictEqual(fromAs.wanted, true, "a 400 use_dpop_nonce body must be recognised.");
  assert.strictEqual(fromAs.nonce, "eyJ7S_zG", "and the nonce must be read from the header.");
  ...
  assert.strictEqual(fromRs.wanted, true,
    "a 401 with use_dpop_nonce in WWW-Authenticate must be recognised — a resource server " +
  ...
  assert.strictEqual(dpop.nonceRequested(400, { error: "invalid_grant" }, {}).wanted, false,
    "an unrelated error must NOT look like a nonce request, or the wallet retries forever " +
  ...
  log.info("[nonce] OK — both request shapes recognised, unrelated errors are not.");

  // Header case: a Headers object is case-insensitive, a plain object is not,
  // and both reach this code.
  assert.strictEqual(dpop.nonceFromResponse({ headers: { "DPoP-Nonce": "x" } }), "x",
    "a plain header object spelled as the RFC spells it must be read.");
  assert.strictEqual(dpop.nonceFromResponse({
    headers: { get: function (n) { return n === "DPoP-Nonce" ? "y" : null; } } }), "y",
    "a fetch() Headers object must be read through get().");
  assert.strictEqual(dpop.nonceFromResponse(null), "", "and no response is no nonce.");
...
function tokenIsPresentedAsDpopNotBearer() {
  ...
  assert.strictEqual(dpop.authorizationHeader("abc"), "DPoP abc",
    "a DPoP-bound token is presented with the DPoP scheme. Presenting it as Bearer is a " +
    "protocol error even though the bytes are identical, and a resource server that accepts " +
    "either has thrown the binding away.");
  assert.ok(dpop.authorizationHeader("abc").indexOf("Bearer") === -1, "and never as Bearer.");
...
async function symmetricKeysAreRefused() {
  ...
  assert.strictEqual(dpop.algOfJwk({ kty: "EC", crv: "P-384", x: "a", y: "b" }), "",
    "a curve this wallet cannot sign with must map to no algorithm rather than to ES256, " +
    "which would sign with the wrong hash.");
  assert.strictEqual(dpop.algOfJwk({ kty: "EC", crv: "P-256", x: "a", y: "b" }), "ES256");
  assert.strictEqual(dpop.algOfJwk({ kty: "RSA", n: "a", e: "AQAB" }), "RS256");
  await assert.rejects(
    dpop.proof({ key: { alg: "HS256", publicJwk: { kty: "oct" }, privateJwk: {} },
...
async function test() {
  ...
  log.info("Starting Test run. Verifying client/src/dpop.js against RFC 9449 and RFC 7638.");
...
program
  .name("dpop")
  .description("Verify client/src/dpop.js: RFC 9449 proofs, RFC 7638 thumbprints, htu/ath/jti.")
