...
const NON_SPEC_PATHS = ["/oid4vci/last_request", "/oid4vci/notification/:id",
                        "/oid4vp/result/:state", "/sts-metadata",
                            "/healthcheck"];
...
async function theDocumentIsServed() {
  ...
  assert.ok(json.ok, "GET /sts-metadata?format=json should answer 200; got " +
            json.status);
  const doc = json.body;
  ...
  for (const e of doc.endpoints) {
    ...
    for (const method of e.methods) {
      ...
      assert.ok(!routeMissing,
        method + " " + path +
            " is listed on /sts-metadata but no route is registered for it — " +
  ...
  advertised.filter(Boolean).forEach(function (url) {
    ...
    assert.ok(listed.has(path),
      "the service advertises " + url +
          " in its own metadata, but /sts-metadata does not list " +
...
program
  .name("sts_metadata")
  .description("Verify GET /sts-metadata lists exactly the endpoints the STS " +
      "registers, and the specs it implements.")
