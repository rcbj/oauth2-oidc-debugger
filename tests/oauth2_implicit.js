...
async function verifyFirstPaneRowCollapsed(driver) {
  ...
  const panes = [["config_fieldset", "config_expand_button", "Configuration Parameters"],
                 ["tools_fieldset", "tools_expand_button", "Tools"],
                 ["token_fieldset", "token_expand_button", "the token request"]];
  for (const [fieldsetId, legendId, name] of panes) {
    await driver.wait(until.elementLocated(By.id(fieldsetId)), waitTime);
    const display = await driver.findElement(By.id(fieldsetId)).getCssValue("display");
    assert.strictEqual(display, "none",
      "The " + name + " pane should be collapsed by default once an implicit flow has " +
      "returned a token, but #" + fieldsetId + " has display: " + display + ".");
    await driver.wait(until.elementIsVisible(driver.findElement(By.id(legendId))), waitTime,
      "The " + name + " pane's title (#" + legendId + ") should still be visible, so the " +
      "collapsed pane can be expanded.");
  }
  log.info("Leaving verifyFirstPaneRowCollapsed().");
}

// The Authorization Endpoint Results pane. For an implicit flow this is the only
// place the token is ever shown — there is no token endpoint call — so it has to
// be a pane like every other one on the page, and the token has to carry the
// same links and buttons it would get had a code exchange returned it.
//
...
// it looked became the placeholder NO_..._PRESENTED_IN_EXPECTED_LOCATIONS in the
// token's own field, where it reads as something the identity provider said.
async function verifyAuthorizationEndpointResultsPane(driver, access_token) {
  ...
  const facts = await driver.executeScript(function () {
    var panes = Array.prototype.filter.call(
      ...
      function (pane) {
        ...
        return !!legend && legend.textContent.trim().indexOf("Authorization Endpoint Results") === 0;
      });
    ...
    var history = [];
    try { history = JSON.parse(window.localStorage.getItem("token_history") || "[]"); } catch (e) { history = []; }
    return {
      paneCount: panes.length,
      legendTarget: pane ? pane.querySelector(".dbg-legend").getAttribute("data-target") : null,
      fieldsetId: fieldset ? fieldset.id : null,
      text: pane ? pane.textContent : "",
      hrefs: pane ? Array.prototype.map.call(pane.querySelectorAll("a"), function (a) {
        return a.getAttribute("href");
      }) : [],
      revokeButtons: pane ? pane.querySelectorAll(".revoke_token_btn").length : 0,
      revokeType: revoke ? revoke.getAttribute("data-revoke-type") : null,
      revokeGeneration: revoke ? revoke.getAttribute("data-revoke-generation") : null,
  ...
  // History is named by generation, because the current-token slots stop meaning
  // this token the moment a hybrid flow exchanges its code — so what is asserted
  // is the property that matters either way: the token each control names is the
  // one displayed beside it.
  const tokenNamedBy = function (type, generation) {
    if (type === "access" || type === "token_access_token") return facts.currentAccessSlot;
  ...
  const linkTo = function (page) {
    const href = facts.hrefs.filter(function (h) { return h && h.indexOf(page + "?") === 0; })[0];
    if (!href) return null;
    const params = new URLSearchParams(href.substring(href.indexOf("?") + 1));
    return { href: href, type: params.get("type"), generation: params.get("generation") };
  };

  assert.strictEqual(facts.paneCount, 1,
    "There should be exactly one Authorization Endpoint Results pane; found " + facts.paneCount +
  ...
  assert.ok(facts.legendTarget && facts.fieldsetId === facts.legendTarget,
    "The pane's title should be a .dbg-legend whose data-target names its own fieldset, the way every " +
    "other pane on the page is built. data-target=" + facts.legendTarget + ", fieldset id=" + facts.fieldsetId + ".");
  ...
  ["/token_detail.html", "/introspection.html", "/userinfo.html"].forEach(function (page) {
    const link = linkTo(page);
    assert.ok(link, "The access token needs its " + page + " link. Links found: " + JSON.stringify(facts.hrefs));
    assert.strictEqual(tokenNamedBy(link.type, link.generation), access_token,
      "The " + page + " link should name the token displayed beside it. It names type=" + link.type +
  ...
  assert.strictEqual(tokenNamedBy(facts.revokeType, facts.revokeGeneration), access_token,
    "Revoke Token should revoke the token displayed beside it, not whichever token is current. " +
    "It names data-revoke-type=" + facts.revokeType + ", data-revoke-generation=" + facts.revokeGeneration + ".");
  ...
  assert.ok(facts.copyOnClick && facts.copyOnClick.indexOf("#authz_access_token") >= 0,
    "The Copy Token button should carry an onclick naming this pane's own field. onclick=" + facts.copyOnClick);
  assert.ok(facts.text.indexOf("PRESENTED_IN_EXPECTED_LOCATIONS") < 0,
    "The pane should not contain a NO_..._PRESENTED_IN_EXPECTED_LOCATIONS placeholder.");
  assert.ok(!facts.idFieldPresent,
    "OAuth2 Implicit Grant asks for no id_token, so the pane should not draw an ID Token row.");
  ...
  const collapsed = await driver.executeScript(
    "dbgSetAllPanes(false); return document.getElementById(arguments[0]).style.display;", facts.fieldsetId);
  assert.strictEqual(collapsed, "none",
    "Collapse all panes should collapse the Authorization Endpoint Results pane; its fieldset (" +
    facts.fieldsetId + ") is not in the switch's list.");
  const expanded = await driver.executeScript(
    "dbgSetAllPanes(true); return document.getElementById(arguments[0]).style.display;", facts.fieldsetId);
  assert.strictEqual(expanded, "block",
    "Expand all panes should expand the Authorization Endpoint Results pane again.");
...
async function verifyTokenRecordedInHistories(driver, access_token) {
  ...
  const recorded = await driver.executeScript(function () {
    ...
    return {
      ...
      operations: parse("operation_history").map(function (o) { return o.operation; }),
      tokenPanelVisible: !!panel && window.getComputedStyle(panel).display !== "none"
    };
  });
  ...
  assert.strictEqual(fromAuthorization.length, 1,
    "The authorization response's token set should be recorded in Token History exactly once. " +
    "Sources found: " + JSON.stringify(recorded.tokenHistory.map(function (e) { return e.source; })));
  ...
  assert.ok(recorded.operations.indexOf("Authorization Endpoint") >= 0,
    "Operation History should record the Authorization Endpoint call. Operations found: " +
...
async function test() {
  ...
  try {
    ...
    // every pane expanded — including the row the check above requires collapsed.
