...
// every token would pass every positive check here, so the last section signs in
// WITHOUT the second factor and requires the tokens to say `["pwd"]` and
...
//
// The redirect_uri deliberately points at a path the STS does not serve. Nothing
// needs to receive the code — this test reads it off the URL — and pointing at a
// port nothing is listening on behaves differently in a container than on a
// host, which is the sort of difference that costs a run to diagnose.
// ---------------------------------------------------------------------------
...
var baseUrl = "http://localhost:3000";   // unused by this test; accepted for uniformity
var headless = true;
...
const MFA_USER = "mfauser-" + process.pid.toString(36) + "-" + Date.now().toString(36);
...
async function exchange(code) {
  ...
  assert.ok(body.id_token, "the token response carried no id_token: " + JSON.stringify(body));
  return body;
}
...
async function test() {
  ...
  // stack is http://sts:8081 — not a secure context, where navigator.credentials
  // does not exist at all. See tests/browser_flags.js.
  ...
  try {
    ...
    await section("acr_values=mfa forces the second factor and disables the opt-out", async () => {
      await driver.get(authorizeUrl("&acr_values=mfa"));
      await driver.wait(until.elementLocated(By.id("use_webauthn")), waitTime * 4);
      const box = await driver.findElement(By.id("use_webauthn"));
      assert.ok(await box.isSelected(),
        "a relying party that asked for step-up should get the WebAuthn step selected");
      assert.ok(!(await box.isEnabled()),
        "and it must not be possible to opt out of a step-up the relying party demanded — " +
        "otherwise acr_values is advisory and the acr in the token means nothing");
      return "checked and disabled";
    });

    await section("the first sign-in enrols a key and completes the flow", async () => {
      ...
      assert.ok(/Enrol/i.test(heading),
        "with no key enrolled the step should register one; the heading read: " + heading);
    ...
    await section("the ID token records BOTH factors", async () => {
      ...
      assert.deepStrictEqual(claims.amr, ["pwd", "hwk"],
        "amr must name both factors; RFC 8176's hwk is proof of possession of a hardware key, " +
        "which is what a WebAuthn assertion demonstrates. Got " + JSON.stringify(claims.amr));
      assert.strictEqual(claims.acr, "mfa", "and the acr should be stepped up; got " + claims.acr);
      assert.ok(claims.auth_time, "auth_time should be present");
      return "amr=" + JSON.stringify(claims.amr) + " acr=" + claims.acr;
    });

    await section("a second sign-in asserts with the enrolled key rather than enrolling again",
      async () => {
        await driver.get(STS + "/oauth2/logout");
        await signIn(driver, MFA_USER, authorizeUrl("&acr_values=mfa"));
        await driver.wait(until.elementLocated(By.id("wa-go")), waitTime * 4);
        ...
        assert.ok(/Use your security key/i.test(heading),
          "the key enrolled earlier should now be asserted with, not replaced; heading: " + heading);
        await driver.findElement(By.id("wa-go")).click();
        const tokens = await exchange(await codeFromRedirect(driver));
    ...
    await section("without the second factor the tokens say so", async () => {
      ...
      assert.deepStrictEqual(claims.amr, ["pwd"],
        "a password-only sign-in must not claim a hardware key; got " + JSON.stringify(claims.amr));
    ...
    // PURPOSE, because this test is the relying party and reads the code off the
    // URL. Anything else still fails — a filter broad enough to swallow a real
    // error would make this assertion decorative.
    ...
    const severe = all.filter((e) => !EXPECTED_404.some((p) => p.test(e.message)));
    ...
    log.info("PASS  no console errors across the ceremony pages (" +
             (all.length - severe.length) + " expected 404(s) filtered by name)");
...
program
  ...
  .description("WebAuthn as the second factor of an OIDC sign-in against the mock STS.")
  .addOption(new Option("-u, --url <url>", "The debugger base URL; unused here."))
