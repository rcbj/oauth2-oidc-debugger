...
//
//   1. walt.id must OFFER jwt_vc_json. Until its container is restarted onto
//      the configuration in waltid/config/, it offers only dc+sd-jwt.
//
//   2. The credential must be presentable BY THIS WALLET. walt.id's own
//      jwt_vc_json profiles bind the holder with a subject DID
//      (mapping.credentialSubject.id = "<subjectDid>"), not with the cnf.jwk
//      our mock uses. A VP JWT has to be signed by the key the credential
//      names, and for a DID-bound credential this wallet has no such key — it
//      never held one, because the proof of possession it made at issuance
//      offered a JWK. That is a real interoperability gap, not a defect in
//      either implementation, and it is reported as such: the run says what
//      walt.id did and what would have to be built to answer it, rather than
//      failing on a difference nobody has agreed to yet.
//
// When walt.id does bind with cnf.jwk, the full presentation runs and is
// checked by walt.id's own policies.
// ---------------------------------------------------------------------------
...
// /.well-known/openid-credential-issuer/openid4vci, because RFC 8414 INSERTS
// the well-known segment before the path rather than appending it. Passing the
// bare base looks for the document one path short and finds a 404, which reads
// as "walt.id offers no jwt_vc_json" when in fact nothing was ever asked.
...
// what the pre-authorized offer is created from, and therefore how a real
// access token is obtained without driving a browser through Keycloak.
var WALTID_PROFILE_ID = process.env.WALTID_JWT_VC_PROFILE_ID ||
...
function dcqlQuery(types) {
  ...
  return {
    credentials: [{
      ...
      claims: REQUESTED.map(function (name) { return { path: [
...
async function createVerificationSession(types) {
  ...
  const created = await common.httpJson(verifierBase +
...
async function test() {
  ...
  assert.ok(verifierBase, "WALTID_VERIFIER_URL is not set, so walt.id's " +
            "verifier was never asked to " +
    "verify a jwt_vc_json presentation.");
  assert.ok(issuerBase, "WALTID_ISSUER_URL is not set; this job issues the " +
            "credential it presents from " +
    "walt.id so that neither end of the exchange is ours.");
  log.info("Running jwt_vc_json presentation against walt.id. issuer=" +
           issuerBase +
           ", verifier=" + verifierBase);

  const live = await common.httpJson(verifierBase +
      "/livez").catch(function () { return { ok: false }; });
  assert.ok(live.ok, "walt.id's verifier is not answering at " + verifierBase +
            "/livez.");

  const found = await common.jwtVcJsonConfigurationId(issuerBase);
  assert.ok(found.id,
    "walt.id offers no jwt_vc_json credential configuration, so this " +
        "interoperability check did not run. " +
    "Offered: " + Object.keys(((found.meta ||
        {}).credential_configurations_supported) || {}).join(", ") +
    ". The configuration is in waltid/config/credential-issuer-metadata.conf " +
        "with a matching profile in " +
    "issuer2-profiles.conf — restart waltid-issuer-api to load it.");

  const accessToken = await common.preAuthorizedAccessToken(issuerBase,
      WALTID_PROFILE_ID);
  assert.ok(accessToken,
    "could not obtain an access token from walt.id with the pre-authorized " +
        "code grant for profile \"" +
    WALTID_PROFILE_ID + "\".");
  // "did": walt.id resolves the subject DID from the proof for this format.
  const held = await common.mintJwtVcJson(issuerBase, found.id, accessToken,
      "did");
  const parsed = common.assertIsJwtVcJson(held.credential, "walt.id");
  log.info("[issuance] walt.id issued a " + parsed.types.join("/") +
           " to present.");
  ...
  //
  // Two ways a credential can name its holder, and the earlier assumption that
  // a subject DID meant "not us" was wrong. walt.id binds jwt_vc_json with
  // credentialSubject.id = "<subjectDid>", and it derives that DID from the
  // proof of possession this wallet made — a did:jwk, which is the wallet's own
  // public key encoded into the identifier. So the wallet does hold the private
  // half and CAN sign for it; what it must do differently is present as that
  // DID rather than as an anonymous key.
  //
  ...
  if (binding.kind === "cnf.jwk") {
    ...
    assert.strictEqual(binding.subjectId, held.did,
      "walt.id bound this credential to " + binding.subjectId +
          ", which is not the did:jwk this wallet " +
      "proved possession of (" + held.did +
          "). A Verifiable Presentation JWT has to be signed by the key " +
      "the credential names, and this wallet does not hold that one.");
    presentingAs = binding.subjectId;
    log.info("[binding] walt.id bound it by subject DID, and it is this " +
             "wallet's own did:jwk — so the " +
             "presentation is signed with that key and presented as that DID.");
  } else {
    assert.fail("the credential names no holder at all (" + binding.kind +
                "), so nobody can present it.");
  }
  ...
  assert.ok(created.status === 200 || created.status === 201,
    "walt.id would not create a jwt_vc_json verification session (HTTP " +
        created.status + "): " +
    String(created.raw).slice(0, 300) +
           ". Its DCQL support for this format differs from its SD-JWT path.");
  assert.ok(created.body && created.body.sessionId,
    "walt.id should return a sessionId. Got: " + String(created.raw).slice(0,
        300));

  const url = created.body.fullAuthorizationRequestUrl ||
  ...
  const payload = common.b64u(JSON.stringify({
    ...
    vp: { "@context": ["https://www.w3.org/2018/credentials/v1"],
          type: ["VerifiablePresentation"],
                 verifiableCredential: [held.credential] }
  }));
  ...
  log.info("[presentation] walt.id answered HTTP " + posted.status +
           " to the VP JWT.");

  const info = await common.httpJson(verifierBase + "/verification-session/" +
    encodeURIComponent(created.body.sessionId) + "/info");
  assert.ok(info.ok, "walt.id should report the session outcome. Got HTTP " +
            info.status);
  log.info("[presentation] walt.id's verdict: " + String(info.raw).slice(0,
           400));
  assert.ok(posted.status >= 200 && posted.status < 400,
    "walt.id should accept a correctly built VP JWT over a credential it " +
        "issued itself. Got HTTP " +
