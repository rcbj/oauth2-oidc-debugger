...
async function waitForText(driver, id, pattern, timeoutMs, what) {
  ...
  // succeeds during parsing and says nothing about whether the exchange happened.
  let last = "";
  try {
    ...
    throw new Error(what + " (last text in #" + id + ": " + JSON.stringify(last.slice(0, 300)) + ")");
  }
  return last;
}
...
// What has to be true before any of this means anything. Each answer is a different
// reason to skip, and naming which one matters — "the stack is not up" and "the api
// has the service relay switched off" send you to different places.
async function preconditions() {
  try {
    const principals = await fetch(stsUrl + "/krb5/principals");
    if (!principals.ok) {
      return { ok: false, why: stsUrl + "/krb5/principals answered " + principals.status +
        " — the mock STS may be an older build without the KDC" };
    }
    const body = await principals.json();
    if (body.realm !== realm) {
      return { ok: false, why: "the mock KDC serves realm " + body.realm + ", not " + realm };
    }
    if ((body.implemented || []).indexOf("TGS exchange") === -1) {
      return { ok: false, why: "the mock KDC does not implement the TGS exchange (it reports: " +
        (body.implemented || []).join(", ") + ") — the sts/ gitlink may predate it" };
    }
    const service = await fetch(stsUrl + "/krb5/service");
    if (!service.ok) {
      return { ok: false, why: "the mock STS has no ticket-protected service (" +
    ...
    if (!l.serviceEnabled) {
      return { ok: false, why: "the api has POST /krb5/service DISABLED (krb5ServicePorts is " +
        JSON.stringify(l.servicePorts) + "). That endpoint is off by default because a Kerberos " +
        "service can be on any port, so it is a broader capability than the KDC relay — set " +
        "krb5ServicePorts to " + servicePort + " to run this test" };
    }
    return { ok: true, kdcPort: String(body.kdcPort), servicePort: String((await service.json()).port) };
  } catch (e) {
...
async function getATgtOnTheAsPage(driver) {
  ...
  await waitForText(driver, "krb_as_status", /PREAUTH_REQUIRED|issued a ticket WITHOUT/, 60000,
  ...
  const status = await waitForText(driver, "krb_as_status", /A TGT for|will not decrypt|refused/, 60000,
    "the AS page produced no ticket");
  assert.ok(/A TGT for/.test(status), "a TGT is needed before this test can start: " + status);

  // The handoff itself: the TGT must be where the next page will look.
  const held = await driver.executeScript("return !!sessionStorage.getItem('krb_ccache');");
  assert.strictEqual(held, true,
    "the AS page must leave the TGT in sessionStorage for the TGS page to find — that handoff is " +
...
async function theTgsPageRefusesWithNoTgt(driver) {
  ...
  // Clear the cache and reload: the page must refuse up front rather than when the
  // button is pressed. A user with no TGT should not be able to send a request that
  // cannot possibly be answered.
  await driver.executeScript("sessionStorage.clear(); localStorage.removeItem('krb_ccache');");
  await driver.navigate().refresh();
  await driver.wait(until.elementLocated(By.id("krb_tgs_button")), 20000);
  const disabled = await driver.findElement(By.id("krb_tgs_button")).getAttribute("disabled");
  assert.ok(disabled, "with no TGT held the TGS button must be DISABLED, not merely fail when pressed");
  // NOTE on what that assertion is worth on its own: the button is `disabled` in the
  // MARKUP, so it is satisfied even if renderHeldTgt() does nothing at all —
  // mutation-testing showed exactly that. The assertion that carries the weight is the
  // opposite direction, in theTgsPageSpendsTheTgt: the button must become ENABLED once
  // a TGT is held, which only the logic can do.
...
async function theTgsPageSpendsTheTgt(driver) {
  ...
  assert.ok(/krbtgt/.test(tgtPane),
    "the TGS page must show the held TGT, and it is a ticket for krbtgt: " + tgtPane.slice(0, 200));
  ...
  assert.ok(!enabled,
    "with a valid TGT held the TGS button must be ENABLED. The markup ships it disabled, so this " +
    "is the assertion that proves the page read the cache rather than doing nothing.");
  ...
  // Without a subkey the reply is at key usage 8. Reported, not hidden: a client that
  // always tries one of the two fails whenever the other applies.
  assert.ok(/key usage 8/.test(status),
    "with no subkey sent, the reply must be reported as opened at key usage 8: " + status);

  const tickets = await driver.findElement(By.id("krb_tickets_pane")).getText();
  assert.ok(tickets.indexOf(spn) !== -1, "the service ticket must be listed: " + tickets.slice(0, 200));
  // Only the AS exchange issues an `initial` ticket, and a service may insist on that
  // distinction — so a TGS-issued ticket carrying it would be a lie.
  // getText() on a two-column table puts the NAME and the VALUE on separate lines, so
  // the line matching /^flags/ is the label and the flags themselves are the line after
  // it. Matching the label and asserting against it compares a heading with a flag
  // list, which fails for a reason that has nothing to do with the ticket.
  const lines = tickets.split("\n").map(function (l) { return l.trim(); });
  const flagsAt = lines.indexOf("flags");
  assert.ok(flagsAt !== -1 && lines[flagsAt + 1],
    "the ticket pane must have a flags row with a value under it: " + tickets.slice(0, 300));
  ...
  await driver.findElement(By.id("krb_tgs_button")).click();
  const withSubkey = await waitForText(driver, "krb_tgs_status", /key usage 9|key usage 8|—/, 60000,
    "the TGS exchange with a subkey produced no result");
  assert.ok(/key usage 9/.test(withSubkey),
    "with a subkey in the Authenticator the reply must come back at key usage 9, and the page must " +
    "say so: " + withSubkey);

  // And the handoff to the AP page.
  const stored = await driver.executeScript("return !!sessionStorage.getItem('krb_service_tickets');");
  assert.strictEqual(stored, true, "the service ticket must be left where the AP page will look for it");
...
async function theApPagePresentsItAndChecksTheEcho(driver) {
  ...
  const environment = await waitForText(driver, "krb_environment_note", /relay|did not answer/, 20000,
    "the AP page never reported what the api allows");
  assert.ok(/presenting a ticket to a service is enabled/.test(environment),
    "the AP page must confirm the service relay is enabled before offering the button: " +
  ...
  assert.ok(ticketPane.indexOf(spn) !== -1,
    "the ticket the TGS page bought must be offered here: " + ticketPane.slice(0, 200));
  ...
  const status = await waitForText(driver, "krb_ap_status",
    /ACCEPTED|FAILED|REPEAT|BADKEYVER|SKEW|—/, 90000, "the AP exchange produced no result");
  assert.ok(/ACCEPTED/.test(status), "the service should have accepted the ticket: " + status);
  assert.ok(/proved itself/.test(status),
    "and the page must say the service proved itself, not merely that it accepted: " + status);
  ...
  assert.ok(/0x60/.test(request) && /1\.2\.840\.113554\.1\.2\.2/.test(request),
    "the page must show the InitialContextToken wrapper and the Kerberos mechanism OID: " +
    request.slice(0, 250));
  assert.ok(/01 00/.test(request), "and the AP-REQ token id");

  // The 0x8003 structure, field by field. This is the pane the AP page exists for.
  const checksum = await driver.findElement(By.id("krb_checksum_pane")).getText();
  assert.ok(/Lgth/.test(checksum), "the 0x8003 checksum must be decoded, not shown as hex: " +
    checksum.slice(0, 200));
  assert.ok(/little-endian/i.test(checksum),
    "and the pane must say its integers are little-endian, which is why it is the field that most " +
    "often goes wrong");
  assert.ok(/sixteen ZERO bytes|no channel bindings/i.test(checksum),
    "Bnd with no channel bindings is sixteen zero bytes rather than absent, and the pane should say " +
    "so: " + checksum.slice(0, 300));
  assert.ok(/MUTUAL/.test(checksum), "MUTUAL must appear in the flags, since it was requested");
  ...
  assert.ok(/CONFIRMED/.test(context), "mutual authentication must be reported as confirmed: " +
    context.slice(0, 200));
  assert.ok(/echoed/.test(context) && /session key/.test(context),
    "and the page must explain that the echo IS the proof: " + context.slice(0, 300));
  assert.ok(/acceptor's subkey/.test(context),
    "once the acceptor offers a subkey, per-message tokens must be keyed from IT rather than from " +
...
async function perMessageTokensWorkAndRejectTampering(driver) {
  ...
  const mic = await waitForText(driver, "krb_permessage_pane", /GetMIC|token id/, 40000,
    "GetMIC produced nothing");
  assert.ok(/04 04/.test(mic), "a MIC token's id is 04 04: " + mic.slice(0, 200));
  assert.ok(/does not verify/.test(mic),
    "the pane must show that the same MIC does NOT verify over a modified message — a MIC that " +
    "verified anything would be worse than none: " + mic.slice(0, 300));
  assert.ok(/initiator/.test(mic),
    "and that the sender's role is in the token, since the verifier must use the SENDER's key usage");

  await driver.findElement(By.id("krb_wrap_button")).click();
  const wrap = await waitForText(driver, "krb_permessage_pane", /GSS_Wrap/, 40000, "Wrap produced nothing");
  assert.ok(/05 04/.test(wrap), "a Wrap token's id is 05 04: " + wrap.slice(0, 200));
  assert.ok(/the quick brown fox/.test(wrap), "and what was wrapped must come back out");
  assert.ok(/appears\s+twice|twice/.test(wrap),
    "the pane should explain that the header appears twice — once clear, once encrypted — which is " +
    "what stops the clear copy being altered: " + wrap.slice(0, 300));
  log.debug("Leaving perMessageTokensWorkAndRejectTampering().");
}

// Without MUTUAL the service need not answer at all. The page must say what that
// means rather than presenting silence as success — this is the difference between
...
async function withoutMutualNothingProvesTheService(driver) {
  ...
  await driver.findElement(By.id("krb_present_button")).click();
  const status = await waitForText(driver, "krb_ap_status",
    /accepted|ACCEPTED|nothing|—/, 90000, "the one-way exchange produced no result");
  assert.ok(/NOTHING has proved|nothing has proved/i.test(status),
    "with MUTUAL unticked the page must say that nothing has proved the service's identity, rather " +
...
async function test() {
  ...
  if (!ready.ok) {
    // Named, never silent. A skip that did not say which precondition failed would be
    // indistinguishable from a pass.
    log.warn("SKIPPED: " + ready.why + ". This test needs the client, the api and the mock STS " +
      "(KDC and ticket-protected service), and the api's krb5ServicePorts set.");
    log.info("Test completed successfully.");
    return;
  }
  if (ready.kdcPort && ready.kdcPort !== String(kdcPort)) {
    log.warn("the mock STS reports its KDC on port " + ready.kdcPort + "; using that.");
    kdcPort = ready.kdcPort;
  }
  if (ready.servicePort && ready.servicePort !== String(servicePort)) {
    log.warn("the mock STS reports its protected service on port " + ready.servicePort + "; using that.");
...
program
  ...
  .description("Verify the TGS and AP exchange pages, and the credential handoff between all three.")
