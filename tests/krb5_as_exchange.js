...
// The mock KDC lives in the sts/ SUBMODULE. In the tests image it is copied flat
// (see tests/Dockerfile); in a checkout it is under sts/. A missing submodule is
// an empty directory, which is why this says so rather than failing with a bare
// MODULE_NOT_FOUND — the same trap tests/Dockerfile's other sts/ COPY warns about.
...
async function startKdc() {
  ...
  if (!modulePath) {
    throw new Error("could not find the mock KDC (sts/krb5_kdc.js). The sts/ directory is a " +
      "SUBMODULE — run `git submodule update --init sts`, and note that an uninitialised " +
  ...
  // reason. Say what to do about it: the mock KDC requires app.js and helpers.js
  // (requiring a module registers its routes there), and helpers.js requires
  // bbs2023.js, so the tests image needs all of them under sts/.
  let kdcModule;
  try {
    kdcModule = require(modulePath);
  } catch (e) {
    if (/Cannot find module/.test(e.message)) {
      throw new Error("the mock KDC loaded but one of its own dependencies did not: " + e.message +
        ". In the tests image the sts/ modules are copied individually, so a new require inside " +
        "the submodule needs a matching COPY in tests/Dockerfile — the set there is the transitive " +
        "closure of what krb5_kdc.js and krb5_service.js require, and it has to be recomputed when " +
        "that changes.");
    }
    throw e;
  }
  ...
  // both transports — a client failing over from UDP to TCP sends the retry to the
  // same place. Awaiting whenReady is what makes the UDP case below meaningful
  // rather than a timeout against a listener on a port nobody knows.
  const listeners = kdcModule.listen(0);
  await listeners.whenReady;
  kdcPort = listeners.port;
  assert.strictEqual(listeners.tcp.address().port, kdcPort, "TCP must be on the reported port");
  assert.strictEqual(listeners.udp.address().port, kdcPort,
    "and UDP must be on the SAME port — a KDC presents one port number for both transports");
...
function stopKdc() {
  if (!kdc) return;
  try { kdc.listeners.tcp.close(); } catch (e) { /* already closed */ }
  try { kdc.listeners.udp.close(); } catch (e) { /* already closed */ }
  kdc = null;
}
...
function buildAsReq(fields) {
  ...
  return msgs.encKdcReq({
    ...
    padata: f.padata || [],
    reqBody: {
      kdcOptions: f.kdcOptions || [msgs.KDC_OPTION.FORWARDABLE, msgs.KDC_OPTION.RENEWABLE],
      cname: f.cname || { type: msgs.NAME_TYPE.PRINCIPAL, name: ["alice"] },
      realm: f.realm || "EXAMPLE.COM",
      sname: f.sname || { type: msgs.NAME_TYPE.SRV_INST, name: ["krbtgt", "EXAMPLE.COM"] },
...
async function encTimestampPadata(password, info) {
  ...
  const key = await profile.stringToKey(password, prim.utf8(info.salt || ""), info.s2kparams);
...
async function theTwoMessageDanceWorksAndCarriesTheSalt() {
  ...
  // 1. A bare AS-REQ. A real KDC does not treat this as an error to be logged and
  // forgotten — it answers with the information needed to try properly.
  const first = await exchange(buildAsReq());
  assert.strictEqual(first.decoded.kind, "KRB-ERROR",
    "a bare AS-REQ must be answered with KDC_ERR_PREAUTH_REQUIRED, not with a ticket");
  const err = first.decoded.error;
  assert.strictEqual(err.errorCode, 25, "the error code must be KDC_ERR_PREAUTH_REQUIRED");
  assert.ok(err.eDataPaData, "e-data must carry PA-DATA — without it this error is useless");

  // The salt. This is the whole reason the round trip exists: it is not derivable
  // from the principal name, so a client can only learn it here.
  ...
  assert.ok(entries.length >= 2, "the KDC should offer more than one etype: " + entries.length);
  ...
  assert.strictEqual(aes256.salt, "EXAMPLE.COMalice",
    "the salt must be Active Directory's shape for a user — realm + sAMAccountName, no separator");
  assert.ok(aes256.s2kparams && prim.toBytes(aes256.s2kparams).length === 4,
    "s2kparams must carry the iteration count");
  const iterations = prim.toBytes(aes256.s2kparams).reduce(function (a, b) { return a * 256 + b; }, 0);
  ...
  assert.strictEqual(rc4.salt, null, "arcfour's ETYPE-INFO2 entry must have no salt");

  // AD also sends the older PA-PW-SALT alongside, for clients that predate
  // ETYPE-INFO2. Its presence is worth asserting because a client must prefer the
  // newer one.
  const pwSalt = err.eDataPaData.filter(function (pa) { return pa.type === 3; })[0];
  assert.ok(pwSalt, "PA-PW-SALT should be sent alongside, as Active Directory does");
  assert.strictEqual(asn1.decLatin1(prim.toBytes(pwSalt.value)), "EXAMPLE.COMalice",
  ...
  const second = await exchange(buildAsReq({ padata: [preauth.padata], nonce: 0xcafe1234 }));
  ...
  assert.deepStrictEqual(rep.cname.name, ["alice"], "the client named in the reply");
  assert.strictEqual(rep.ticket.encPart.etype, 18,
    "the KDC must pick the client's FIRST acceptable etype, which was aes256");
  assert.strictEqual(rep.ticket.encPart.kvno, 3, "the ticket should name the service's key version");

  // The client opens its half with its own long-term key.
  const part = msgs.readEncKdcRepPart(
    await preauth.profile.decrypt(preauth.key, kcrypto.KEY_USAGE.AS_REP_ENCPART, rep.encPart.cipher));
  assert.strictEqual(part.nonce, 0xcafe1234,
    "THE NONCE MUST COME BACK UNCHANGED. It is the client's only defence against a replayed " +
  ...
  assert.ok(part.endtime > part.authtime, "the ticket must be valid for a positive interval");
  ...
  assert.ok(flags.indexOf("initial") !== -1,
    "a ticket from the AS exchange must be flagged initial: " + flags.join(", "));
  assert.ok(flags.indexOf("pre-authent") !== -1,
    "and pre-authent, because pre-authentication actually happened: " + flags.join(", "));
...
// same session key the client got. That equality IS the mechanism, and it is the
// one thing neither end can verify alone.
async function theTicketAndTheClientAgreeOnTheSessionKey(context) {
  ...
  // The krbtgt key, derived the way the KDC derives it. Its password is the mock's
  // own default; a test knowing it is what stands in for having the keytab.
  ...
  const ticketPart = msgs.readEncTicketPart(
    await preauth.profile.decrypt(krbtgtKey, kcrypto.KEY_USAGE.KDC_REP_TICKET, rep.ticket.encPart.cipher));

  assert.strictEqual(prim.toHex(ticketPart.key.key), prim.toHex(clientPart.key.key),
    "the session key in the TICKET and the one in the client's enc-part must be the SAME BYTES. " +
    "They are encrypted under different keys for different readers, and their equality is the " +
    "whole mechanism — neither end can check it alone.");
  assert.deepStrictEqual(ticketPart.cname.name, ["alice"], "the ticket names the client");
...
async function anAccountWithoutPreAuthGetsATicketStraightAway() {
  ...
  assert.strictEqual(reply.decoded.kind, "AS-REP",
    "an account with pre-authentication disabled must be answered with a ticket, not an error");
  const rep = reply.decoded.rep;
  const profile = kcrypto.etypeById(rep.encPart.etype);
  const key = await profile.stringToKey("no-preauth-here", prim.utf8("EXAMPLE.COMnoreauth"), null);
  const part = msgs.readEncKdcRepPart(
    await profile.decrypt(key, kcrypto.KEY_USAGE.AS_REP_ENCPART, rep.encPart.cipher));
  assert.strictEqual(part.nonce, 0x5150, "the nonce comes back unchanged here too");
  const flags = msgs.ticketFlagNames(part.flags);
  assert.ok(flags.indexOf("initial") !== -1, "still an initial ticket");
  assert.strictEqual(flags.indexOf("pre-authent"), -1,
    "but NOT pre-authent: no pre-authentication happened, and a service can insist on that flag, " +
    "so setting it would be a lie with security consequences. Flags: " + flags.join(", "));
...
async function theKdcRefusesInItsOwnVocabulary(context) {
  ...
  async function expectError(label, request, code, textMatch) {
    ...
    assert.strictEqual(e.errorCode, code,
      label + ": expected " + msgs.describeError(code).name + " (" + code + "), got " +
    ...
    // Every refusal must carry the KDC's own clock, because that is how a client
    // measures skew and skew is one of the commonest causes of failure.
  ...
  await expectError("an unknown principal",
    buildAsReq({ cname: { type: 1, name: ["nobody"] } }), 6, /no such principal/);

  await expectError("an unknown service",
    buildAsReq({ sname: { type: 2, name: ["krbtgt", "OTHER.REALM"] } }), 7, /no such service/);

  await expectError("the wrong realm",
    buildAsReq({ realm: "OTHER.REALM", sname: { type: 2, name: ["krbtgt", "OTHER.REALM"] } }),
    68, /serves EXAMPLE.COM/);

  await expectError("a locked account",
    buildAsReq({ cname: { type: 1, name: ["locked"] } }), 18, /disabled or locked/);
  ...
  // what a hardened Windows Server 2025 domain controller does to a legacy client,
  // and the error reads as "the KDC is broken" unless you know what it means.
  const noSupp = await expectError("an RC4-only client against an AES-only account",
    buildAsReq({ cname: { type: 1, name: ["aesonly"] }, etypes: [23] }), 14,
    /no common encryption type/);
  assert.ok(/aes256|aes128/.test(noSupp.eText || ""),
    "the refusal should say what the principal DOES support, which is the whole diagnosis: " +
    noSupp.eText);
  ...
  // indistinguishable to it from a wrong salt or a wrong key usage number, which
  // is exactly why showing the salt matters.
  const wrongPassword = await encTimestampPadata("not the password", context.info);
  ...
  assert.ok(failed.eDataPaData,
    "PREAUTH_FAILED must re-send ETYPE-INFO2: the client may have used the wrong SALT rather than " +
  ...
  // different salt. Indistinguishable from a wrong password at the KDC, which is
  ...
  const key = await profile.stringToKey("hunter2", prim.utf8("EXAMPLE.COMalice"), null);
  ...
  const skewed = await expectError("a timestamp twenty minutes old",
    buildAsReq({ padata: [{
      ...
      value: msgs.encEncryptedData({
        etype: 18,
        cipher: await profile.encrypt(key, kcrypto.KEY_USAGE.AS_REQ_PA_ENC_TIMESTAMP, stale)
      })
    }] }), 37, /clock skew/);
  ...
  const apRep = msgs.encApRep({ encPart: { etype: 18, cipher: kcrypto.randomBytes(32) } });
  ...
  // Now past the relay, straight into the KDC, which is why the module is loaded
  // in-process rather than only reached over a socket.
  ...
  assert.strictEqual(direct.kind, "KRB-ERROR",
    "an AP-REP reaching the KDC must be answered with an error, not with silence");
  ...
  // dropped connection. A bare DER SEQUENCE has no [APPLICATION n] tag, so the KDC
  // cannot even guess what it is.
  const garbage = msgs.readKdcResponse(
    await kdc.module.handleMessage(Buffer.from([0x30, 0x03, 0x02, 0x01, 0x05])));
  assert.strictEqual(garbage.kind, "KRB-ERROR", "garbage must still be answered");
  assert.strictEqual(garbage.error.errorCode, 60, "KRB_ERR_GENERIC");
  assert.ok(/could not decode/.test(garbage.error.eText || ""),
    "and e-text must say what the KDC objected to, which is where a KDC explains itself: " +
    garbage.error.eText);
  ...
  // rather than KRB_ERR_GENERIC. It is a reminder that the application tag is one
  // byte and text can wear it by coincidence, which is exactly why the relay's
  // pre-flight also checks that the declared length matches.
  ...
  assert.strictEqual(textish.error.errorCode, 40,
    "text beginning with 0x6e is read as an AP-REQ, so the answer is KRB_AP_ERR_MSG_TYPE");

  // A TGS-REQ whose PA-TGS-REQ is nonsense. The TGS exchange itself is covered by
  // tests/krb5_tgs_ap.js — this case is only here to check that the KDC answers a
  // malformed one with a reason rather than failing opaquely or dropping it.
  //
  // (This assertion used to require the text "does not implement the TGS exchange
  // yet", and it broke the moment phase 3 implemented it — which is the right kind
  // of failure: a test asserting the absence of a feature must be revisited when the
  // feature arrives, not deleted quietly.)
  const tgs = await exchange(msgs.encKdcReq({
    ...
    reqBody: { kdcOptions: [], realm: "EXAMPLE.COM",
               sname: { type: 3, name: ["HTTP", "web.example.com"] },
               till: new Date(Date.now() + 3600000), nonce: 7, etypes: [18] }
  }));
  assert.strictEqual(tgs.decoded.kind, "KRB-ERROR", "a malformed TGS-REQ must be answered with an error");
  ...
  // And a TGS-REQ with no PA-TGS-REQ at all is structurally unanswerable, which is a
  // different refusal from a malformed one.
  const noTgt = await exchange(msgs.encKdcReq({
    msgType: msgs.MSG_TYPE.TGS_REQ,
    reqBody: { kdcOptions: [], realm: "EXAMPLE.COM",
               sname: { type: 3, name: ["HTTP", "web.example.com"] },
               till: new Date(Date.now() + 3600000), nonce: 8, etypes: [18] }
  }));
  assert.strictEqual(noTgt.decoded.error.errorCode, 25,
    "a TGS-REQ carrying no TGT at all must be refused as needing pre-authentication");
...
async function computerAccountsHaveAHostShapedSalt() {
  ...
  assert.strictEqual(first.decoded.kind, "KRB-ERROR", "expected PREAUTH_REQUIRED");
  ...
  // The salt for a COMPUTER account is not the principal name in any arrangement:
  // realm + "host" + short name + DNS domain. An implementation that derives the
  // salt from the principal string works until the first machine account — which
  // is the moment somebody is debugging a service rather than a user.
...
async function theKdcHonoursTheClientsEtypeOrder() {
  ...
  const cases = [
    ...
    [[23, 18], 23, "arcfour first — a client may ask for the weak one, and a KDC that supports it obliges"],
  ...
  for (const [offered, expected, label] of cases) {
    ...
    assert.strictEqual(reply.decoded.kind, "KRB-ERROR", label + ": expected PREAUTH_REQUIRED");
    const entries = msgs.readEtypeInfo2(
      reply.decoded.error.eDataPaData.filter(function (pa) { return pa.type === 19; })[0].value);
    // The KDC advertises what the PRINCIPAL supports, in the KDC's order — that is
    // not the same list as what the client offered, and conflating the two is how a
    // client ends up deriving a key of the wrong type.
    assert.ok(entries.length >= 1, label + ": the KDC must advertise something");
    const chosen = entries.filter(function (e) { return e.etype === expected; })[0];
    assert.ok(chosen, label + ": the KDC must advertise the etype it will accept (" +
    ...
    const done = await exchange(buildAsReq({ etypes: offered, padata: [preauth.padata] }));
    assert.strictEqual(done.decoded.kind, "AS-REP", label + ": expected a ticket");
    assert.strictEqual(done.decoded.rep.ticket.encPart.etype, expected,
      label + ": the KDC must choose the FIRST etype the client offered that the principal " +
...
async function test() {
  ...
  log.info("Starting Test run. The AS exchange: common/krb5 against the mock KDC, through the relay.");
  await startKdc();
  try {
...
program
  ...
  .description("The Kerberos AS exchange end to end: the wallet codec against the mock KDC.")
  // Accepted and ignored: run-report.js passes --url to every job.
  .addOption(new Option("-u, --url <url>", "base url (unused: this test starts its own KDC)"))
  .parse(process.argv);

test().then(function () {
  // The KDC's listeners are closed, but express in the sts module keeps no handle
  // here; exit explicitly so a lingering reference cannot hold the process open.
  process.exit(0);
}).catch(function (e) {
