...
async function waitForText(driver, id, pattern, timeoutMs, what) {
  ...
  // until.elementLocated succeeds while the page is still parsing and says nothing
  // about whether the exchange has happened. See tests/wait_for.js.
  let last = "";
  try {
...
// Is the KDC reachable at all? Asked through the mock STS's own index rather than
// by opening a socket, because that is what a browser test can do — and it answers
// the more useful question of whether the KDC is the one this test expects.
async function kdcIsReachable() {
  try {
    const response = await fetch(stsUrl + "/krb5/principals");
    if (!response.ok) return { ok: false, why: stsUrl + "/krb5/principals answered " + response.status };
    const body = await response.json();
    if (body.realm !== realm) {
      return { ok: false, why: "the mock KDC serves realm " + body.realm + ", not " + realm };
    }
    const names = (body.principals || []).map(function (p) { return p.principal.split("@")[0]; });
    if (names.indexOf(principal) === -1) {
      return { ok: false, why: "the mock KDC has no principal named " + principal +
        " (it has " + names.join(", ") + ")" };
    }
    return { ok: true, kdcPort: String(body.kdcPort) };
  } catch (e) {
    return { ok: false, why: "the mock STS at " + stsUrl + " did not answer (" + e.message + ")" };
  }
}
...
async function thePageIsWiredAndSaysWhatItNeeds(driver) {
  ...
  assert.strictEqual(loaded, "object",
    "the kerberos bundle did not load — check the BUNDLES entry in client/build.js AND the " +
    "browserify line in client/Dockerfile. A page registered in only one of the two builds fine " +
    "for the static deployments while the containerized page's <script> 404s, so the failure " +
  ...
  const environment = await waitForText(driver, "krb_environment_note", /relay to ports|did not answer/,
  ...
  assert.ok(/relay to ports/.test(environment),
    ...
    "\nOn a browser this is usually the api's CORS allowlist (uiUrl) not matching the origin the " +
    "page is served from — a failure no node test can see.");
  log.info("the api's relay limits, as the page sees them: " + environment.replace(/\s+/g, " ").slice(0, 160));
...
async function stepTwoIsGatedOnStepOne(driver) {
  ...
  assert.ok(disabled,
    "step 2 must start DISABLED. Without step 1 there is no salt, and a user who guesses one gets " +
    "KDC_ERR_PREAUTH_FAILED — which reads as a wrong password and sends them looking in the wrong " +
    "place entirely.");
...
async function stepOneDiscoversTheSalt(driver) {
  ...
  assert.ok(/PREAUTH_REQUIRED/.test(status),
    "a bare AS-REQ for " + principal + " should be answered with KDC_ERR_PREAUTH_REQUIRED: " + status);
  assert.ok(/not a failure/.test(status),
    "and the page must say that is the EXPECTED answer rather than presenting it as a failure: " +
    status);
  ...
  assert.ok(discovered.indexOf(expectedSalt) !== -1,
    "the salt the KDC sent must be shown — it is not guessable and this is the only place it comes " +
    "from. Expected " + expectedSalt + " in: " + discovered.slice(0, 300));
  assert.ok(/not guessable/i.test(discovered),
    "and the pane should say why the salt matters, since that is the point of the round trip");
  ...
  assert.strictEqual(saltField, expectedSalt,
    "the salt field must be filled from PA-ETYPE-INFO2, not left for the user to type");
  ...
  // The request that went out is shown, which is the page's claim to be a debugger.
  const request = await driver.findElement(By.id("krb_request_pane")).getText();
  assert.ok(/AS-REQ/.test(request), "the request pane must show what was sent: " + request.slice(0, 120));
...
async function stepTwoGetsATicketAndTreatsItAsACredential(driver) {
  ...
  const status = await waitForText(driver, "krb_as_status",
    /A TGT for|will not decrypt|refused|does not match/, 60000, "step 2 produced no result");
  assert.ok(/A TGT for/.test(status),
    "with the right password and the KDC's own salt, a ticket must be issued: " + status);

  const cache = await driver.findElement(By.id("krb_cache_pane")).getText();
  assert.ok(cache.indexOf(principal + "@" + realm) !== -1, "the cache must name the client: " + cache.slice(0, 200));
  assert.ok(/krbtgt\//.test(cache), "and the service, which for a TGT is krbtgt");
  assert.ok(/pre-authent/.test(cache),
    "the ticket must carry pre-authent, since pre-authentication actually happened");
  assert.ok(/A CREDENTIAL/.test(cache),
    "and the session key must be labelled as the credential it is, because that is not obvious");

  // Where it is kept. sessionStorage by default; localStorage only when asked.
  const inSession = await driver.executeScript("return !!sessionStorage.getItem('krb_ccache');");
  const inLocal = await driver.executeScript("return !!localStorage.getItem('krb_ccache');");
  assert.strictEqual(inSession, true, "the ticket must be in sessionStorage");
  assert.strictEqual(inLocal, false,
    "and NOT in localStorage while the box is unticked. A session key is standing access to whatever " +
  ...
  // spot. An opt-out that leaves yesterday's session key behind is not an opt-out.
  await driver.findElement(By.id("krb_save_ccache")).click();
  await driver.findElement(By.id("krb_preauth_button")).click();
  await waitForText(driver, "krb_as_status", /A TGT for/, 60000, "the second exchange produced no ticket");
  assert.strictEqual(await driver.executeScript("return !!localStorage.getItem('krb_ccache');"), true,
    "with the box ticked the ticket must be in localStorage");
  await driver.findElement(By.id("krb_save_ccache")).click();
  assert.strictEqual(await driver.executeScript("return !!localStorage.getItem('krb_ccache');"), false,
    "unticking the box must PURGE the stored cache immediately, not merely stop writing new ones");
  ...
  assert.strictEqual(await driver.executeScript(
    "return !!(localStorage.getItem('krb_ccache') || sessionStorage.getItem('krb_ccache'));"), false,
...
async function aWrongPasswordIsDiagnosedAndStoresNothing(driver) {
  ...
  await driver.findElement(By.id("krb_preauth_button")).click();
  const status = await waitForText(driver, "krb_as_status",
    /PREAUTH_FAILED|will not decrypt|refused/, 60000, "the wrong-password case produced no result");
  assert.ok(/PREAUTH_FAILED|will not decrypt/.test(status),
    "a wrong password must be reported: " + status);
  // The diagnosis that matters: at the KDC this is indistinguishable from a wrong
  // SALT, and a page that does not say so sends people to change their password.
  assert.ok(/salt/i.test(status),
    "and the message must mention the salt, because the KDC cannot tell a wrong password from a " +
...
async function test() {
  ...
  if (!reachable.ok) {
    ...
    log.warn("SKIPPED: " + reachable.why + ". This test needs the client, the api and the mock STS " +
      "(which carries the KDC on port " + kdcPort + "). Start the stack, or run " +
      "./local-run-tests.sh which does.");
    log.info("Test completed successfully.");
    return;
  }
  if (reachable.kdcPort && reachable.kdcPort !== String(kdcPort)) {
    log.warn("the mock STS reports its KDC on port " + reachable.kdcPort + " but this test was told " +
      kdcPort + "; using the reported one.");
    kdcPort = reachable.kdcPort;
  }

  const options = new chrome.Options();
  // --headless=new, never bare --headless: in the image's Chrome 121 the old mode
  // ignores --unsafely-treat-insecure-origin-as-secure, so crypto.subtle stays
  // undefined and the key derivation on this page silently has no crypto.
...
program
  ...
  .description("Verify the Kerberos AS exchange page: wiring, CORS, the two-step flow, credential handling.")
