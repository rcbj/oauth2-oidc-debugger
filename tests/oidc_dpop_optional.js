...
// read from the pane, so "the token is bound to this key" is checked against the
...
function thumbprint(jwk) {
  ...
  return b64u(crypto.createHash("sha256").update(JSON.stringify(canonical), "utf8").digest());
}
...
async function runAuthorizationCodeFlow(driver, { expectJkt }) {
  log.info("Entering runAuthorizationCodeFlow(). expectJkt=" + (expectJkt || "(none)"));
  ...
  // Expanded if it is not already, rather than clicked unconditionally. Once the
  // page has been through discovery it remembers (debugger_initialized) and opens
  // this fieldset itself, so a blind click COLLAPSES it and every field below is
  // "element not interactable" — which reads as a broken page rather than as a
  // toggle pressed twice.
  ...
  const preview = await driver.findElement(By.id("display_authz_request_form_textarea1"))
                              .getAttribute("value");
  const sentJkt = (preview.match(/dpop_jkt=([^&\n]*)/) || [])[1] || "";
  if (expectJkt) {
    assert.strictEqual(sentJkt, expectJkt,
      "The authorization request should carry dpop_jkt=" + expectJkt + ", and carries \"" +
      sentJkt + "\". Request:\n" + preview);
  } else {
    assert.strictEqual(sentJkt, "",
      "DPoP is off for this workflow, and the authorization request still carries dpop_jkt=" +
      sentJkt + ". Request:\n" + preview);
  }

  await driver.findElement(By.css("input[type=\"submit\"][value=\"Authorize\"]")).click();

  // Sign in only if asked to. This test runs the flow four times in one browser,
  // and the OP keeps a session — so the second and later passes come straight
  // back with the response, which is correct behaviour and not something to wait
  // for a login screen through. Whichever arrives first decides.
  await driver.wait(async function () {
    if ((await driver.getCurrentUrl()).indexOf("/debugger2.html") >= 0) return "returned";
    return (await driver.findElements(By.id("username"))).length ? "login" : false;
...
async function test() {
  ...
  try {
    ...
    assert.strictEqual(await driver.findElement(By.id("dpop_enabled")).isSelected(), false,
      "DPoP must be OFF by default on the OAuth2/OIDC workflow. A debugger that sender-constrains " +
      "tokens without being asked cannot demonstrate the Bearer exchange the specifications " +
      "describe first.");
    ...
    assert.ok(!run.claims.cnf,
      "DPoP is off, and the access token came back sender-constrained anyway (cnf=" +
    ...
    const publicJwk = JSON.parse(await storage(driver, "oauth_dpop_public_jwk"));
    ...
    assert.strictEqual(storedJkt, expectedJkt,
      "The page recorded jkt " + storedJkt + " for a key whose RFC 7638 thumbprint is " +
      expectedJkt + ".");
    assert.ok(!(await storage(driver, "sdjwtvc_dpop_private_jwk")),
      "Switching DPoP on here generated a key in the SD-JWT VC workflow's storage. The two " +
    ...
    assert.ok(run.claims.cnf && run.claims.cnf.jkt,
      "DPoP is on and a proof was sent, but the access token came back with no cnf.jkt.");
    assert.strictEqual(run.claims.cnf.jkt, expectedJkt,
      "The token is bound to " + run.claims.cnf.jkt + ", not to this page's key (" +
      expectedJkt + ").");
    // Read from the RESULTS pane, not the request form: the success handler
    // collapses the form, so a verdict rendered there would be true and invisible.
    const verdictEl = await driver.findElements(By.id("dpop_result_status"));
    assert.ok(verdictEl.length,
      "The Token Endpoint Results pane says nothing about the binding that was just obtained.");
    ...
    log.info("[on] OK — dpop_jkt on the authorization request, a proof on the Token Request, and " +
    ...
    assert.ok(!(await storage(driver, "oauth_dpop_private_jwk")),
      "Switching DPoP off left the private key in storage. A key nothing will use again is one a " +
    ...
    assert.ok(!run.claims.cnf,
      ...
      JSON.stringify(run.claims.cnf) + ").");
    log.info("[off again] OK — the key was discarded and the exchange is Bearer once more.");
    ...
    await driver.executeScript(
      ...
      "window.localStorage.setItem('sdjwtvc_dpop_jkt','a-vc-workflow-key-thumbprint');");
    ...
    await driver.executeScript(
      ...
      "  .then(function(jwks){" +
      "    window.localStorage.setItem('sdjwtvc_dpop_public_jwk', JSON.stringify(jwks[0]));" +
      "    window.localStorage.setItem('sdjwtvc_dpop_private_jwk', JSON.stringify(jwks[1]));" +
      "    return true; });");
    assert.ok(await storage(driver, "sdjwtvc_dpop_private_jwk"),
      "The VC workflow's DPoP key was not seeded, so this phase would pass vacuously.");
    await openDebugger2(driver);
    assert.strictEqual(await driver.findElement(By.id("dpop_enabled")).isSelected(), false,
    ...
    assert.ok(!run.claims.cnf,
      ...
      JSON.stringify(run.claims.cnf) + "). DPoP is meant to be optional here, and this is the " +
      "state in which it was mandatory.");
    log.info("[vc switch on, this one off] OK — the VC workflow's DPoP no longer decides for the " +
...
program
  ...
  .description("DPoP is optional on the OAuth2/OIDC workflow: off by default, on when asked.")
