...
//                 a fact rather than an intention, and nobody at a store reviews
//                 an unpacked extension on our behalf, so it is the only thing
//                 standing behind that claim.
//
// It must NOT compare clientDataJSON byte for byte. Chrome inserts
// `other_keys_can_be_added_here` into some ceremonies and not others, at random,
// specifically to break implementations that compare it against a template —
// measured happening on 2026-08-08. The comparison is over the parsed fields.
//
...
const STS = (process.env.OID4VCI_ISSUER_URL ||
             (process.env.WSTRUST_STS_URL || "http://localhost:8081/sts").replace(/\/sts\/?$/, ""));
const CLIENT_ID = "webauthn-extension-test";
const REDIRECT = STS + "/oauth2/callback-sink";
const USER = "extuser-" + process.pid.toString(36) + "-" + Date.now().toString(36);
...
].filter(function (p) { return fs.existsSync(path.join(p, "manifest.json")); })[0];

function authorizeUrl() {
  return STS + "/oauth2/authorize?response_type=code&client_id=" + encodeURIComponent(CLIENT_ID) +
...
async function buildDriver(withExtension) {
  ...
  if (withExtension) {
    // --load-extension, not addExtensions(): the latter wants a packed .crx, and
    // an unpacked directory is what this project ships and what a user loads.
    options.addArguments("--load-extension=" + EXTENSION_DIR);
    // chromedriver passes --disable-extensions among its default switches, which
    // would silently cancel the line above.
...
// Ask the extension, through the bridge, from the debugger's own origin. This is
// the contract: a namespaced postMessage handshake, because externally_connectable
// is Chrome-only and this has to work in Firefox too.
async function capturesFromBridge(driver) {
  return driver.executeAsyncScript(
    ...
    "  if (!e.data || e.data.channel !== 'idptools-webauthn-response') return;" +
    "  if (e.data.id !== id) return;" +
    "  clearTimeout(timer); window.removeEventListener('message', handler);" +
    "  done(e.data.result || { empty: true });" +
    "});" +
    "window.postMessage({ channel: 'idptools-webauthn-request', id: id, action: 'get' }," +
...
async function test() {
  if (!EXTENSION_DIR) {
    log.error("No unpacked CI extension build found. Run `node extension/build.js` (it writes " +
              "extension/dist/ci), or stage it beside the tests as extension-ci/. This job cannot " +
  ...
  try {
    ...
    log.error("No mock STS at " + STS + " (" + e.message + "). This job needs one.");
    process.exitCode = 1;
    return;
  }
  ...
  await section("the CI build differs from the shipped build in exactly one file", () => {
    ...
    assert.deepStrictEqual(extra, ["autoarm.json"],
      "the CI build should add autoarm.json and nothing else; it adds: " + extra.join(", "));
    ...
    const ciManifest = JSON.parse(fs.readFileSync(path.join(EXTENSION_DIR, "manifest.json"), "utf8"));
    const shippedManifest = JSON.parse(fs.readFileSync(path.join(shipped, "manifest.json"), "utf8"));
  ...
  try {
    ...
    await section("the extension is actually loaded", async () => {
      ...
      assert.ok(marker,
        "the bridge sets a presence attribute at document_start on the debugger's origins, and it " +
        "is absent — so the extension did not load at all. Branded Google Chrome refuses to " +
        "side-load an unpacked extension; use Chrome for Testing (CHROME_BIN).");
    ...
    await section("a ceremony on a third-party origin is captured", async () => {
      ...
      }, waitTime * 8, "no capture reached the debugger's origin through the bridge");
      ...
      assert.ok(capture.origin.indexOf(new URL(STS).host) >= 0,
        "the capture should name the origin it was taken on; got " + capture.origin);
      ...
      assert.ok(answer.version, "the bridge should report the extension's version for drift checks");
      return "v" + capture.v + " " + capture.ceremony + " on " + capture.origin +
             ", extension " + answer.version;
    });

    await section("the REQUEST half is captured — the part no relying party shows", async () => {
      const answer = await capturesFromBridge(driver);
      const capture = answer.captures[0];
      const pk = capture.request && capture.request.publicKey;
      assert.ok(pk, "the capture should carry the options the relying party passed in");
      assert.ok(pk.challenge && pk.challenge.__b64u,
        "including the challenge, as base64url: " + JSON.stringify(pk.challenge));
      assert.ok(Array.isArray(pk.pubKeyCredParams) && pk.pubKeyCredParams.length,
      ...
      return "challenge, " + pk.pubKeyCredParams.length + " algorithm(s), attestation=" +
             pk.attestation + ", timeout=" + pk.timeout;
    });

    await section("a decoded capture agrees with the ceremony it recorded", async () => {
      const answer = await capturesFromBridge(driver);
      const capture = answer.captures[0];
      ...
      assert.ok(/ES256|RS256/.test(cose), "the credential's algorithm should be named: " + cose.slice(0, 80));
      return "the capture decodes on the Analyzer with no re-formatting";
    });

    await section("the Analyzer's capture inbox lists it and loads it with one click", async () => {
      await driver.get(baseUrl + "/webauthn_analyzer.html");
      await driver.wait(until.elementLocated(By.id("wa_ext_body")), waitTime * 4);
      ...
      }, waitTime * 6, "the inbox never listed the capture; it holds: " + listed.slice(0, 200));

      const armedNote = await driver.findElement(By.id("wa_ext_status")).getText();
      assert.ok(/Observing/.test(armedNote),
        "the pane should say which origin is being observed and until when: " + armedNote);
      assert.ok(!/format v/.test(armedNote),
        "and it should not be reporting capture-format drift, since both sides came from this " +
        "tree: " + armedNote);
      assert.ok(/Extension version /.test(armedNote),
        "the pane should state the extension's build, which is what somebody reporting a problem " +
      ...
      }, waitTime * 6, "loading a capture did not populate the request pane; it holds: " +
                       request.slice(0, 200));
      const cose = await driver.findElement(By.id("wa_cose_body")).getText();
      assert.ok(/jwk\.kty/.test(cose), "and the credential should have decoded: " + cose.slice(0, 120));
      return "listed, loaded, request and key both on screen";
    });

    await section("Done — stop observing disarms the extension from the page", async () => {
      ...
      }, waitTime * 6, "the page never confirmed the extension had stopped; status: " + st);
      // And the buffer really is gone, asked over the bridge rather than believed.
      const after = await capturesFromBridge(driver);
      assert.ok(after && !after.armed,
        "the extension should no longer be armed after the page said it stopped");
      assert.strictEqual((after.captures || []).length, 0,
        "and the capture buffer should have gone with it — a ceremony is somebody's " +
        "authentication and keeping it past the window it was collected in is not this " +
  ...
  await section("the extension changes nothing about the ceremony", () => {
    assert.ok(withExtension && withoutExtension, "both fingerprints should have been taken");
    ...
    assert.strictEqual(withExtension.type, withoutExtension.type, "clientData.type");
    assert.strictEqual(withExtension.origin, withoutExtension.origin, "clientData.origin");
    assert.strictEqual(withExtension.crossOrigin, withoutExtension.crossOrigin, "clientData.crossOrigin");
    assert.strictEqual(withExtension.alg, withoutExtension.alg, "the credential algorithm");
    assert.strictEqual(withExtension.completed, true, "the observed ceremony completed");
    assert.strictEqual(withoutExtension.completed, true, "the unobserved ceremony completed");
...
// What the ceremony determined, read out of the artifacts themselves in the page
// that produced them, so the two runs are compared on the same terms.
async function ceremonyFingerprint(driver, username) {
  ...
  return driver.executeAsyncScript(
    "var done = arguments[arguments.length - 1];" +
    "var b64u = function (b) { return btoa(String.fromCharCode.apply(null, new Uint8Array(b)))" +
    "  .split('+').join('-').split('/').join('_').split('=').join(''); };" +
    "navigator.credentials.create({ publicKey: {" +
    "  rp: { name: 'fingerprint', id: location.hostname }," +
    "  user: { id: new TextEncoder().encode('fp'), name: 'fp', displayName: 'fp' }," +
    "  challenge: new Uint8Array(32).fill(3)," +
    "  pubKeyCredParams: [{ type: 'public-key', alg: -7 }]," +
    "  authenticatorSelection: { userVerification: 'preferred' }," +
    "  attestation: 'direct', timeout: 20000 } })" +
    ".then(function (c) {" +
    "  var cd = JSON.parse(new TextDecoder().decode(c.response.clientDataJSON));" +
    "  var ad = new Uint8Array(c.response.getAuthenticatorData());" +
    "  var f = ad[32];" +
    "  done({ completed: true, type: cd.type, origin: cd.origin, crossOrigin: cd.crossOrigin," +
    "         alg: c.response.getPublicKeyAlgorithm ? c.response.getPublicKeyAlgorithm() : null," +
    "         flags: { UP: !!(f & 1), UV: !!(f & 4), AT: !!(f & 64), ED: !!(f & 128) } });" +
    "}).catch(function (e) { done({ completed: false, error: e.name + ': ' + e.message }); });");
}

const program = new Command();
program
  .name("webauthn_extension")
  .description("The read-only extension observing a third party's ceremony, and changing nothing.")
  .addOption(new Option("-u, --url <url>", "The debugger base URL.").makeOptionMandatory())
