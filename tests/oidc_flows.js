...
//
//   * a code-bearing flow to carry `dpop_jkt` on the authorization request
//     (RFC 9449 section 10 — this is what binds the CODE, and it can only travel
...
//     pane is required to state the opposite, and the token is required to carry
//     no `cnf`.
//
// That asymmetry is the reason this runs as a matrix rather than as "DPoP works":
...
// RFC 7638 section 3.1: the required members only, lexicographic, no whitespace.
// Recomputed here rather than read from the page, because "the token is bound to
// this key" has to be checked against the arithmetic — the three ways a
// thumbprint goes wrong (an extra member, the wrong order, whitespace) all yield
// a value that is stable and plausible.
function thumbprint(jwk) {
  ...
  return crypto.createHash("sha256").update(JSON.stringify(canonical), "utf8").digest()
    .toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}

// What DPoP should do to THIS flow. The distinction is not on/off but on/off
// crossed with "does this flow reach the token endpoint": RFC 9449 binds a token
// issued there, and section 10's dpop_jkt binds a code. An Implicit flow has
// neither, so DPoP being on must change nothing about the wire and must be
...
function halfHash(value) {
  const digest = crypto.createHash("sha256").update(String(value), "ascii").digest();
...
// login screen, and that the same subject appears on every artifact of the flow.
...
async function prepareAuthorizationRequest(driver, flow, { clientId, scope }) {
  ...
  if (!(await driver.findElement(authorization_grant_type).isDisplayed())) {
    ...
    await driver.wait(until.elementIsVisible(driver.findElement(authorization_grant_type)), waitTime);
  }
  await new Select(await driver.findElement(authorization_grant_type)).selectByVisibleText(flow.label);

  // Expanded only if it is not already. Once the page has been through discovery
  // it remembers (debugger_initialized) and opens this fieldset itself, so a
  // blind click COLLAPSES it and every field below becomes "not interactable" —
  // which reads as a broken page rather than as a toggle pressed twice. It bit
  // once here the moment the DPoP runs added a detour to debugger2.html and back.
  ...
  await driver.executeScript("debug.recalculateAuthorizationRequestDescription();");
  const preview = await driver.findElement(By.id("display_authz_request_form_textarea1")).getAttribute("value");
  const nonce = await driver.findElement(By.id("nonce_field")).getAttribute("value");
...
//
// It has to happen BEFORE the authorization request, because dpop_jkt travels on
// that request and debugger.html assembles it synchronously from storage. A key
// generated later would leave the code unbound with nothing to show for it.
async function enableDpop(driver) {
  ...
  assert.ok(await driver.findElement(box).isSelected(), "The DPoP checkbox did not switch on.");
  ...
  const stored = await driver.executeScript("return window.localStorage.getItem('oauth_dpop_jkt');");
  assert.strictEqual(stored, jkt,
    "The page recorded jkt " + stored + " for a key whose RFC 7638 thumbprint is " + jkt + ".");
...
// identity every token then describes, which is what `user` is asserted against.
async function signIn(driver, user) {
  ...
  try {
    ...
    await driver.wait(until.elementIsVisible(driver.findElement(username)), waitTime);
  } catch (e) {
    ...
    throw new Error("The OP did not show its login screen. The browser is at: " + url);
  }
...
async function checkAuthorizationResponse(driver, flow, sent, expected) {
  ...
  if (carriesToken) {
    assert.ok(Object.keys(response.fragment).length,
      "response_type \"" + flow.responseType + "\" must answer in the fragment, and the fragment is empty. " +
      "URL: " + response.url);
    assert.ok(!response.query.access_token && !response.query.id_token,
      "A token came back in the QUERY, which OIDC forbids (it leaks through Referer and logs). URL: " +
      response.url);
  }
  const params = Object.assign({}, response.query, response.fragment);

  assert.strictEqual(params.state, sent.state,
    "The state did not come back unchanged. Sent " + sent.state + ", got " + params.state + ".");

  // Exactly the artifacts the response_type named, and no others.
  const artifacts = { code: flow.code, access_token: flow.accessToken, id_token: flow.idToken };
  Object.keys(artifacts).forEach(function (name) {
    if (artifacts[name]) {
      assert.ok(params[name],
        "response_type \"" + flow.responseType + "\" must return " + name + ", and none arrived. " +
        "Got: " + Object.keys(params).join(", ") + ".");
    } else {
      assert.ok(!params[name],
        "response_type \"" + flow.responseType + "\" must NOT return " + name + ", and one arrived.");
    }
  });
  log.info("[" + flow.responseType + "] OK — the response arrived " +
           (carriesToken ? "in the fragment" : "in the query") + " with exactly " +
           Object.keys(artifacts).filter(function (k) { return artifacts[k]; }).join(", ") + ".");

  // The ID token, against the specification's own rules.
  if (flow.idToken) {
    const claims = expected.verify(params.id_token, "the authorization endpoint's ID token");
    assert.strictEqual(claims.iss, expected.issuer, "The ID token's iss is " + claims.iss + ".");
    assert.strictEqual(claims.aud, expected.clientId, "The ID token's aud is " + claims.aud + ".");
    assertDescribesUser(claims, expected.user, "The ID token");
    assert.strictEqual(claims.nonce, sent.nonce,
      "The ID token's nonce is " + claims.nonce + ", not the one the page sent (" + sent.nonce + "). " +
    ...
    if (flow.accessToken) {
      assert.strictEqual(claims.at_hash, halfHash(params.access_token),
        "at_hash does not match the access token that came with it (OIDC 3.3.2.11).");
    }
    if (flow.code) {
      assert.strictEqual(claims.c_hash, halfHash(params.code),
        "c_hash does not match the code that came with it (OIDC 3.3.2.11).");
    }
    log.info("[" + flow.responseType + "] OK — the ID token verifies against jwks_uri, names " + claims.sub +
             ", carries the nonce" +
  ...
  if (flow.accessToken) {
    const claims = expected.verify(params.access_token, "the authorization endpoint's access token");
    assert.strictEqual(claims.iss, expected.issuer, "The access token's iss is " + claims.iss + ".");
    assertDescribesUser(claims, expected.user, "The access token");
    if (flow.idToken) {
      assert.strictEqual(claims.sub, claimsOf(params.id_token).sub,
        "The access token and the ID token from the same response describe different subjects.");
    }
    log.info("[" + flow.responseType + "] OK — the access token verifies against jwks_uri" +
             (flow.idToken ? " and names the same subject as the ID token" : "") + ".");
  }
...
async function checkPanesShowArtifacts(driver, flow, params) {
  ...
  // results pane entirely was not caught until this was narrowed. The ids differ
  // per flow (the panes are generated per grant type), so the fields are
  // collected from the containers rather than named one by one.
  const shown = await driver.executeScript(
    "var out = { panes: [], code: null };" +
    "['authorization_endpoint_result','authorization_endpoint_id_token_result']" +
    "  .forEach(function(id){ var pane = document.getElementById(id); if (!pane) return;" +
  ...
  if (flow.accessToken) {
    assert.ok(shown.panes.indexOf(params.access_token) >= 0,
      "The access token came back, and the Authorization Endpoint Results panes do not show it. " +
      "They hold: " + JSON.stringify(shown.panes) + ".");
  }
  if (flow.idToken) {
    assert.ok(shown.panes.indexOf(params.id_token) >= 0,
      "The ID token came back, and the Authorization Endpoint Results panes do not show it. " +
      "They hold: " + JSON.stringify(shown.panes) + ".");
  }
  if (flow.code) {
    assert.ok(shown.code === params.code,
      "The authorization code came back in the fragment but the Token Request's code field holds \"" +
      shown.code + "\". Nothing can be exchanged from there.");
  }
  log.info("[" + flow.responseType + "] OK — the page shows every artifact that arrived.");
...
// these two flows it achieves nothing, because the access token is minted by the
// authorization endpoint and RFC 9449 binds tokens minted at the token endpoint.
// A pane that showed a ready key and said nothing else would be telling the user
// their token is sender-constrained when it is not.
// ---------------------------------------------------------------------------
async function checkNoTokenRequestNotice(driver, flow) {
  ...
  const text = await driver.executeScript(
    "var e = document.getElementById('dpop_status'); return e ? e.textContent : '';");
  assert.ok(/no Token Request/i.test(text),
    "DPoP is on and the " + flow.label + " flow cannot use it, and the pane does not say so. " +
    "It says: \"" + text + "\".");
  assert.ok(/RFC 9449/.test(text),
    "The notice should name the specification it is explaining. It says: \"" + text + "\".");
  log.info("[" + flow.responseType + "/dpop-on] OK — the pane states that this flow has no Token " +
...
async function exchangeCode(driver, flow, sent, expected, { dpopJkt } = {}) {
  ...
  // api CORS allow-list that has to name whichever origin the debugger is served
  // from. The second is that it is the leg that carries a DPoP proof when one is
  // in play; the proxied call cannot, and debugger2.html says so in its banner.
  // Clicked rather than set in storage: useFrontEnd is only assigned by
  // setInitiateFromEnd(), which runs on the radio's own click handler.
  const frontEnd = await driver.findElements(By.id("token_initiateFromFrontEnd"));
  assert.ok(frontEnd.length, "debugger2.html has no token_initiateFromFrontEnd radio to select.");
  await driver.executeScript("arguments[0].scrollIntoView({block: 'center'});", frontEnd[0]);
  await frontEnd[0].click();
  assert.ok(await frontEnd[0].isSelected(),
    "The browser-direct radio did not take, so the Token Request would go through the api.");
  ...
  try {
    await driver.wait(async function () {
      ...
      if (errors.length) {
        ...
        if (text && text.trim()) throw new Error("The token endpoint refused the code: " + text);
      }
      return false;
    }, waitTime * 5);
  } catch (e) {
  ...
  assert.strictEqual(claims.iss, expected.issuer, "The exchanged access token's iss is " + claims.iss + ".");
  assertDescribesUser(claims, expected.user, "The exchanged access token");

  // The binding, in both directions. "Bound" is read off the token's own cnf.jkt
  // rather than off the fact that a proof was sent, because an authorization
  // server that ignores DPoP answers with a perfectly ordinary Bearer token and
  // the client cannot tell the difference any other way.
  if (dpopJkt) {
    assert.ok(claims.cnf && claims.cnf.jkt,
      "A DPoP proof was sent with the Token Request and the access token came back with no " +
      "cnf.jkt — it is an ordinary Bearer token, and nothing said so.");
    assert.strictEqual(claims.cnf.jkt, dpopJkt,
      "The token is bound to " + claims.cnf.jkt + ", not to this page's key (" + dpopJkt + ").");
    ...
    assert.ok(verdict.length,
      "The token came back sender-constrained and the results pane says nothing about it.");
    assert.ok(await verdict[0].isDisplayed(), "The binding verdict is in the page but not visible.");
    const verdictText = await verdict[0].getText();
    assert.ok(/sender-constrained/i.test(verdictText),
      "The results pane does not report the binding. It says: \"" + verdictText + "\".");
    log.info("[" + flow.responseType + "/dpop-on] OK — the exchanged token carries cnf.jkt = the " +
             "thumbprint of this page's key, and the results pane says so.");
  } else {
    assert.ok(!claims.cnf,
      "DPoP is off for this workflow and the exchanged access token came back sender-constrained " +
  ...
  assert.ok(idFields.length, "The Token Endpoint results pane has no ID token field.");
  const idValue = await idFields[0].getAttribute("value");
  assert.ok(idValue && idValue.split(".").length === 3,
    "The token endpoint returned no ID token for an openid request. Field holds: \"" + idValue + "\".");
  const idClaims = expected.verify(idValue, "the token endpoint's ID token");
  assert.strictEqual(idClaims.nonce, sent.nonce,
    "The token endpoint's ID token carries nonce " + idClaims.nonce + ", not the one the page sent.");
  log.info("[" + flow.responseType + "] OK — the code redeemed at the token endpoint, and both tokens verify.");
...
async function test() {
  ...
  try {
    ...
    assert(flow, "OIDC_FLOW=\"" + flowKey + "\" is not one of: " + Object.keys(FLOWS).join(", ") + ".");
    ...
    // DPoP is switched on — when it is — BEFORE the authorization request, since
    // dpop_jkt can only travel on that request. Storage was cleared just above,
    // so this is also what proves the switch defaults to off: with OIDC_DPOP=off
    // nothing here runs and the assertions below require an unbound exchange.
    var dpopJkt = "";
    if (dpopOn) {
    ...
    const sent = await prepareAuthorizationRequest(driver, flow, { clientId: clientId, scope: scope });
    ...
    assert.strictEqual(sentResponseType, flow.responseType,
      "The page is about to send response_type=\"" + sentResponseType + "\" for the " + flow.label +
      " flow, which asks for \"" + flow.responseType + "\". The request preview reads:\n" + sent.preview);
    assert.ok(sent.nonce, "The authorization request carries no nonce, which OIDC requires for this flow.");
    assert.ok(sent.preview.indexOf("nonce=" + sent.nonce) >= 0,
      "The nonce field holds " + sent.nonce + " but the request being sent does not carry it.");
    log.info("[" + flow.responseType + "] OK — the page sends response_type=" + sentResponseType +
             " with a nonce.");

    // RFC 9449 section 10 on the authorization request. Asserted in BOTH
    // directions: absent when DPoP is off (or when the flow has no code to bind),
    // and exactly this key's thumbprint when it should be there. A dpop_jkt sent
    // for the wrong key is worse than none — the code becomes unredeemable.
    const sentJkt = (sent.preview.match(/dpop_jkt=([^&\n]*)/) || [])[1] || "";
    if (dpop.sendJkt) {
      assert.strictEqual(sentJkt, dpopJkt,
        "The authorization request should carry dpop_jkt=" + dpopJkt + " and carries \"" + sentJkt +
        "\". Request:\n" + sent.preview);
      log.info("[" + flow.responseType + "/dpop-" + dpopSetting +
               "] OK — the authorization request binds the code with dpop_jkt.");
    } else {
      assert.strictEqual(sentJkt, "",
        "The authorization request carries dpop_jkt=" + sentJkt + ", and it should not: " +
        (dpopOn ? "this flow returns no code for it to bind." : "DPoP is off for this workflow."));
      log.info("[" + flow.responseType + "/dpop-" + dpopSetting +
    ...
    // whatever the switch says: there was no request for a proof to ride on. This
    // is the assertion that stops "DPoP is on" from being read as "everything is
    // bound now".
    if (flow.accessToken) {
      ...
      assert.ok(!authzClaims.cnf,
        ...
        JSON.stringify(authzClaims.cnf) + ". Nothing proved possession of a key on that leg — " +
        "the token arrived in the fragment — so a binding here would be a claim with nothing " +
        "behind it.");
    }
    ...
    if (flow.code) {
      await exchangeCode(driver, flow, sent, expected, { dpopJkt: dpop.expectBound ? dpopJkt : "" });
    }
...
program
  ...
  .addOption(
    new Option(
      "-f, --flow <flow>",
      "Which flow to run (overrides OIDC_FLOW): " + Object.keys(FLOWS).join(", "))
  )
  .addOption(
    new Option(
      "-b, --browser",
