...
//     first, then issues a code, honours state, reports errors the way OAuth 2.0
//     says to, and puts tokens in the fragment (never the query) for the
...
function claimsOf(token) { return JSON.parse(b64uDecode(String(token).split(".")[1]).toString("utf8")); }
function headerOf(token) { return JSON.parse(b64uDecode(String(token).split(".")[0]).toString("utf8")); }
...
function makeVerifier(jwks) {
  ...
  return function (token, what) {
    ...
    assert.strictEqual(header.alg, "RS256", what + " should be signed RS256, got " + header.alg + ".");
...
function parseRedirect(location) {
  ...
  return {
    location: location,
    fragment: hashAt >= 0,
    params: new URLSearchParams(hashAt >= 0 ? location.slice(hashAt + 1) : new URL(location).search)
  };
}
...
async function authorize(meta, params, options) {
  ...
  assert.strictEqual(r.status, 200,
    "the authorization endpoint should either redirect or show the login screen, got HTTP " + r.status + ".");
  ...
  r = await fetch(meta.issuer + "/oauth2/login", {
    ...
    body: form({ login_id: loginId, username: username, password: options.password || "any-password",
  ...
  assert.strictEqual(r.status, 302,
    "the authorization endpoint should answer after the login, got HTTP " + r.status + ".");
  const out = parseRedirect(r.headers.get("location"));
  out.prompted = true;
  out.cookie = cookie;
...
async function testLoginScreen(meta, verify) {
  ...
  assert.strictEqual(first.status, 200, "an unauthenticated request should show the login screen.");
  assert.ok(/text\/html/.test(first.headers.get("content-type") || ""), "the login screen should be HTML.");
  const page = await first.text();
  assert.ok(/<form[^>]+action="\/oauth2\/login"/.test(page), "the login screen should post to /oauth2/login.");
  assert.ok(/id="username"/.test(page) && /id="password"/.test(page),
    "the login screen should ask for a username and a password.");
  assert.ok(/name="login_id" value="[^"]+"/.test(page), "the form should carry the request it interrupted.");
  ...
  assert.ok(page.indexOf(CLIENT_ID) !== -1 && page.indexOf(REDIRECT_URI) !== -1,
    "the login screen should say which client and redirect_uri it is signing in for.");
  assert.ok(!/code=/.test(page), "nothing should be issued before the user signs in.");
  log.info("[login] OK — an unauthenticated authorization request is answered with a login form.");
  ...
  assert.ok(authz.viaAuthorize && authz.viaAuthorize.indexOf(meta.authorization_endpoint) === 0,
    "submitting the login form should redirect back to the authorization endpoint. Got: " + authz.viaAuthorize);
  ...
  assert.ok(authz.location.indexOf(REDIRECT_URI) === 0,
    "the authorization response should go to the client's redirect_uri. Got: " + authz.location);
  const code = authz.params.get("code");
  assert.ok(code, "no authorization code came back after the login.");
  assert.strictEqual(authz.params.get("state"), "login-state", "state must survive the login round trip.");
  ...
  assert.ok(at.sub.indexOf(username) !== -1,
    "the subject should be derived from the username signed in with. Got: " + at.sub);
  assert.strictEqual(it.preferred_username, username, "the ID token should name that user too.");
  assert.strictEqual(it.given_name, username, "the ID token's claims should describe that user.");
  assert.ok(String(it.email).indexOf(username) === 0,
    "the ID token's email should be derived from the username. Got: " + it.email);
  assert.strictEqual(it.nonce, "login-nonce", "the nonce must survive the login round trip.");
  assert.ok(it.auth_time > 0, "the ID token should say when the user authenticated.");

  const ins = await postForm(meta.introspection_endpoint, { token: set.access_token });
  assert.strictEqual(ins.body.username, username, "introspection should report that user as well.");
  const refreshed = await postForm(meta.token_endpoint, {
    grant_type: "refresh_token", refresh_token: set.refresh_token, client_id: CLIENT_ID
  });
  assert.strictEqual(claimsOf(refreshed.body.id_token).preferred_username, username,
    "refreshing should keep describing the user who signed in.");
  log.info('[login] OK — the tokens describe "' + username + '", the name that was typed in.');
  ...
  assert.ok(again.params.get("code"), "the session request should still issue a code.");
  const forced = await get(meta.authorization_endpoint + "?" + form(Object.assign({ prompt: "login" }, params)),
    { headers: { cookie: authz.cookie } });
  assert.strictEqual(forced.status, 200, "prompt=login should show the login screen even with a session.");
  const silent = await authorize(meta, Object.assign({ prompt: "none" }, params));
  assert.strictEqual(silent.params.get("error"), "login_required",
    "prompt=none with no session must fail rather than show UI. Got: " + silent.params.get("error"));
  log.info("[login] OK — the session skips the prompt, prompt=login forces it, prompt=none refuses.");
  ...
  assert.strictEqual(cancelled.params.get("error"), "access_denied",
    "cancelling at the login screen should be access_denied. Got: " + cancelled.params.get("error"));
  assert.strictEqual(cancelled.params.get("state"), "login-state", "even the cancel keeps state.");

  const noName = await get(meta.authorization_endpoint + "?" + form(params));
  const loginId = (await noName.text()).match(/name="login_id" value="([^"]+)"/)[1];
  const blank = await fetch(meta.issuer + "/oauth2/login", {
    method: "POST", redirect: "manual", headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: form({ login_id: loginId, username: "", password: "x", action: "login" })
  });
  assert.strictEqual(blank.status, 200, "an empty username should re-show the form, not redirect.");
  assert.ok(/Enter a username/.test(await blank.text()), "the form should say what was wrong.");
  const refused = await fetch(meta.issuer + "/oauth2/login", {
    method: "POST", redirect: "manual", headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: form({ login_id: loginId, username: "carol", password: "invalid", action: "login" })
  });
  assert.ok(/Authentication failed for carol/.test(await refused.text()),
    'the reserved password "invalid" should be refused at the login screen too.');
  log.info("[login] OK — cancel, a missing username and the reserved password are all handled.");

  // 7. Signing out means the next request prompts again.
  const loggedOut = await get(meta.issuer + "/oauth2/logout", { headers: { cookie: authz.cookie } });
  assert.strictEqual(loggedOut.status, 200, "logout should answer.");
  const afterLogout = await get(meta.authorization_endpoint + "?" + form(params),
    { headers: { cookie: authz.cookie } });
...
async function testEveryAdvertisedEndpointAnswers(meta) {
  ...
  for (const member of simple) {
    ...
    assert.strictEqual(r.status, 200, member + " (" + meta[member] + ") returned HTTP " + r.status + ".");
  }
  ...
  const token = await postForm(meta.token_endpoint, { grant_type: "no-such-grant" });
  assert.strictEqual(token.status, 400, "the token endpoint should answer, got HTTP " + token.status + ".");
  assert.strictEqual(token.body.error, "unsupported_grant_type",
    "an unknown grant should be an unsupported_grant_type error. Got: " + token.raw);

  const introspect = await postForm(meta.introspection_endpoint, { token: "" });
  assert.strictEqual(introspect.status, 200, "the introspection endpoint should answer 200.");
  assert.strictEqual(introspect.body.active, false, "introspecting nothing should be inactive.");

  const revoke = await postForm(meta.revocation_endpoint, { token: "not-a-token" });
  assert.strictEqual(revoke.status, 200,
    "RFC 7009: revoking an invalid token is still a 200. Got HTTP " + revoke.status + ".");

  const authz = await authorize(meta, {
    response_type: "code", client_id: CLIENT_ID, redirect_uri: REDIRECT_URI, scope: "openid", state: "s0"
  });
  assert.ok(authz.prompted, "an unauthenticated authorization request should show the login screen.");
...
async function testAuthorizationCode(meta, verify) {
  ...
  assert.strictEqual(authz.params.get("state"), "state-1", "the authorization response must echo state.");
  ...
  assert.strictEqual(wrong.body.error, "invalid_grant", "a bad code_verifier should be invalid_grant.");
  ...
  assert.strictEqual(tokens.status, 200, "the code exchange failed: " + tokens.raw);
  ...
  assert.strictEqual(at.iss, meta.issuer, "the access token's iss should be the issuer.");
  assert.strictEqual(at.client_id, CLIENT_ID, "the access token should name the client.");
  assert.strictEqual(at.scope, "openid profile email", "the access token should carry the granted scope.");
  assert.ok(at.exp > at.iat, "the access token should expire after it was issued.");
  assert.strictEqual(it.aud, CLIENT_ID, "the ID token's audience is the client.");
  assert.strictEqual(it.nonce, nonce, "the ID token must echo the nonce from the authorization request.");
  assert.strictEqual(it.at_hash, halfHash(set.access_token),
    "the ID token's at_hash should be the left half of the SHA-256 of the access token.");
  assert.strictEqual(it.sub, at.sub, "the ID token and access token should describe the same subject.");
  assert.strictEqual(rt.typ, "Refresh", "the refresh token should say what it is.");
  log.info("[code] OK — access / ID / refresh tokens all verify against jwks_uri, with matching claims.");
...
async function testAuthorizationErrors(meta) {
  ...
  const r = await get(meta.authorization_endpoint + "?" + form({ response_type: "code", client_id: CLIENT_ID }));
  assert.strictEqual(r.status, 400, "a request with no redirect_uri must not redirect anywhere.");
  const body = JSON.parse(await r.text());
  assert.strictEqual(body.error, "invalid_request", "the error should be invalid_request. Got: " + body.error);

  // With one, errors go back to the client.
  const unsupported = await authorize(meta, {
    response_type: "cwazy", client_id: CLIENT_ID, redirect_uri: REDIRECT_URI, state: "s"
  });
  assert.strictEqual(unsupported.params.get("error"), "unsupported_response_type",
    "an unknown response_type should come back as unsupported_response_type.");
  assert.strictEqual(unsupported.params.get("state"), "s", "an error response must echo state too.");
...
async function testImplicitAndHybrid(meta, verify) {
  ...
  for (const responseType of ["token", "id_token", "code id_token", "code id_token token"]) {
    const authz = await authorize(meta, {
      response_type: responseType, client_id: CLIENT_ID, redirect_uri: REDIRECT_URI,
      scope: "openid", nonce: "n-" + responseType.replace(/\s/g, "-"), state: "s"
    });
    assert.strictEqual(authz.fragment, true,
      responseType + ": a response carrying a token must use the fragment, never the query.");
    const wants = responseType.split(" ");
    if (wants.indexOf("code") >= 0) assert.ok(authz.params.get("code"), responseType + ": no code.");
    if (wants.indexOf("token") >= 0) {
      const at = verify(authz.params.get("access_token"), responseType + "'s access token");
      assert.strictEqual(at.client_id, CLIENT_ID, responseType + ": the access token should name the client.");
    }
    if (wants.indexOf("id_token") >= 0) {
      const it = verify(authz.params.get("id_token"), responseType + "'s ID token");
      ...
      if (authz.params.get("access_token")) {
        assert.strictEqual(it.at_hash, halfHash(authz.params.get("access_token")),
          responseType + ": at_hash should match the access token in the same response.");
      }
      if (authz.params.get("code")) {
        assert.strictEqual(it.c_hash, halfHash(authz.params.get("code")),
          responseType + ": c_hash should match the code in the same response.");
      }
    }
  }
...
async function testOtherGrants(meta, verify, codeTokens) {
  ...
  const refreshed = await postForm(meta.token_endpoint, {
    grant_type: "refresh_token", refresh_token: codeTokens.refresh_token, client_id: CLIENT_ID
  });
  assert.strictEqual(refreshed.status, 200, "the refresh failed: " + refreshed.raw);
  verify(refreshed.body.access_token, "the refreshed access token");
  assert.ok(advertised.indexOf("refresh_token") >= 0, "refresh_token should be advertised.");

  const cc = await postForm(meta.token_endpoint, { grant_type: "client_credentials", scope: "api" },
    { Authorization: "Basic " + Buffer.from("service-client:secret").toString("base64") });
  assert.strictEqual(cc.status, 200, "client_credentials failed: " + cc.raw);
  const ccClaims = verify(cc.body.access_token, "the client_credentials access token");
  assert.strictEqual(ccClaims.sub, "service-client",
    "client_credentials should describe the client itself (from client_secret_basic).");
  assert.ok(!cc.body.refresh_token, "client_credentials has no user, so no refresh token.");
  assert.ok(!cc.body.id_token, "client_credentials has no user, so no ID token.");

  const ro = await postForm(meta.token_endpoint, {
    grant_type: "password", username: "alice", password: "s3cret", scope: "openid", client_id: CLIENT_ID
  });
  assert.strictEqual(ro.status, 200, "the password grant failed: " + ro.raw);
  const roClaims = verify(ro.body.access_token, "the password grant access token");
  assert.ok(/alice/.test(roClaims.sub), "the password grant should describe the user who authenticated.");
  verify(ro.body.id_token, "the password grant ID token");

  const roBad = await postForm(meta.token_endpoint, {
    grant_type: "password", username: "alice", password: "invalid", client_id: CLIENT_ID
  });
  assert.strictEqual(roBad.status, 400, "the reserved 'invalid' password must be refused.");
  assert.strictEqual(roBad.body.error, "invalid_grant", "a failed password grant is invalid_grant.");
  ...
  assert.strictEqual(tx.body.issued_token_type, "urn:ietf:params:oauth:token-type:access_token",
  ...
  assert.strictEqual(txClaims.sub, claimsOf(refreshed.body.access_token).sub,
    "the exchanged token should keep the subject of the token it was exchanged for.");
...
async function testIntrospectionAndRevocation(meta, verify) {
  ...
  const authz = await authorize(meta, {
    response_type: "code", client_id: CLIENT_ID, redirect_uri: REDIRECT_URI, scope: "openid profile"
  });
  const set = (await postForm(meta.token_endpoint, {
  ...
  let ins = await postForm(meta.introspection_endpoint, { token: set.access_token });
  assert.strictEqual(ins.body.active, true, "a freshly issued token should introspect as active.");
  assert.strictEqual(ins.body.client_id, CLIENT_ID, "introspection should report the client.");
  assert.strictEqual(ins.body.scope, "openid profile", "introspection should report the granted scope.");
  assert.strictEqual(ins.body.sub, at.sub, "introspection should report the subject.");
  assert.strictEqual(ins.body.jti, at.jti, "introspection should report the token identifier.");
  assert.strictEqual(ins.body.token_type, "Bearer", "an access token introspects as a Bearer token.");

  ins = await postForm(meta.introspection_endpoint, { token: set.refresh_token });
  assert.strictEqual(ins.body.active, true, "the refresh token should introspect as active.");
  assert.strictEqual(ins.body.token_type, "refresh_token",
    "introspection should distinguish a refresh token. Got: " + ins.body.token_type);

  // A token this server did not sign must not introspect as active, however
  // well-formed it looks.
  const forged = ["e30", b64u(JSON.stringify({ sub: "attacker", exp: 4102444800 })), "not-a-signature"].join(".");
  ins = await postForm(meta.introspection_endpoint, { token: forged });
  assert.strictEqual(ins.body.active, false, "a forged token must introspect as INACTIVE.");
  log.info("[introspect] OK — real tokens report their claims, forged ones are inactive.");

  assert.strictEqual((await postForm(meta.revocation_endpoint, { token: set.access_token })).status, 200,
    "revocation should answer 200.");
  ins = await postForm(meta.introspection_endpoint, { token: set.access_token });
  assert.strictEqual(ins.body.active, false, "a revoked access token must stop being active.");
...
async function testRegistration(meta) {
  ...
  assert.strictEqual(r.status, 201, "registration should answer 201 Created, got " + r.status + ".");
  ...
  assert.ok(reg.client_id_issued_at > 0, "registration should say when the client was issued.");
  assert.strictEqual(reg.client_secret_expires_at, 0, "0 means the secret does not expire.");
  assert.strictEqual(reg.client_name, "Endpoint Test Client", "the registration should echo the metadata.");
  ...
  assert.strictEqual((await read.json()).client_id, reg.client_id, "the read should return the same client.");

  const unauth = await fetch(reg.registration_client_uri);
  assert.strictEqual(unauth.status, 401, "reading without the registration access token must be refused.");

  const updated = await fetch(reg.registration_client_uri, {
    method: "PUT", headers: Object.assign({ "Content-Type": "application/json" }, authed),
    body: JSON.stringify({ client_name: "Renamed Client", redirect_uris: [REDIRECT_URI] })
  });
  assert.strictEqual(updated.status, 200, "updating the registration failed.");
  const after = await updated.json();
  assert.strictEqual(after.client_name, "Renamed Client", "the update should take.");
  assert.strictEqual(after.client_id, reg.client_id, "an update must not change the client_id.");

  const deleted = await fetch(reg.registration_client_uri, { method: "DELETE", headers: authed });
  assert.strictEqual(deleted.status, 204, "deleting the registration should answer 204.");
...
program.addOption(new Option('-u, --url <url>', 'ignored; kept for a uniform CLI across the suite'));
