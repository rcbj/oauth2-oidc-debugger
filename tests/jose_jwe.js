...
// the weakest possible check of a key derivation — two sides of the same mistake
// agree perfectly — so the Concat KDF (RFC 7518 section 4.6) is also checked
// against an implementation written here, from the RFC text, that shares no code
// with the module.
//
...
var jose = paths.requireSharedModule(
  [__dirname + "/../client/src/jose_jwe.js", __dirname + "/jose_jwe.js"], "jose_jwe.js");
...
async function keyDerivationMatchesTheRfc() {
  ...
  // key-wrap variants (AlgorithmID is the full "alg").
  var cases = [
  ...
  for (var i = 0; i < cases.length; i++) {
    ...
    assert.ok(mine.equals(independent),
      "the derived key for " + cases[i][0] + " differs from an independent reading of RFC 7518 " +
  ...
  var other = Buffer.from(await jose.concatKdf(nodeCrypto.randomBytes(32), 32, "A256GCM"));
  var same = Buffer.from(await jose.concatKdf(z, 32, "A256GCM"));
  assert.ok(!other.equals(same), "a different agreed secret must derive a different key.");
...
async function everyAlgorithmRoundTrips() {
  ...
  var rsaSha1 = await crypto.subtle.generateKey(
    ...
    true, ["encrypt", "decrypt"]);
  var ec = await crypto.subtle.generateKey({ name: "ECDH", namedCurve: "P-256" }, true, ["deriveBits"]);
  ...
  for (var a = 0; a < algs.length; a++) {
    for (var e = 0; e < encs.length; e++) {
      ...
      assert.ok(pair, "no key pair for " + alg + " — the module offers an algorithm this test does not cover.");
      ...
      assert.strictEqual(parts.length, 5,
        alg + " / " + enc + ": compact serialization must have five segments, got " + parts.length);
      assert.strictEqual(produced.header.alg, alg, "the protected header must name the alg used.");
      ...
      if (alg === "ECDH-ES") {
        assert.strictEqual(parts[1], "",
          "ECDH-ES is direct key agreement: encrypted_key must be empty. Got " + parts[1].length +
          " characters.");
      } else {
      ...
      var back = await jose.decryptCompact({ jwe: produced.jwe, key: pair.privateKey });
      assert.strictEqual(back.plaintext, plaintext,
        alg + " / " + enc + " did not round-trip.");
      assert.strictEqual(back.header.alg, alg, "the decrypted header should be the one that was sent.");
      pairs++;
    }
  }
...
async function keysInEveryForm() {
  ...
  var pubPem = jose.derToPem(await crypto.subtle.exportKey("spki", rsa.publicKey), "PUBLIC KEY");
  var privPem = jose.derToPem(await crypto.subtle.exportKey("pkcs8", rsa.privateKey), "PRIVATE KEY");
  ...
  var fromJwks = { kty: pubJwk.kty, n: pubJwk.n, e: pubJwk.e, alg: "RSA-OAEP-256", use: "enc", kid: "issuer-enc-1" };
  var forms = [
    ["a CryptoKey", rsa.publicKey, rsa.privateKey],
    ["a JWK object from a JWKS", fromJwks, privJwk],
    ["JWK text, as a page field holds it", JSON.stringify(pubJwk), JSON.stringify(privJwk)],
    ["PEM", pubPem, privPem]
  ];
  for (var i = 0; i < forms.length; i++) {
    var produced = await jose.encryptCompact({
      alg: "RSA-OAEP-256", enc: "A256GCM", plaintext: "the same either way", key: forms[i][1]
    });
    var back = await jose.decryptCompact({ jwe: produced.jwe, key: forms[i][2] });
    assert.strictEqual(back.plaintext, "the same either way",
      "encrypting with " + forms[i][0] + " should work — that is what makes the module reusable.");
  }
...
async function tamperingAndBadInputAreRefused() {
  ...
  header.enc = "A128GCM";
  var edited = [Buffer.from(JSON.stringify(header)).toString("base64url")].concat(parts.slice(1)).join(".");
  await assert.rejects(
    jose.decryptCompact({ jwe: edited, key: rsa.privateKey }),
    "editing the protected header must fail: it is the additional authenticated data.");

  // A flipped bit in the ciphertext, likewise.
  var ciphertext = Buffer.from(parts[3], "base64url");
  ciphertext[0] ^= 0x01;
  var flipped = parts.slice(0, 3).concat([ciphertext.toString("base64url"), parts[4]]).join(".");
  await assert.rejects(
  ...
  assert.throws(function () { jose.parseCompact("one.two.three"); }, /five segments/,
    "a three-part token is a JWS, not a JWE, and should be named as such.");
  assert.throws(function () { jose.parseCompact("!!!!.b.c.d.e"); }, /readable JSON/,
  ...
  var noEpk = JSON.parse(Buffer.from(ecdhParts[0], "base64url").toString("utf8"));
  delete noEpk.epk;
  ...
  await assert.rejects(
    jose.encryptCompact({ alg: "A256KW", enc: "A256GCM", plaintext: "x", key: rsa.publicKey }),
    /unsupported key management algorithm/,
    "a key-management algorithm this module does not implement should be named.");
  await assert.rejects(
    jose.encryptCompact({ alg: "RSA-OAEP-256", enc: "A256CBC-HS512", plaintext: "x", key: rsa.publicKey }),
