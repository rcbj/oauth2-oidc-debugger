...
const schema = paths.requireSharedModule(
  [path.join(__dirname, "metadata_schema.js"), path.join(ROOT, "client", "src", "metadata_schema.js")],
...
async function freshRequestBody(meta, holderKey) {
  ...
  return wallet.buildRequestBody({ credentialConfigurationId: CONFIG_ID, proofs: [proof] });
}
...
function forgeJwe(body, jwk, overrides) {
  ...
  const b64 = function (b) { return Buffer.from(b).toString("base64url"); };
  ...
  const cipher = crypto.createCipheriv("aes-" + (keyBytes * 8) + "-gcm", cek, iv);
...
async function assertWaltidStillDoesNotOffer() {
  ...
  try {
    ...
    log.info("[walt.id] its metadata could not be read (" + e.message + "); support not re-checked.");
    return;
  }
  if (!meta) {
  ...
  if (meta.credential_request_encryption) {
    ...
    log.warn("[walt.id] NOW ADVERTISES credential_request_encryption: " +
      ...
      " — an interoperability test against it is now possible and should be written.");
  } else {
...
async function test() {
  ...
  assert.ok(offered,
    "the issuer should advertise credential_request_encryption. Without it a wallet has no key and " +
    "MUST NOT encrypt, so every assertion below would be untestable.");
  assert.ok(offered.jwks && Array.isArray(offered.jwks.keys) && offered.jwks.keys.length,
    "jwks is REQUIRED and must carry at least one key.");
  offered.jwks.keys.forEach(function (k, i) {
    assert.ok(k.kid, "key " + i + " has no kid, and \"Each JWK in the set MUST have a kid\".");
    assert.ok(k.alg, "key " + i + " has no alg. For REQUESTS the alg comes from the key — there is no " +
      "alg_values_supported to fall back on — so a key without one cannot be used at all.");
  });
  assert.ok(Array.isArray(offered.enc_values_supported) && offered.enc_values_supported.length,
  ...
  assert.strictEqual(offered.alg_values_supported, undefined,
    "credential_request_encryption must NOT carry alg_values_supported — that member belongs to the " +
    "RESPONSE side. Copying the response object's shape here is the likely mistake, and it produces " +
    "metadata that looks complete while telling the wallet to read the algorithm from the wrong place.");
  log.info("[metadata] OK — " + offered.jwks.keys.length + " key(s), enc " +
    offered.enc_values_supported.join("/") + ", required=" + offered.encryption_required + ".");
  ...
  const relevant = (verdict.errors || []).filter(function (e) {
    return String(e.cite || "").indexOf("credential_request_encryption") !== -1 ||
           String(e.member || "").indexOf("credential_request_encryption") !== -1;
  });
  assert.deepStrictEqual(relevant, [],
    "the issuer's own metadata must satisfy the schema rules the debugger applies to it: " +
    JSON.stringify(relevant));
  log.info("[schema] OK — the advertised object passes the debugger's own checker.");
  ...
  assert.strictEqual(offer.usable, true, "and be able to use it: " + offer.reason);
  assert.strictEqual(offer.alg, offer.jwk.alg,
    "the JWE alg MUST equal the alg of the chosen JWK — that is the rule that replaces " +
    "alg_values_supported for requests.");
  assert.ok(offered.enc_values_supported.indexOf(offer.enc) !== -1,
    "and the enc must be one the issuer said it can decode; the wallet chose " + offer.enc + ".");
  assert.strictEqual(offer.kid, offer.jwk.kid, "and it must remember which key it picked.");
  ...
  assert.strictEqual(noAlg.usable, false,
    "a key with no alg is not usable: section 10 requires the alg to be present, and defaulting one " +
    "would encrypt with an algorithm the issuer never claimed to support.");
  const noCommonEnc = wallet.requestEncryptionOffer({
    credential_request_encryption: {
      jwks: { keys: [{ kty: "RSA", n: "a", e: "AQAB", kid: "k1", alg: "RSA-OAEP-256" }] },
  ...
  assert.strictEqual(noCommonEnc.required, true,
    "and the wallet must still report that it was REQUIRED — that combination is the one a pane has " +
    "to explain, because it means the endpoint cannot be used at all.");
  const notOffered = wallet.requestEncryptionOffer({});
  assert.strictEqual(notOffered.offered, false, "an issuer that says nothing is offering nothing,");
  assert.strictEqual(notOffered.required, false, "and requiring nothing.");
  log.info("[select] OK — declines a key with no alg, an incompatible enc, and an absent offer.");
  ...
  assert.strictEqual(header.alg, offer.jwk.alg, "the header alg is the chosen key's alg.");
  assert.strictEqual(header.enc, offer.enc, "and the negotiated enc.");
  assert.strictEqual(header.kid, offer.jwk.kid,
    "and the kid MUST be echoed: \"If the selected public key contains a kid parameter, the JWE MUST " +
    "include the same value in the kid JWE Header Parameter.\" Omitting it leaves an issuer with " +
    "several keys guessing.");
  assert.ok(!header.zip,
    "no zip: this issuer advertises no zip_values_supported, and \"If absent then no compression " +
    "algorithms are supported\".");
  assert.ok(jwe.indexOf(CONFIG_ID) === -1,
    "the configuration id must not appear in the JWE as plaintext — if it does, nothing was encrypted.");
  ...
  for (const enc of offered.enc_values_supported) {
    if (wallet.REQUEST_ENC_PREFERENCE.indexOf(enc) === -1) {
      log.info("[enc] " + enc + " is advertised but this wallet does not perform it; not exercised.");
      continue;
    }
    const b = await freshRequestBody(meta, holderKey);
    const j = await wallet.encryptRequestBody(b, Object.assign({}, offer, { enc: enc }));
    const r = await postJwe(meta.credential_endpoint, j);
    assert.strictEqual(r.status, 200,
      "the issuer advertises enc " + enc + ", so it must decode it: HTTP " + r.status + " " +
  ...
  const refusals = [
    {
      ...
      why: "this is what a wallet holding cached metadata sends after the issuer rotates its key. " +
           "Ignoring the kid would turn that into an unexplainable decryption failure."
    },
    {
      what: "an enc the issuer did not advertise",
      jwe: forgeJwe(await freshRequestBody(meta, holderKey), offer.jwk, { enc: "A192GCM" }),
      why: "accepting an unadvertised algorithm makes enc_values_supported meaningless.",
    ...
    },
    {
      ...
      jwe: forgeJwe(await freshRequestBody(meta, holderKey), offer.jwk, { zip: "DEF" }),
      why: "\"If absent then no compression algorithms are supported.\""
    },
    {
      what: "an alg other than the chosen key's",
      jwe: forgeJwe(await freshRequestBody(meta, holderKey), offer.jwk, { alg: "RSA-OAEP" }),
      why: "\"The JWE alg algorithm used MUST be equal to the alg value of the chosen JWK\", and " +
           "RSA-OAEP (SHA-1) is not RSA-OAEP-256 however similar the name looks."
    },
    {
      what: "something that is not a JWE at all",
      jwe: "this.is.not.a.jwe",
      why: "an application/jwt body that cannot be parsed must be refused, not treated as JSON."
    }
  ];
  for (const c of refusals) {
    const r = await postJwe(meta.credential_endpoint, c.jwe);
    assert.strictEqual(r.status, 400, c.what + " must be refused with 400 — " + c.why +
      " Got HTTP " + r.status + " " + String(r.raw).slice(0, 160));
    assert.ok(r.body && r.body.error, "and a JSON error object. Got: " + String(r.raw).slice(0, 160));
    if (c.mentions) {
      assert.ok(String(r.body.error_description || "").indexOf(c.mentions) !== -1,
        "the refusal must say why in terms of the advertised policy (expected the description to " +
        "contain \"" + c.mentions + "\"), not fail incidentally somewhere deeper. Got: " +
  ...
  const good = await wallet.encryptRequestBody(await freshRequestBody(meta, holderKey), offer);
  const bits = good.split(".");
  bits[3] = bits[3].slice(0, -4) + (bits[3].slice(-4) === "AAAA" ? "BBBB" : "AAAA");
  ...
  log.info("[refuse] OK — tampered ciphertext → " + (tampered.body || {}).error);
  ...
  const control = await postJwe(meta.credential_endpoint,
    await wallet.encryptRequestBody(await freshRequestBody(meta, holderKey), offer));
  assert.strictEqual(control.status, 200,
    "the control: an untampered encrypted request still succeeds, so the refusals above are " +
    "discriminating rather than blanket.");
  log.info("[refuse] OK — the control still succeeds, so the refusals are discriminating.");

  // --- the deferred endpoint, same rule -------------------------------------
  log.info("=== The Deferred Credential Request (section 10 applies identically) ===");
  if (!meta.deferred_credential_endpoint) {
    log.info("[deferred] this issuer advertises no deferred_credential_endpoint; not exercised.");
  } else {
    ...
    const deferredJwe = await wallet.encryptRequestBody({ transaction_id: "no-such-transaction" }, offer);
    const d = await postJwe(meta.deferred_credential_endpoint, deferredJwe);
    assert.ok(d.body && d.body.error,
      "the deferred endpoint should answer with a JSON error: " + String(d.raw).slice(0, 200));
    assert.notStrictEqual(d.body.error, "invalid_encryption_parameters",
      "the encrypted deferred request must be DECRYPTED — an encryption error here means section 10 " +
      "was implemented on the credential endpoint only, which is the half that gets forgotten. Got: " +
    ...
    log.info("[deferred] OK — decrypted, then refused on its contents (" + d.body.error + ").");
  }
  ...
  if (offered.encryption_required === false) {
    ...
    assert.strictEqual(plain.status, 200,
      "with encryption_required = false the Wallet MAY choose, so an unencrypted request must still " +
    ...
    log.info("[required=true] not exercised here: it is a start-up setting " +
      "(OID4VCI_REQUEST_ENCRYPTION_REQUIRED=true) and this issuer is running with it off. The " +
    ...
    assert.strictEqual(plain.status, 400,
      "with encryption_required = true an unencrypted request SHOULD be rejected.");
