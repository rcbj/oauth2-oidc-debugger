...
// here. There is a TEST called tests/jwk_pem_encoding.js, so the container's flat layout
// would resolve that name to the test rather than the module — the collision the
// tests image's own comments warn about. cose.js requires it relative to itself
// and cannot be confused.

const VECTORS = JSON.parse(fs.readFileSync(path.join(__dirname, "webauthn_vectors.json"), "utf8"));
...
function cborVectors() {
  ...
  cases.forEach(function ([hex, expected]) {
    ...
    if (got instanceof Uint8Array) {
      assert.strictEqual(Buffer.from(got).toString("hex"), expected, "0x" + hex);
...
function cborRefusals() {
  ...
  mustThrow.forEach(function ([hex, pattern, what]) {
    const bytes = Uint8Array.from(Buffer.from(hex, "hex"));
    assert.throws(function () { cbor.decode(bytes); }, pattern, what + " must be refused");
  });
...
function registrationAgreesWithBrowser(name, vector) {
  ...
  assert.ok(ad.credentialPublicKeyJwk, "the credential public key must decode: " +
            (ad.credentialPublicKeyError || ""));
  assert.strictEqual(toB64u(ad.credentialId), vector.rawId,
    "the credential ID inside the authenticator data must equal the credential's rawId");

  // ORACLE 1: the browser's own SPKI for the same key.
  const described = cose.describe(ad.credentialPublicKey);
  assert.ok(described.pem, "the COSE key should render as a PEM: " + described.pemUnavailable);
  const derivedDer = Buffer.from(described.pem.replace(/-----[^-]+-----|\s+/g, ""), "base64");
  const browserDer = Buffer.from(b64u(vector.oracle.publicKeySpki));
  assert.strictEqual(derivedDer.toString("base64"), browserDer.toString("base64"),
    "our COSE -> JWK -> SPKI must be byte-identical to the browser's getPublicKey()");
  ...
  assert.strictEqual(toB64u(att.authDataBytes), vector.oracle.authenticatorData,
    "the authData we pulled out of the attestation object must equal getAuthenticatorData()");

  const cd = webauthn.parseClientDataJSON(b64u(vector.clientDataJSON));
  assert.strictEqual(cd.type, "webauthn.create", "clientData.type on a registration");
  ...
  return att.fmt + ", " + algName + ", aaguid " + ad.aaguidHex +
         ...
         (cd.extraMembers.length ? ", clientData carries " + cd.extraMembers.join("/") : "");
}

// --- assertion: our verifier, then node's ------------------------------------

const EXPECTED_ASSERTION = {
  challenge: toB64u(Uint8Array.from([0xca, 0xfe, 0xba, 0xbe].concat(Array(28).fill(0x22)))),
...
async function tamper(name, mutate, expectFailing) {
  ...
  assert.ok(failed.some(n => n.indexOf(expectFailing) !== -1),
    "expected the failure to be \"" + expectFailing + "\"; the failing checks were: " +
    failed.join("; "));
  // The point of reporting per-check: exactly one thing should be wrong.
  assert.strictEqual(failed.length, 1,
    "exactly one check should fail, so the pane names the right cause; failing: " + failed.join("; "));
...
async function test() {
  ...
  check("ES256 registration decodes and matches getPublicKey()",
        () => registrationAgreesWithBrowser("es256", VECTORS.es256_registration));
  check("RS256 registration decodes and matches getPublicKey()",
        () => registrationAgreesWithBrowser("rs256", VECTORS.rs256_registration));
  ...
  await checkAsync("RS256 assertion verifies",
    () => assertionVerifies(VECTORS.rs256_registration, VECTORS.rs256_assertion,
                            { expected: { requireUserVerification: true } }));
  await checkAsync("a discoverable-credential assertion carries the user handle", async () => {
    const v = VECTORS.es256_assertion_discoverable;
    assert.ok(v.userHandle, "a discoverable credential's assertion should carry userHandle");
  ...
  await checkAsync("tampered authenticatorData fails the signature", () =>
    tamper("authData", (i) => { i.authenticatorData[36] ^= 0xff; }, "signature verifies"));
  await checkAsync("tampered clientDataJSON fails the signature (the SHA-256 binding is real)", () =>
    tamper("clientData", (i) => {
      // Change a byte inside the JSON that is not one of the fields checked
      // separately, so ONLY the signature check can catch it.
      const text = Buffer.from(i.clientDataJSON).toString("utf8").replace('"crossOrigin":false', '"crossOrigin":true');
      i.clientDataJSON = new Uint8Array(Buffer.from(text, "utf8"));
    }, "signature verifies"));
  await checkAsync("a challenge mismatch is caught by name", () =>
    tamper("challenge", (i) => { i.expected.challenge = toB64u(new Uint8Array(32).fill(9)); },
           "challenge matches"));
  await checkAsync("an origin mismatch is caught by name", () =>
    tamper("origin", (i) => { i.expected.origin = "https://evil.example"; }, "origin matches"));
  await checkAsync("an RP ID mismatch is caught by name", () =>
    tamper("rpId", (i) => { i.expected.rpId = "example.com"; }, "rpIdHash is SHA-256"));
  await checkAsync("a sign-count regression is caught by name", () =>
    tamper("signCount", (i) => { i.expected.previousSignCount = 99999; }, "signature counter advanced"));
  ...
  await checkAsync("a UV-clear assertion is rejected on the FLAG, not the signature", async () => {
    ...
    const result = await webauthn.verifyAssertion({
      ...
      signature: b64u(asr.signature),
      publicKeyJwk: jwk,
      expected: Object.assign({}, EXPECTED_ASSERTION, { requireUserVerification: true }),
    });
    assert.ok(!result.valid, "UV was required and not performed, so this must not pass");
    assert.ok(result.signatureValid,
      "and the SIGNATURE must still be valid — reporting this as a bad signature would send " +
      "the user hunting for the wrong problem");
    const failed = result.checks.filter(c => !c.ok).map(c => c.name);
    assert.deepStrictEqual(failed, ["user verification (UV) was performed, as required"],
      "exactly the UV check should fail; got: " + failed.join("; "));
    return "signature valid, UV flag clear, one check failed";
  });

  await checkAsync("the same UV-clear assertion PASSES when UV is not required", async () => {
    const reg = VECTORS.uv_clear_registration, asr = VECTORS.uv_clear_assertion;
    const jwk = webauthn.parseAttestationObject(b64u(reg.attestationObject))
                        .authData.credentialPublicKeyJwk;
    const result = await webauthn.verifyAssertion({
    ...
    assert.ok(result.valid, "with UV not required this is a perfectly good assertion; failing: " +
  ...
  check("DER -> raw ECDSA conversion pads short components", () => {
    ...
    assert.strictEqual(raw[31], 0x01, "r must be right-aligned in the first half");
    assert.strictEqual(raw.slice(0, 31).every(b => b === 0), true, "and zero-padded on the left");
...
program
  ...
  .description("Decode and verify real WebAuthn artifacts; no browser, no network.")
