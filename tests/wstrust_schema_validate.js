...
// with the SAME generator the page uses (client/src/wstrust_msg.js) and
// validates it against a schema derived from the official OASIS WS-Trust 1.3
// XSD (see tests/schemas/ws-trust-rst.template.xsd for why "derived": libxml2,
// the engine behind every Node XSD validator, cannot compile the official
// schema's WS-Security / WS-Policy imports).
//
// Validation runs on libxml2 either way, reached by whichever binding is
// usable:
//
...
//      and no build toolchain to compile one, or (the common case here) a
//      binary built for a different Node version, which fails to load with a
...
// failing the suite. run-report spawns it with a --url argument, which it
// ignores.
...
//
// The level is read through a guard because this script runs BOTH ways: spawned
// by run-report.js, which sets CONFIG_FILE, and directly from a checkout, where
// it is unset and a bare require of it would throw.
...
if (lib) {
  ...
  engine = {
    ...
    validate(xml, xsd) {
      ...
      log.debug("Leaving validate().");
      return { ok, detail: ok ? "" : doc.validationErrors.map(e =>
              e.message.trim()).join("; ") };
    },
  };
} else {
  ...
  if (!probe.error && probe.status === 0) {
    ...
    engine = {
      name: "xmllint CLI (libxml2 " + (probe.stderr || probe.stdout ||
      ...
      validate(xml, xsd) {
        ...
        const r = spawnSync("xmllint", ["--noout", "--schema", xsdFiles[xsd],
            docFile], { encoding: "utf8" });
        if (r.error) {
          log.debug("Leaving validate().");
          return { ok: false, detail: "xmllint failed to run: " +
                  r.error.message };
        }
        const ok = r.status === 0;
...
if (!engine) {
  log.info("SKIP: no XSD validator available — libxmljs2 does not load and " +
           "the xmllint CLI is not on PATH.");
  log.info("      Install either (npm i libxmljs2, or the libxml2-utils " +
           "package) to enable WS-Trust message schema validation.");
  process.exit(0);
}

function loadFrom(candidates, what) {
  log.debug("Entering loadFrom().");
  for (const p of candidates) { if (fs.existsSync(p)) {
    log.debug("Leaving loadFrom().");
    return p;
  } }
...
function schemaForNs(ns) {
  log.debug("Entering schemaForNs().");
  if (!schemaCache[ns]) schemaCache[ns] = template.split("{{NS}}").join(ns);
  log.debug("Leaving schemaForNs().");
...
const TARGET_TOKEN =
  '<saml:Assertion xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion" ' +
      'ID="_tok" Version="2.0" IssueInstant="2026-01-01T00:00:00Z">' +
  '<saml:Issuer>urn:wstrust:mock:sts</saml:Issuer></saml:Assertion>';

function optsFor(version, op) {
  log.debug("Entering optsFor().");
  const v = wm.versionCfg(version);
  log.debug("Leaving optsFor().");
  return {
    version: version,
    operation: op,
    tokenType: "saml2",
    // Mirror the page: Bearer is only offered for 1.3+, so use SymmetricKey
    // below.
...
function main() {
  ...
  log.info("== WS-Trust RST schema validation (derived from OASIS " +
           "ws-trust-1.3.xsd) ==");
  ...
  for (const version of versions) {
    ...
    for (const op of ops) {
      ...
      check("WS-Trust " + version + " (" + ns.replace(/^https?:\/\//, "") +
            ") — " + op, r.ok, r.detail);
    }
  }
  ...
  const badIssue = goodIssue.replace("<wst:KeySize>256</wst:KeySize>",
      "<wst:KeySize>not-an-int</wst:KeySize>");
  const tampered = badIssue !== goodIssue;
  const neg = validate(badIssue, ns);
  check("negative control: non-integer KeySize is REJECTED", tampered &&
        neg.ok === false,
    tampered ? "schema accepted an invalid KeySize" : "tamper no-op (no " +
        "KeySize present)");
