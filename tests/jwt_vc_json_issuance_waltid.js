...
// DID (mapping.credentialSubject.id = "<subjectDid>"). Neither is wrong, so
// this test reports which it got and fails only when there is no binding at all
// — asserting our mock's choice here would be asserting that walt.id agrees
// with us, which is not what an interoperability test is for.
//
// Skipped, not failed, when walt.id offers no jwt_vc_json configuration. That
// is the state of a checkout whose walt.id container has not been restarted
// onto the configuration in waltid/config/credential-issuer-metadata.conf.
// ---------------------------------------------------------------------------
...
async function test() {
  ...
  assert.ok(issuerBase,
    "WALTID_ISSUER_URL is not set, so walt.id's issuer was never asked for a " +
  ...
  assert.ok(found.id,
    ...
    "Offered: " + Object.keys(found.meta.credential_configurations_supported ||
        {}).join(", ") +
  ...
  assert.ok(entry.credential_definition &&
    ...
    "a jwt_vc_json configuration identifies its credential with " +
        "credential_definition.type. Got: " +
  ...
  assert.ok(accessToken,
    ...
    WALTID_PROFILE_ID + "\". Without one the credential endpoint answers 401 " +
        "and nothing about " +
  ...
  assert.ok(claimCount > 0,
            "walt.id should assert something about the subject.");
  ...
  assert.notStrictEqual(binding.kind, "none",
    "an issued credential must say who may present it — cnf.jwk or a subject " +
        "DID in credentialSubject.id. " +
    "walt.id's credential carries neither, which would make it unpresentable " +
        "by anyone.");
  if (binding.kind === "cnf.jwk") {
    assert.strictEqual(binding.jwk.x, held.publicJwk.x,
      "a cnf.jwk binding should name the key the proof of possession " +
          "presented.");
    log.info("[binding] walt.id bound this credential with cnf.jwk, the same " +
             "way our mock does.");
  } else {
    log.info("[binding] walt.id bound this credential by SUBJECT DID (" +
             binding.subjectId + "), where our " +
             "mock uses cnf.jwk. Both are legitimate; it means a wallet has " +
                 "to sign the presentation with " +
             "the key behind that DID, which is what the presentation job " +
                 "reports on.");
  }

  log.info("[issuance] OK — walt.id issued a " + parsed.types.join("/") +
           " with " + claimCount +
