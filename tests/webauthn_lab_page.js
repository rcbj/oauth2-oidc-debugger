...
//
// **The virtual authenticator is the mock smart card.** `addVirtualAuthenticator`
// is a WebDriver-standard command backed by Chrome's CDP WebAuthn domain: a
// CTAP2 authenticator with configurable user verification, resident-key support
// and transports, entirely in the browser. No hardware, no touch, deterministic,
// headless. Its knobs are what make the negatives possible — `setIsUserVerified`
// and `setHasUserVerification` produce a genuine UV-clear ceremony, and
// `removeAllCredentials` produces the no-credential path.
//
...
// UV-required ceremony against an authenticator that cannot verify is refused by
// the BROWSER, so the relying-party-side UV check cannot be exercised from this
// page at all; that lives in tests/webauthn_decode.js, where the material can be
// manufactured.
// ---------------------------------------------------------------------------
...
// plus a sleep is the bet this suite has lost repeatedly. See tests/wait_for.js.
...
async function test() {
  ...
  if (headless) {
    ...
    // headless implementation ignores --unsafely-treat-insecure-origin-as-secure
    // and so leaves this page with no navigator.credentials at all.
  ...
  try {
    ...
    await driver.wait(until.elementLocated(By.id("wl_create_button")), waitTime * 4);
    await driver.executeScript("localStorage.clear();");
    await driver.navigate().refresh();
    await driver.wait(until.elementLocated(By.id("wl_create_button")), waitTime * 4);

    await section("the capability pane reports a usable environment", async () => {
      const caps = await waitForText(driver, "wl_caps_body", (t) => /secure context/.test(t),
        "the capability pane never filled");
      assert.ok(/PublicKeyCredential\s*available/.test(caps.replace(/\s+/g, " ")),
        "PublicKeyCredential should be available here. If it is not, the origin is not a secure " +
        "context and browser_flags.js did not relax it: " + caps.slice(0, 200));
      const disabled = await driver.findElement(By.id("wl_create_button")).getAttribute("disabled");
      assert.ok(!disabled, "the Create button should be enabled when the API is available");
      ...
      assert.ok(rpId && rpId.length, "the RP ID should be prefilled from this origin; got " + rpId);
      return "RP ID prefilled as " + rpId;
    });

    await driver.addVirtualAuthenticator(
      authenticatorOptions({ hasUserVerification: true, isUserVerified: true }));

    await section("registration runs a real ceremony and decodes it", async () => {
      await driver.findElement(By.id("wl_create_button")).click();
      const result = await waitForValue(driver, "wl_reg_result", (v) => /"attestationObject"/.test(v),
      ...
      assert.ok(parsed.response.clientDataJSON, "clientDataJSON should be present");

      const authData = await waitForText(driver, "wl_authdata_body", (t) => /AAGUID/.test(t),
        "the authenticator data pane never filled");
      assert.ok(/AT ✓/.test(authData), "AT should be set on a registration");
      assert.ok(/UV ✓/.test(authData), "UV should be set — this authenticator verifies");

      const cose = await waitForText(driver, "wl_cose_body", (t) => /jwk\.kty/.test(t),
        "the COSE key pane never filled");
      assert.ok(/ES256/.test(cose), "ES256 was requested and should be what came back");
      const pem = await valueOf(driver, "wl_cose_pem");
      assert.ok(/BEGIN PUBLIC KEY/.test(pem), "the SPKI PEM should be rendered");

      const cd = await textOf(driver, "wl_clientdata_body");
      assert.ok(/webauthn\.create/.test(cd), "clientData.type should be webauthn.create");
      return "credential " + parsed.rawId.slice(0, 16) + "…, ES256, PEM rendered";
    });

    await section("the ceremony trace names the authenticator boundary", async () => {
      const trace = await waitForText(driver, "wl_trace_body", (t) => t.length > 0,
        "the ceremony trace never filled");
      assert.ok(/private key did not cross/.test(trace),
        "the trace must state that the private key never crossed the boundary — that is the " +
        "single most useful thing this page has to say: " + trace.slice(0, 200));
      assert.ok(/CTAP/.test(trace),
        "and it must say that CTAP is not observable, rather than leaving the user to look for " +
        "a pane that cannot exist");
      return "boundary and CTAP both stated";
    });

    await section("authentication verifies against the key from that registration", async () => {
      await driver.findElement(By.id("wl_get_button")).click();
      const verdict = await waitForText(driver, "wl_auth_status", (t) => /VALID|NOT VALID/.test(t),
        "the assertion never produced a verdict");
      assert.ok(/^VALID/.test(verdict), "the assertion should verify: " + verdict);
      const checks = await textOf(driver, "wl_verify_body");
      assert.ok(/✓.*signature verifies/.test(checks), "the signature check should be listed as passing");
      assert.ok(/✓.*rpIdHash/.test(checks), "the RP ID hash check should be listed as passing");
      const result = await valueOf(driver, "wl_auth_result");
      assert.ok(/"signature"/.test(result), "the assertion JSON should be shown");
      return verdict;
    });

    await section("the sign counter advances between two assertions", async () => {
      const before = await textOf(driver, "wl_authdata_body");
      const firstCount = parseInt((before.match(/sign count\s+(\d+)/) || [])[1], 10);
      await driver.findElement(By.id("wl_get_button")).click();
      await driver.wait(async function () {
      ...
      }, waitTime * 6, "the sign counter did not advance across two assertions");
      const after = await textOf(driver, "wl_authdata_body");
      const secondCount = parseInt((after.match(/sign count\s+(\d+)/) || [])[1], 10);
      const verdict = await textOf(driver, "wl_auth_status");
      assert.ok(/^VALID/.test(verdict),
        "and the second assertion must still verify — the counter check compares against the " +
        "count this page remembered, so an off-by-one here reads as a cloned authenticator: " + verdict);
      return firstCount + " → " + secondCount;
    });

    await section("a credential the authenticator does not hold is reported, not hung", async () => {
      ...
      assert.ok(/NotAllowedError/.test(st), "the browser's error should be surfaced: " + st);
      // WebAuthn collapses several situations into one error on purpose; the page
      // must say that rather than guess which one happened.
      assert.ok(/privacy|indistinguishable|several situations/i.test(st),
        "and the page should explain that this error is deliberately ambiguous rather than " +
    ...
    await section("no pane overflows its box", async () => {
      const m = await driver.executeScript(
        ...
        "      worst = Math.round(r.right - pr.right); id = e.id || e.className; }" +
        "  });" +
        "});" +
        "return { worst: worst, id: id, sideways: doc.scrollWidth > doc.clientWidth + 2 };");
      assert.ok(m.worst <= 2, "content escapes its pane by " + m.worst + "px (" + m.id + ")");
      assert.strictEqual(m.sideways, false, "and the page must not scroll sideways");
...
program
  ...
  .description("Drive the WebAuthn Lab page against the WebDriver virtual authenticator.")
