...
| any page, bundle, layout or in-browser protocol implementation | `client/CLAUDE.md`, which indexes six topic docs under `docs/` |
...
OAuth2/OIDC Debugger — a two-service web application for testing and debugging OAuth2, OIDC, SAML, WS-Trust, WS-Federation, SD-JWT VC (issuance and presentation), WebAuthn, **Kerberos v5** and **LDAP v3** flows against real identity providers, issuers, verifiers, key distribution centers and security keys. Supports Authorization Code, Implicit, Client Credentials, Resource Owner Password, and Refresh grants, plus all three OIDC authentication flows (Authorization Code, Implicit, Hybrid).
