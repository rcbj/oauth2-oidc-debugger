...
- `sd_jwt_vc_presentation_waltid.js` (the presentation workflow against walt.id's `verifier-api2`: walt.id issues the credential, walt.id verifies the presentation, and our pages are the wallet in between. Positive plus a withheld-claim negative. Skipped when `WALTID_VERIFIER_URL` is unset)
