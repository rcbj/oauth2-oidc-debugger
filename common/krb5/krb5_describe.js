...
function parseInput(text) {
  ...
  if (!raw) throw new Error("Nothing to decode — paste a Kerberos message first.");
  ...
  if (/^[0-9a-f]+$/i.test(cleaned) && cleaned.length % 2 === 0 && cleaned.length >= 4) {
    ...
    if (pad === 1) throw new Error("This is not valid base64 (its length cannot be right) and not hex either.");
  ...
  if (bytes.length > 4 && (bytes[0] & 0x80) === 0) {
    ...
    if (declared === bytes.length - 4 && asn1.peekApplicationNumber(bytes.subarray(4)) !== null) {
...
function hexOf(bytes, limit) {
  ...
  var cap = limit || 96;
  return h.length > cap * 2 ? h.slice(0, cap * 2) + "… (" + prim.toBytes(bytes).length + " bytes)" : h;
}
...
function principalOf(p, realm) {
  ...
  return msgs.principalToString(p, realm) +
    "   [name-type " + p.type + (nameTypeName ? " NT-" + nameTypeName.replace(/_/g, "-") : "") + "]";
}

function row(name, value, note) {
  return { name: name, value: value === undefined || value === null ? "(absent)" : String(value), note: note || null };
}

function etypeListOf(ids) {
  return (ids || []).map(function (id) {
    return id + " " + kcrypto.etypeName(id) + (kcrypto.isSupportedEtype(id) ? "" : " (not performed here)");
  }).join("\n");
}
...
async function tryDecrypt(encPart, usage, keys, what) {
  var candidates = (keys || []).filter(function (k) { return k.etype === encPart.etype; });
  if (!candidates.length) {
    var haveOther = (keys || []).length > 0;
    return {
      ok: false,
      note: haveOther
        ? "Encrypted with " + encPart.etypeName + " (etype " + encPart.etype + "), and none of the keys " +
  ...
  try {
    ...
    return { ok: false, note: "This build cannot decrypt " + encPart.etypeName + ": " + err.message };
  }
  for (var i = 0; i < candidates.length; i++) {
    try {
      ...
      // none works, because "the key you gave is not this key" is a real answer.
      log.debug("key " + (candidates[i].label || i) + " did not decrypt: " + err.message);
    }
  }
  return {
    ok: false,
    note: candidates.length + " key(s) of the right type were tried at key usage " + usage +
      " and none decrypted this. Either the key is not this principal's, or its kvno is stale."
  };
}
  ...
  return {
    ...
    rows: [
      ...
      row("kvno", d.kvno === null ? "(absent)" : d.kvno,
          d.kvno === null ? null : "the key version — a stale one is KRB_AP_ERR_BADKEYVER"),
...
async function describeTicket(ticket, keys, problems) {
  var section = {
    title: "Ticket",
    note: "Opaque to its holder: encrypted under the SERVICE's long-term key. Supply that key " +
    ...
    sections: [describeEncryptedData(ticket.encPart, "Ticket enc-part (EncTicketPart)")]
  };
  if (ticket.tktVno !== 5) {
    problems.push("The ticket's tkt-vno is " + ticket.tktVno + ", not 5.");
  }
  var attempt = await tryDecrypt(ticket.encPart, kcrypto.KEY_USAGE.KDC_REP_TICKET, keys,
    "Supply the service's key to read the session key, the client name and the PAC.");
  if (attempt.ok) {
    try {
      var part = msgs.readEncTicketPart(attempt.plain);
      section.sections.push(await describeEncTicketPart(part, attempt.usedKey, problems, keys));
    } catch (e) {
      section.sections.push({ title: "EncTicketPart", rows: [row("decrypted, but", e.message)] });
    }
  } else {
    section.sections[0].rows.push(row("decryption", "not attempted or not possible", attempt.note));
  }
  return section;
}
...
async function describeEncTicketPart(p, usedKey, problems, keys) {
  var rows = [
    row("flags", msgs.ticketFlagNames(p.flags).join(", ") || "(none)",
        "bits " + p.flags.join(", ")),
    row("key", p.key.etypeName + ", " + hexOf(p.key.key), "the SESSION key — a credential in itself"),
    row("crealm", p.crealm),
  ...
  if (p.authorizationData) {
    rows.push(row("authorization-data", p.authorizationData.map(function (ad) {
      return kpac.adTypeName(ad.type) + " (" + prim.toBytes(ad.data).length + " bytes)";
    }).join(", "), "the top-level elements only; the PAC is nested inside AD-IF-RELEVANT"));

    var found = kpac.findPacs(p.authorizationData);
    if (!found.length) {
      rows.push(row("PAC", "none",
        "no AD-WIN2K-PAC in here. A ticket from Active Directory carries one unless the account " +
        "has USER_NO_AUTH_DATA_REQUIRED set or the client asked for none — and a service that " +
        "authorizes on groups has nothing to read without it"));
    } else if (found.length > 1) {
      problems.push("This ticket carries " + found.length + " PACs. A ticket has one; the extra " +
        "ones are a place to hide a different set of groups.");
    }
    // The key that opened this ticket IS the service's long-term key by definition, so
    // it is exactly the key the server signature is made with — which is why the PAC's
    // signatures can be checked at all here without asking for anything more.
    var pool = (keys || []).slice();
    if (usedKey && pool.indexOf(usedKey) === -1) pool.push(usedKey);
    for (var i = 0; i < found.length; i++) {
      // usedKey is passed as the ASSERTED service key, not merely thrown into the
      // pool: a server signature that fails against the key which just decrypted
      // this ticket is a real finding, while the KDC signature failing against a pool
      // of unlabelled keys is only ever "you do not have the krbtgt key".
  ...
  if (p.flags.indexOf(msgs.TICKET_FLAG.OK_AS_DELEGATE) !== -1) {
    rows.push(row("note", "ok-as-delegate is set",
      "the KDC is telling the client this service may be trusted with delegated credentials"));
  }
  return {
    title: "EncTicketPart (decrypted)",
    note: usedKey ? "Decrypted with " + (usedKey.label || "a supplied key") + "." : null,
    rows: rows,
...
async function describePac(found, keys, problems, serviceKey) {
  ...
  try {
    ...
    // A PAC that will not parse at all is content, not an exception: the reader wants
    // to know that, and to see how big the thing was.
    return {
  ...
  var section = {
    ...
    note: "Found at " + found.path + ". " + found.bytes.length + " bytes in " + parsed.cBuffers +
      " buffer(s). This is what a Windows service reads to decide what the holder may do — " +
      "everything from here down is little-endian C structures and NDR, not DER.",
    rows: [
      row("buffers", parsed.buffers.map(function (b) {
        return b.type + " " + b.typeName + " — " + b.size + " bytes at offset " + b.offset;
      }).join("\n")),
      row("PACTYPE Version", parsed.version, parsed.version === 0 ? null : "[MS-PAC] requires 0")
    ],
    sections: []
  };

  var logon = kpac.bufferOfType(parsed, kpac.TYPE.LOGON_INFO);
  if (logon && logon.parsed) section.sections.push(describeLogonInfo(logon.parsed, problems));
  else if (logon) {
  ...
  if (client && client.parsed) {
    section.sections.push({
      ...
      note: "Used to check the PAC belongs to this ticket's client rather than to another one.",
      rows: [
        row("name", client.parsed.name),
        row("ClientId", client.parsed.clientId.text,
          "the INITIAL TGT's authentication time, which is the same across every service " +
  ...
  if (upn && upn.parsed) {
    ...
    section.sections.push({
      ...
      rows: [
        ...
        row("SAM name", u.samName, u.extended ? null : "only present when the S flag is set"),
  ...
  if (requestor && requestor.parsed) {
    section.sections.push({
      title: "PAC requestor",
      note: "The SID of the account that ASKED for this ticket. In a delegation flow that is " +
        "not the account the ticket is for, which is the whole point of the buffer.",
      rows: [row("SID", requestor.parsed.sid.text +
        (requestor.parsed.name ? "  (" + requestor.parsed.name + ")" : ""))]
    });
  }

  // Anything present that this codec does not decode, listed rather than dropped.
  ...
  if (undecoded.length) {
    section.sections.push({
      ...
      note: "Present in the PAC and not read by this codec. Listed rather than dropped, because " +
  ...
  section.sections.push(await describePacSignatures(parsed, keys, problems, serviceKey));
...
function describeLogonInfo(info, problems) {
  var rows = [
    row("account", info.effectiveName, info.fullName ? "full name: " + info.fullName : null),
    row("account SID", info.userSid,
      "assembled from LogonDomainId + UserId (" + info.userId + ") — a PAC never carries it whole"),
    row("primary group SID", info.primaryGroupSid, "RID " + info.primaryGroupId +
      (kpac.ridName(info.primaryGroupId) ? ", " + kpac.ridName(info.primaryGroupId) : "")),
    ...
    row("groups (" + info.groups.length + ")", info.groups.length
      ? info.groups.map(function (g) {
        return (info.logonDomainId ? info.logonDomainId.text + "-" : "RID ") + g.relativeId +
          (g.name ? "  (" + g.name + ")" : "") + "   [" + g.attributeNames.join(", ") + "]";
      }).join("\n")
      : "(none)",
      "groups in the ACCOUNT's domain, given as RIDs — the SID is the domain SID plus the RID")
  ];
  ...
  if (info.extraSids.length) {
    rows.push(row("extra SIDs (" + info.extraSids.length + ")",
      ...
      "SIDs from outside the account's domain, and the well-known ones that record HOW the " +
  ...
  if (info.resourceGroups.length) {
    rows.push(row("resource groups (" + info.resourceGroups.length + ")",
      info.resourceGroups.map(function (g) {
        return (info.resourceGroupDomainSid ? info.resourceGroupDomainSid.text + "-" : "RID ") +
          g.relativeId + (g.name ? "  (" + g.name + ")" : "") + "   [" +
          g.attributeNames.join(", ") + "]";
      }).join("\n"),
      "domain-local groups in the RESOURCE's domain, which is a different domain SID from the " +
      "account's"));
  }
  if (info.userSessionKey && !prim.toBytes(info.userSessionKey).every(function (b) { return b === 0; })) {
    rows.push(row("UserSessionKey", hexOf(info.userSessionKey),
      "[MS-PAC] requires this to be zero for Kerberos — it is an NTLM field"));
    problems.push("The PAC's UserSessionKey is not zero. [MS-PAC] section 2.5 requires zero for " +
      "anything other than NTLM.");
  }

  // Each of these parses fine and changes how a real service behaves, which is the
  // definition of a finding here rather than a note.
  info.notes.forEach(function (n) { problems.push("PAC logon information: " + n); });

  return {
    title: "Logon information (KERB_VALIDATION_INFO)",
    note: "NDR-encoded ([MS-RPCE] section 2.2.6), which is why this one buffer starts with " +
      "01 10 08 00 cc cc cc cc. These groups are what a Windows service authorizes on.",
    rows: rows
  };
}
...
async function describePacSignatures(parsed, keys, problems, serviceKey) {
  ...
  var rows = results.map(function (r) {
    var state = r.verified === true ? "verified" : r.verified === false ? "DOES NOT VERIFY"
      : "not checked";
    return row(r.name, state + " — " + r.signatureTypeName, r.note);
  });
  if (!results.length) {
    rows.push(row("signatures", "none present",
      "a PAC inside a ticket always carries at least the server and KDC signatures"));
  }
  // A signature that failed against a key we KNOW is the right one is a finding. One
  // that no unlabelled key in the pool happened to verify is not — the reader has
  // simply not supplied the krbtgt key, which on a service ticket they cannot have,
  // and reporting that as tampering would cry wolf on every paste.
  //
  // Keyed off `roleAsserted` rather than off the wording of the note: matching prose
  // is how this check silently stopped firing once the note was reworded.
  results.forEach(function (r) {
    if (r.verified === false && r.roleAsserted) {
      problems.push("The PAC's " + r.name + " does not verify against the " + r.role +
        " — the very key that decrypted this ticket. Either the PAC has been altered since it " +
        "was signed, or it was not signed for this service.");
    }
  });
  return {
    title: "PAC signatures",
    note: "Four signatures over four DIFFERENT things, with two different keys. A service holds " +
      "only its own key, so it can check the server signature and nothing else — and altering " +
      "the PAC's contents leaves the KDC signature intact, because that one covers only the " +
      "server signature's bytes. That is why the extended KDC signature exists.",
    rows: rows
  };
}
...
async function describeKdcReq(req, input, keys, problems) {
  ...
  var doc = {
    ...
    summary: (isTgs ? "A ticket-granting request" : "An authentication-service request") +
      " for " + msgs.principalToString(body.sname || { name: ["(no sname)"] }, body.realm) +
  ...
  req.padata.forEach(function (pa) {
    padataRows.push(row(pa.typeName + " (" + pa.type + ")", prim.toBytes(pa.value).length + " bytes"));
  });
  doc.sections.push({
    title: "Pre-authentication (padata)",
    note: req.padata.length ? null
      : (isTgs
          ? "A TGS-REQ with no PA-TGS-REQ cannot be answered — the TGT travels in the padata."
          : "None. An AS-REQ without PA-ENC-TIMESTAMP is what a KDC answers with " +
            "KDC_ERR_PREAUTH_REQUIRED, and that answer is where the salt comes from."),
    rows: padataRows.length ? padataRows : [row("padata", "(none)")]
  });
  if (isTgs && !req.padata.some(function (p) { return p.type === msgs.PA_TYPE.TGS_REQ; })) {
    problems.push("This TGS-REQ carries no PA-TGS-REQ, so it has no TGT and no KDC can answer it.");
  }
  ...
  req.padata.forEach(function (pa) {
    if (pa.type === msgs.PA_TYPE.ENC_TIMESTAMP) {
      try {
        ...
        doc.sections.push(describeEncryptedData(enc, "PA-ENC-TIMESTAMP (encrypted with the client's key)"));
      } catch (e) {
        problems.push("PA-ENC-TIMESTAMP does not decode as EncryptedData: " + e.message);
      }
    }
    if (pa.type === msgs.PA_TYPE.PAC_REQUEST) {
      try {
        doc.sections.push({
          ...
          rows: [row("include-pac", String(msgs.readPaPacRequest(pa.value).includePac),
    ...
    if (pa.type === msgs.PA_TYPE.FOR_USER) {
      try {
        ...
        doc.sections.push({
          ...
          note: "This request asks the KDC for a ticket to ITSELF on behalf of another user — " +
                "protocol transition.",
          rows: [
            row("userName", principalOf(fu.userName, fu.userRealm)),
            row("userRealm", fu.userRealm),
            row("cksum", "type " + fu.cksum.type + ", " + hexOf(fu.cksum.checksum),
              fu.cksum.type === -138 ? "KERB_CHECKSUM_HMAC_MD5, as MS-SFU requires" : null),
  ...
  var bodyRows = [
    row("kdc-options", optionNames.join(", ") || "(none)", "bits " + body.kdcOptions.join(", ")),
    ...
    row("etype", etypeListOf(body.etypes), "in preference order — the KDC picks from this list")
  ];
  if (body.addresses) {
  ...
  if (body.additionalTickets) {
    bodyRows.push(row("additional-tickets", body.additionalTickets.length + " ticket(s)",
      optionNames.indexOf("cname-in-addl-tkt") !== -1
        ? "with cname-in-addl-tkt set this is S4U2Proxy: the evidence ticket being presented"
  ...
  if (body.realm && body.realm !== body.realm.toUpperCase()) {
    problems.push("The realm \"" + body.realm + "\" is not upper case. Realms are case-sensitive on the " +
      "wire and almost every deployment uses upper case; this is the commonest configuration error there is.");
  }
  if (body.etypes && body.etypes.length && body.etypes.every(function (id) { return id === 23; })) {
    problems.push("The only encryption type offered is arcfour-hmac-md5 (RC4). The Windows Server 2025 " +
      "security baseline disables it, so this request will be refused with KDC_ERR_ETYPE_NOSUPP on a " +
      "current domain controller.");
  }
  (body.etypes || []).forEach(function (id) {
    if (id >= 1 && id <= 7) {
      problems.push("Encryption type " + id + " (" + kcrypto.etypeName(id) + ") is a DES type. DES was " +
        "removed from Windows Server 2025 entirely.");
    }
  });
  if (body.till && body.from && body.till <= body.from) {
    problems.push("till is not after from, so the requested validity window is empty " +
      "(KDC_ERR_NEVER_VALID).");
  }
  return doc;
}
...
async function describeKdcRep(rep, keys, problems) {
  ...
  var doc = {
    kind: kind,
    summary: "A ticket for " + msgs.principalToString(rep.ticket.sname, rep.ticket.realm) +
  ...
  if (rep.padata.length) {
    doc.sections.push({
      ...
      rows: rep.padata.map(function (pa) {
        return row(pa.typeName + " (" + pa.type + ")", prim.toBytes(pa.value).length + " bytes");
      })
    });
  }
  ...
  // tried for a TGS-REP because which one applies depends on whether the request
  // carried a subkey, and the reader of a capture does not necessarily know.
  ...
  var usages = isTgs
    ? [kcrypto.KEY_USAGE.TGS_REP_ENCPART_SESSKEY, kcrypto.KEY_USAGE.TGS_REP_ENCPART_SUBKEY]
    : [kcrypto.KEY_USAGE.AS_REP_ENCPART];
  var got = null;
  for (var i = 0; i < usages.length && !got; i++) {
    var attempt = await tryDecrypt(rep.encPart, usages[i], keys,
      isTgs ? "Supply the TGT's session key." : "Supply the client's key, or its password and salt.");
    if (attempt.ok) got = attempt;
    else encSection.rows.push(row("decryption at key usage " + usages[i], "no", attempt.note));
  }
  if (got) {
    try {
      ...
      var rows = [
        row("tagged as", part.taggedAs,
          part.taggedAs === "EncTGSRepPart" && !isTgs
            ? "An AS-REP whose enc-part is tagged EncTGSRepPart. RFC 4120 section 5.4.2 records this and " +
              "requires a client to accept it — this one does."
            : null),
        row("key", part.key.etypeName + ", " + hexOf(part.key.key), "the session key"),
        ...
        row("key-expiration", timeOf(part.keyExpiration),
          part.keyExpiration ? "the password expiry the KDC is reporting" : null),
        row("srealm", part.srealm),
        row("sname", principalOf(part.sname, part.srealm))
      ];
      doc.sections.push({
        title: "Enc" + (isTgs ? "TGS" : "AS") + "RepPart (decrypted)",
        note: "Decrypted with " + (got.usedKey.label || "a supplied key") + " at key usage " + got.usage + ".",
        rows: rows
      });
      if (part.sname && rep.ticket.sname &&
          part.sname.name.join("/") !== rep.ticket.sname.name.join("/")) {
        problems.push("The sname in the enc-part (" + part.sname.name.join("/") + ") does not match the " +
          "ticket's (" + rep.ticket.sname.name.join("/") + "). With canonicalize set this can be a " +
          "referral; otherwise it is wrong.");
      }
    } catch (e) {
      doc.sections.push({ title: "enc-part", rows: [row("decrypted, but does not parse", e.message)] });
    }
  }
  return doc;
}
...
async function describeApReq(r, keys, problems) {
  var doc = {
    kind: "AP-REQ",
    summary: "A client presenting a ticket for " + msgs.principalToString(r.ticket.sname, r.ticket.realm),
    sections: [{
      title: "Message",
      rows: [
        row("pvno", r.pvno),
        row("msg-type", r.msgType + " (AP-REQ)"),
        row("ap-options", msgs.apOptionNames(r.apOptions).join(", ") || "(none)",
          r.apOptions.indexOf(msgs.AP_OPTION.MUTUAL_REQUIRED) !== -1
            ? "mutual-required: the client expects an AP-REP back"
            : "without mutual-required the service never proves its own identity")
      ]
    }]
  };
  ...
  doc.sections.push(encSection);
  var attempt = await tryDecrypt(r.authenticator, kcrypto.KEY_USAGE.AP_REQ_AUTH, keys,
    "Supply the session key from the ticket, which the service gets by decrypting the ticket itself.");
  if (attempt.ok) {
    try {
      ...
      var rows = [
        ...
        row("cname", principalOf(a.cname, a.crealm),
          "must match the ticket's cname or the service answers KRB_AP_ERR_BADMATCH"),
        row("ctime", timeOf(a.ctime), "checked against the service's clock — five minutes on AD"),
        row("cusec", a.cusec),
        row("seq-number", a.seqNumber),
        row("subkey", a.subkey ? a.subkey.etypeName + ", " + hexOf(a.subkey.key) : "(absent)",
          a.subkey ? "the client proposing a per-context key" : null)
      ];
      if (a.cksum) {
        rows.push(row("cksum", "type " + a.cksum.type + ", " + hexOf(a.cksum.checksum),
          a.cksum.type === 0x8003
            ? "checksum type 0x8003 — the GSS-API channel-binding-and-flags structure of RFC 4121, " +
      ...
      doc.sections.push({ title: "Authenticator", rows: [row("decrypted, but does not parse", e.message)] });
    }
  } else {
...
async function describeApRep(r, keys) {
  var doc = {
    kind: "AP-REP",
    summary: "A service proving its identity back to the client (mutual authentication)",
    sections: [
      { title: "Message", rows: [row("pvno", r.pvno), row("msg-type", r.msgType + " (AP-REP)")] },
      describeEncryptedData(r.encPart, "enc-part (under the ticket's session key)")
    ]
  };
  var attempt = await tryDecrypt(r.encPart, kcrypto.KEY_USAGE.AP_REP_ENCPART, keys,
  ...
  if (attempt.ok) {
    ...
    doc.sections.push({
      ...
      rows: [
        row("ctime", timeOf(p.ctime), "echoes the Authenticator's ctime — that echo IS the proof"),
        row("cusec", p.cusec),
        row("subkey", p.subkey ? p.subkey.etypeName + ", " + hexOf(p.subkey.key) : "(absent)",
          p.subkey ? "the acceptor's subkey, which per-message tokens are then keyed from" : null),
...
function describeKrbError(e, problems) {
  var doc = {
    ...
    sections: [{
      ...
      rows: [
        ...
        row("stime", timeOf(e.stime), "the KDC's own clock, which is how skew is measured"),
        row("susec", e.susec),
  ...
  if (e.stime) {
    ...
    doc.sections[0].rows.push(row("skew against this browser",
      ...
      absSkew > 300
        ? "MORE THAN FIVE MINUTES. That alone will produce KRB_AP_ERR_SKEW on a default AD configuration."
        : "within AD's default five-minute tolerance"));
    if (e.errorCode === 37 || absSkew > 300) {
      problems.push("The clock difference between this browser and the KDC is " + skewSeconds +
  ...
  if (e.eDataPaData) {
    ...
    e.eDataPaData.forEach(function (pa) {
      rows.push(row(pa.typeName + " (" + pa.type + ")", prim.toBytes(pa.value).length + " bytes"));
        ...
        try {
          msgs.readEtypeInfo2(pa.value).forEach(function (info) {
            ...
            rows.push(row("  etype " + info.etype + " " + info.etypeName,
              "salt: " + (info.salt === null ? "(none — arcfour is unsalted)" : JSON.stringify(info.salt)) +
              (iterations !== null ? ", iterations: " + iterations : ""),
              "THIS is the salt to use for string-to-key. It is not guessable: AD uses the realm plus " +
              "the sAMAccountName for a user, but a host-shaped string for a computer account."));
          });
        } catch (err) {
    ...
    doc.sections.push({
      title: "e-data",
      note: e.errorCode === 25
        ? "On KDC_ERR_PREAUTH_REQUIRED this is not an error report — it is the information needed to " +
...
async function describe(input, options) {
  ...
  var opts = options || {};
  var parsed = (input instanceof Uint8Array) ? { bytes: input, encoding: "bytes", framing: null }
                                             : parseInput(input);
  ...
  if (!id) {
    ...
    // structural tree is far more useful than "could not parse", and it is how a
    // codec bug in THIS tool becomes visible.
    doc = {
      kind: "unrecognised",
      summary: "These bytes do not begin with a Kerberos [APPLICATION n] tag. Showing their ASN.1 " +
               "structure instead.",
      sections: [],
      tree: null
    };
    try {
      ...
      doc.summary = "These bytes are neither a Kerberos message nor parseable as ASN.1: " + e.message;
    ...
    try {
      if (n === A.AS_REQ || n === A.TGS_REQ) {
        doc = await describeKdcReq(msgs.readKdcReq(bytes), parsed, keys, problems);
        ...
        doc = {
          ...
          sections: [await describeTicket(msgs.readTicket(bytes), keys, problems)]
        };
      } else if (n === A.KRB_CRED) {
        // Somebody has pasted a DELEGATED CREDENTIAL. Worth saying plainly what that is,
        // because it is the one structure here that is a capability rather than a claim: a
        // ticket-granting ticket someone handed to a service so it could act as them.
        var cred = msgs.readKrbCred(bytes);
        doc = {
          kind: "KRB-CRED",
          summary: "A delegated credential carrying " + cred.tickets.length + " forwarded " +
            "ticket(s) — unconstrained delegation",
          sections: [{
            title: "KRB-CRED",
            note: "This is how a client hands its own ticket-granting ticket to a service so " +
                  "that the service can obtain tickets to ANYTHING as that client, for as long " +
                  "as the ticket lives, without the KDC being asked again. It normally travels " +
                  "inside an AP-REQ Authenticator's 0x8003 checksum rather than on its own.",
            rows: [
              row("pvno", cred.pvno),
              row("msg-type", cred.msgType + " (KRB-CRED)"),
              row("tickets", cred.tickets.map(function (t) {
                return principalOf(t.sname, t.realm);
              }).join("\n"), "each one's SESSION KEY is in the encrypted part below — a ticket " +
                "without its key is opaque, and that separation is what makes this safe to put " +
                "in a checksum field")
            ],
            sections: [describeEncryptedData(cred.encPart, "KRB-CRED enc-part (EncKrbCredPart)")]
          }]
        };
        // The enc-part is at key usage 14 under the AP exchange's subkey, or the ticket's
        // session key when no subkey was sent — not under any long-term key, so a reader
        // usually cannot open it and saying which key is needed is the useful part.
        var credAttempt = await tryDecrypt(cred.encPart, kcrypto.KEY_USAGE.KRB_CRED_ENCPART, keys,
          "Encrypted at key usage 14 under the AP exchange's SUBKEY, or the presented ticket's " +
          "session key when none was sent. Supply that to see the forwarded ticket's own key.");
        if (credAttempt.ok) {
          try {
            ...
            doc.sections[0].sections.push({
              ...
              rows: [
                ...
                row("s-address", credPart.sAddress
                  ? "type " + credPart.sAddress.type + " " + prim.toHex(credPart.sAddress.address)
                  : null),
                row("r-address", credPart.rAddress
                  ? "type " + credPart.rAddress.type + " " + prim.toHex(credPart.rAddress.address)
                  : null)
              ],
                ...
                return {
                  ...
                  rows: [
                    ...
                    row("key", info.key.etypeName + ", " + hexOf(info.key.key),
                        "the forwarded ticket's session key — holding this and the ticket IS " +
                        "being that client"),
                    row("flags", info.flags ? msgs.ticketFlagNames(info.flags).join(", ") : null,
                        info.flags && info.flags.indexOf(msgs.TICKET_FLAG.FORWARDED) !== -1
                          ? "flagged `forwarded`, which is the record a receiving service has " +
                            "that these credentials were handed over rather than presented by " +
                            "their owner"
                          : null),
          ...
          doc.sections[0].sections[0].rows.push(
            row("decryption", "not attempted or not possible", credAttempt.note));
        }
      } else if (n === A.AUTHENTICATOR) {
        var a = msgs.readAuthenticator(bytes);
        doc = {
          kind: "Authenticator",
          summary: "A decrypted Authenticator for " + msgs.principalToString(a.cname, a.crealm),
          sections: [{
            title: "Authenticator",
            rows: [
              row("crealm", a.crealm), row("cname", principalOf(a.cname, a.crealm)),
              row("ctime", timeOf(a.ctime)), row("cusec", a.cusec),
              row("seq-number", a.seqNumber),
              row("cksum", a.cksum ? "type " + a.cksum.type + ", " + hexOf(a.cksum.checksum) : "(absent)")
            ]
          }]
        };
        ...
        doc = {
          ...
          sections: [await describeEncTicketPart(msgs.readEncTicketPart(bytes), null,
            problems, keys)]
        };
        ...
        doc = {
          kind: p.taggedAs,
          summary: "A decrypted " + p.taggedAs + " for " + msgs.principalToString(p.sname, p.srealm),
          sections: [{
            title: p.taggedAs,
            rows: [
              ...
              row("nonce", p.nonce),
              row("flags", msgs.ticketFlagNames(p.flags).join(", ") || "(none)"),
              row("authtime", timeOf(p.authtime)), row("endtime", timeOf(p.endtime)),
              row("srealm", p.srealm), row("sname", principalOf(p.sname, p.srealm))
            ]
          }]
        };
      } else {
        doc = {
          kind: id.name,
          summary: "A " + id.name + ". This build does not decode that message yet; its ASN.1 " +
                   "structure is below.",
          sections: [],
      ...
      // interesting case there is: it is either malformed or this codec is wrong.
      // Both deserve the structure alongside the error.
      doc = {
        kind: id.name + " (does not parse)",
        summary: "These bytes announce themselves as a " + id.name + " but do not parse: " + e.message,
        sections: [],
        tree: null
      };
      problems.push(e.message);
      try {
        ...
        problems.push("The ASN.1 structure could not be shown either: " + e2.message);
      }
    }
  }
  ...
  log.debug("Leaving describe(). kind=" + doc.kind + ", problems=" + doc.problems.length);
  return doc;
}

// A key list from a password, for the AS-REP case where that is all a reader has.
// Every supported etype is derived, because the reader usually does not know
// which one the KDC chose — and the salt is the part they will get wrong, so it
// is a required argument rather than something guessed here.
async function keysFromPassword(password, salt, etypes) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    out.push({
      ...
      label: "the password with salt " + JSON.stringify(salt || "") + " as " + e.name
    });
  }
  return out;
}
