...
function reader(bytes, bigEndian) {
  ...
  return {
    ...
    u8: function () {
      if (at + 1 > b.length) throw new Error("krb5: keytab truncated at offset " + at);
      return b[at++];
    },
    u16: function () {
      if (at + 2 > b.length) throw new Error("krb5: keytab truncated at offset " + at);
      var v = bigEndian ? (b[at] << 8) | b[at + 1] : (b[at + 1] << 8) | b[at];
      at += 2;
      return v;
    },
    i32: function () {
      if (at + 4 > b.length) throw new Error("krb5: keytab truncated at offset " + at);
      var v = bigEndian
    ...
    countedString: function () {
      var n = this.u16();
      if (at + n > b.length) throw new Error("krb5: keytab string of " + n + " bytes runs past the end");
      var s = "";
    ...
    countedBytes: function () {
      var n = this.u16();
      if (at + n > b.length) throw new Error("krb5: keytab key of " + n + " bytes runs past the end");
...
function parseKeytab(bytes) {
  ...
  if (b.length > MAX_KEYTAB_BYTES) {
    throw new Error("krb5: refusing to parse a " + b.length + "-byte keytab (limit " + MAX_KEYTAB_BYTES + ")");
  }
  if (b.length < 4) throw new Error("krb5: too short to be a keytab");
  if (b[0] !== 0x05) {
    throw new Error("krb5: not a keytab — it must start with 0x05, this starts with 0x" +
      b[0].toString(16) + ". (A base64 keytab pasted as text is the usual cause.)");
  }
  var version = b[1];
  if (version !== 0x01 && version !== 0x02) {
    throw new Error("krb5: keytab format version 0x05" + ("0" + version.toString(16)).slice(-2) +
  ...
  while (r.remaining >= 4 && entries.length + deleted < MAX_ENTRIES) {
    ...
    if (entryEnd > b.length) {
      ...
      // truncated file — it is the byte order being wrong, because the same four
      // bytes read the other way round are a sensible number. Saying so here is
      // the difference between a five-minute fix and an afternoon: the header is
      // the ONLY thing that declares the order, and a keytab copied between
      // architectures or produced by a non-MIT tool is exactly how it goes wrong.
      var hint = "";
      if (size > 0xffff) {
        hint = " — a size that large in a " + b.length + "-byte file almost always means the BYTE " +
          "ORDER is wrong. This file declares version 0x05" +
          ("0" + version.toString(16)).slice(-2) + ", i.e. " + (bigEndian ? "big" : "little") +
          "-endian; read the other way those four bytes are " +
          (bigEndian
            ? ((b[start + 3] << 24) | (b[start + 2] << 16) | (b[start + 1] << 8) | b[start]) >>> 0
            : ((b[start] << 24) | (b[start + 1] << 16) | (b[start + 2] << 8) | b[start + 3]) >>> 0) +
          ".";
      }
      throw new Error("krb5: keytab entry at offset " + start + " claims " + size +
    ...
    if (numComponents < 0 || numComponents > 16) {
      throw new Error("krb5: keytab entry at offset " + start + " claims " + numComponents +
        " name components, which is not credible — is the byte order right? " +
        "(this file declares version 0x05" + ("0" + version.toString(16)).slice(-2) + ")");
    }
    ...
    // vno32 is present only if there is room for it inside this entry — which is
    // how the format expresses "optional" with no flag.
    var kvno = vno8;
  ...
  log.debug("Leaving parseKeytab(). entries=" + entries.length + ", deleted slots=" + deleted);
  return { version: 0x0500 | version, bigEndian: bigEndian, entries: entries, deletedSlots: deleted };
}

// The keys from a keytab in the shape the decoder's decryption wants.
function keysFromKeytab(parsed) {
  return (parsed.entries || []).filter(function (e) { return e.supported; }).map(function (e) {
    return {
      etype: e.etype,
      key: e.key,
      label: "keytab " + e.principal + " (kvno " + e.kvno + ", " + e.etypeName + ")"
    };
  });
}
