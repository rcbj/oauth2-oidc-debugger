...
var KRB5_MECH_OID_DER = new Uint8Array([0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x12, 0x01, 0x02, 0x02]);
...
function be64(n) {
  ...
  // any real exchange uses; the top four bytes are written explicitly rather than
  // assumed zero.
...
function readBe64(bytes, at) {
  var hi = ((bytes[at] << 24) | (bytes[at + 1] << 16) | (bytes[at + 2] << 8) | bytes[at + 3]) >>> 0;
  var lo = ((bytes[at + 4] << 24) | (bytes[at + 5] << 16) | (bytes[at + 6] << 8) | bytes[at + 7]) >>> 0;
...
function buildGssChecksum(options) {
  ...
  var bindings = opts.channelBindings ? toBytes(opts.channelBindings) : new Uint8Array(16);
  if (bindings.length !== 16) {
    throw new Error("krb5: GSS channel bindings must be exactly 16 bytes, got " + bindings.length);
  }
  var parts = [le32(16), bindings, le32(flags)];
  if (opts.delegation) {
    var cred = toBytes(opts.delegation);
    if (!(flags & GSS_FLAG.DELEG)) {
      throw new Error("krb5: a delegated credential was supplied but GSS_C_DELEG_FLAG is not set; " +
...
function parseGssChecksum(bytes) {
  var b = toBytes(bytes);
  if (b.length < 24) {
    throw new Error("krb5: a 0x8003 checksum is at least 24 bytes (4 + 16 + 4), got " + b.length);
  }
  var declared = readLe32(b, 0);
  if (declared !== 16) {
    // The commonest symptom of a big-endian encoder: 16 written the other way is
    // 0x10000000, i.e. 268435456.
    throw new Error("krb5: the 0x8003 checksum declares a bindings length of " + declared +
      " rather than 16" + (declared === 0x10000000
        ? " — 16 written BIG-endian. Every integer in this structure is little-endian, unlike the " +
          "rest of Kerberos."
        : "."));
  }
  ...
  var out = {
    ...
    hasChannelBindings: !b.subarray(4, 20).every(function (v) { return v === 0; }),
    flags: flags,
  ...
  if ((flags & GSS_FLAG.DELEG) && b.length >= at + 4) {
    ...
    if (b.length >= at + dlgth) {
      out.delegation = { option: dlgOpt, credential: b.subarray(at, at + dlgth) };
      at += dlgth;
    }
  }
...
function decodeInitialContextToken(bytes) {
  log.debug("Entering decodeInitialContextToken(). bytes=" + toBytes(bytes).length);
  var b = toBytes(bytes);
  if (!b.length || b[0] !== 0x60) {
    throw new Error("krb5: not a GSS InitialContextToken — it must begin with 0x60 (" +
      "[APPLICATION 0]), this begins with 0x" +
      (b.length ? b[0].toString(16) : "nothing") +
      (b.length && b[0] === 0x6e
        ? ". 0x6e is a bare AP-REQ: a Windows service expects it WRAPPED in this token, not on its own."
        : "."));
  }
  var outer = asn1.readTlv(b, 0);
  var value = outer.value;
  if (value.length < KRB5_MECH_OID_DER.length) {
    throw new Error("krb5: the GSS token is too short to carry a mechanism OID");
  }
  ...
  if (!prim.equalConstantTime(oid, KRB5_MECH_OID_DER)) {
    ...
    // caller negotiated rather than using Kerberos directly, which is a different
    // (and not yet implemented) mechanism.
    var spnego = new Uint8Array([0x06, 0x06, 0x2b, 0x06, 0x01, 0x05, 0x05, 0x02]);
    ...
    throw new Error("krb5: this GSS token names " +
      (isSpnego ? "the SPNEGO mechanism (1.3.6.1.5.5.2), which this build does not implement — " +
  ...
  if (rest.length < 2) throw new Error("krb5: the GSS token carries no token id");
  var id = [rest[0], rest[1]];
  var name = Object.keys(TOK_ID).filter(function (k) {
    return TOK_ID[k][0] === id[0] && TOK_ID[k][1] === id[1];
  })[0] || null;
  log.debug("Leaving decodeInitialContextToken(). tokId=" + (name || prim.toHex(new Uint8Array(id))));
  return {
    tokId: id,
    tokIdName: name,
...
function usageFor(role, purpose) {
  if (role !== "initiator" && role !== "acceptor") {
    throw new Error('krb5: the GSS role must be "initiator" or "acceptor", got ' + JSON.stringify(role));
  }
  if (purpose === "sign") {
    return role === "initiator" ? kcrypto.KEY_USAGE.GSS_INITIATOR_SIGN : kcrypto.KEY_USAGE.GSS_ACCEPTOR_SIGN;
  }
  if (purpose === "seal") {
    return role === "initiator" ? kcrypto.KEY_USAGE.GSS_INITIATOR_SEAL : kcrypto.KEY_USAGE.GSS_ACCEPTOR_SEAL;
  }
  throw new Error('krb5: the GSS purpose must be "sign" or "seal"');
}

// The five-flag byte of a per-message token header: bit 0 says the sender is the
// acceptor, bit 1 that the message is sealed, bit 2 that an acceptor subkey was used.
function messageFlags(role, sealed, acceptorSubkey) {
  return (role === "acceptor" ? 0x01 : 0x00) | (sealed ? 0x02 : 0x00) | (acceptorSubkey ? 0x04 : 0x00);
}
...
async function getMic(options) {
  ...
  var header = concat([
    ...
    new Uint8Array([0xff, 0xff, 0xff, 0xff, 0xff]),      // five octets of filler
    be64(opts.sequenceNumber || 0)
  ]);
  // The checksum covers the message AND the header — so the flags and the sequence
  // number are protected rather than merely present.
...
async function verifyMic(options) {
  ...
  if (token.length < 16) throw new Error("krb5: a MIC token is at least 16 bytes, got " + token.length);
...
// APPENDED to the plaintext before encryption, then the ciphertext is rotated right
// by RRC bytes. So the header appears twice — once in clear at the front, once
// encrypted at the back — and a receiver compares them. That is what stops the
// clear header being altered.
async function wrap(options) {
  ...
  var ciphertext = await profile.encrypt(opts.key, usageFor(opts.role, "seal"), plaintext);
...
async function unwrap(options) {
  ...
  var opts = options || {};
  var token = toBytes(opts.token);
  if (token.length < 16) throw new Error("krb5: a Wrap token is at least 16 bytes, got " + token.length);
  ...
  var flags = header[2];
  if (!(flags & 0x02)) {
    throw new Error("krb5: this Wrap token is not sealed (its flags say integrity only), so there " +
  ...
  var plaintext = await profile.decrypt(opts.key, usageFor(senderRole, "seal"), unrotated);
  if (plaintext.length < 16 + ec) {
    throw new Error("krb5: the unwrapped plaintext is too short to contain its own header");
  }
  // The header appears twice, and comparing them is what protects the clear copy.
  var trailing = plaintext.subarray(plaintext.length - 16);
  // RRC and EC in the trailing copy are zero on the wire per RFC 4121; compare the
  // fields that must match rather than the whole sixteen bytes.
  var mismatch = trailing[0] !== header[0] || trailing[1] !== header[1] || trailing[2] !== header[2];
  if (mismatch) {
    throw new Error("krb5: the Wrap token's clear header does not match the encrypted copy inside " +
