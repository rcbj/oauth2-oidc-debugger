...
// A sequence number for the AP exchange. RFC 4120 wants it unpredictable, and it is
// a UInt32 on the wire — the top bit is left clear because some implementations
// treat the field as signed and a negative value then reads as out of order.
...
//
// `tgt` is what the AS exchange produced: { ticket, sessionKey, etype, client, realm }.
async function buildTgsReq(options) {
  log.debug("Entering buildTgsReq().");
  var opts = options || {};
  var tgt = opts.tgt;
  if (!tgt || !tgt.ticket || !tgt.sessionKey) {
    throw new Error("krb5: a TGS-REQ needs a TGT with its session key — that key is what signs the " +
  ...
  // The body, encoded ONCE. These exact bytes are what the checksum covers and what
  // goes on the wire; re-encoding between the two is how a checksum stops matching
  // for no visible reason.
  var body = msgs.encKdcReqBody({
    kdcOptions: opts.kdcOptions || [msgs.KDC_OPTION.FORWARDABLE, msgs.KDC_OPTION.RENEWABLE],
    realm: realm,
    sname: opts.sname,
    till: opts.till || new Date(Date.now() + 10 * 3600 * 1000),
    nonce: nonce,
    etypes: opts.etypes || [tgt.etype],
    // A ticket bound to the addresses it may be used from. AD issues addressless tickets
    // and so does this by default — but the field has to be PASSED THROUGH, or an option
    // the caller sets is silently dropped and the ticket comes back unbound with no error
    // anywhere to say so.
  ...
  var checksum = await profile.checksum(tgt.sessionKey, kcrypto.KEY_USAGE.TGS_REQ_AUTH_CKSUM, body);
  ...
  var authenticator = msgs.encAuthenticator({
    ...
    seqNumber: opts.sequenceNumber === undefined ? randomSequenceNumber() : opts.sequenceNumber
  });
  ...
  var apReq = msgs.encApReq({
    ...
    authenticator: {
      etype: tgt.etype,
      cipher: await profile.encrypt(tgt.sessionKey, kcrypto.KEY_USAGE.TGS_REQ_AUTH, authenticator)
    }
  });

  var request = msgs.encKdcReq({
    msgType: msgs.MSG_TYPE.TGS_REQ,
    padata: [{ type: msgs.PA_TYPE.TGS_REQ, value: apReq }].concat(opts.extraPadata || []),
    // The SAME bytes. encKdcReq takes `raw` and uses it verbatim.
    reqBody: { raw: body }
  });
  log.debug("Leaving buildTgsReq(). bytes=" + request.length + ", nonce=" + nonce);
  // `sname` is returned so the caller can hand it back to readTgsRep as
  // `requestedSname`: a referral is only detectable by comparing the reply's sname
  // with what was ASKED FOR, and a caller that has to remember that separately is a
  // caller that will forget.
  return { request: request, nonce: nonce, subkey: subkey, body: body, sname: opts.sname };
}

// Read a TGS-REP and produce the service ticket.
//
// The enc-part is at key usage 8 under the TGT session key, or 9 under the
// Authenticator's subkey when one was sent. Both are tried in the right order and
// which one worked is reported, because a client that guesses wrong sees only an
// integrity failure.
async function readTgsRep(options) {
  ...
  if (response.kind !== "TGS-REP") {
    throw new Error("krb5: expected a TGS-REP, the KDC answered " + response.kind);
  }
  ...
  var attempts = [];
  if (opts.subkey) {
    attempts.push({ key: opts.subkey.key, usage: kcrypto.KEY_USAGE.TGS_REP_ENCPART_SUBKEY,
                    label: "the Authenticator's subkey (key usage 9)" });
  }
  attempts.push({ key: tgt.sessionKey, usage: kcrypto.KEY_USAGE.TGS_REP_ENCPART_SESSKEY,
  ...
  for (var i = 0; i < attempts.length && !part; i++) {
    try {
      part = msgs.readEncKdcRepPart(
        await profile.decrypt(attempts[i].key, attempts[i].usage, rep.encPart.cipher));
      used = attempts[i];
    } catch (e) {
  ...
  if (!part) {
    throw new Error("krb5: the TGS-REP's enc-part would not decrypt. Tried " + failures.join("; ") +
      ". The KDC uses key usage 9 when the request carried a subkey and 8 when it did not, so a " +
      "client that always tries one of them fails half the time.");
  }

  if (opts.nonce !== undefined && part.nonce !== opts.nonce) {
    throw new Error("krb5: the TGS-REP's nonce is " + part.nonce + " but the request sent " +
      opts.nonce + ". This is not an answer to that request and may be a replay.");
  }
  ...
  var got = part.sname;
  if (asked && got && (asked.name || []).join("/") !== (got.name || []).join("/")) {
    var toRealm = (got.name || []).length > 1 && got.name[0] === "krbtgt" ? got.name[1] : null;
    referral = {
      requested: msgs.principalToString(asked),
      issued: msgs.principalToString(got, part.srealm),
      toRealm: toRealm,
      note: toRealm
        ? "This is a REFERRAL, not the ticket that was asked for. " + part.srealm + " has no " +
          msgs.principalToString(asked) + " and has issued a ticket-granting ticket for " +
          toRealm + " instead, sealed with the trust key those two realms share. Present THIS " +
          "ticket to " + toRealm + "'s KDC and ask it for the service again."
        : "The reply names " + msgs.principalToString(got, part.srealm) + ", which is not the " +
          msgs.principalToString(asked) + " that was requested, and is not a krbtgt principal " +
          "either. The KDC has answered a different question from the one asked."
    };
    log.info("krb5: the TGS-REP is a referral to " + (toRealm || "an unexpected principal") + ".");
  }
...
async function buildApReq(options) {
  ...
  if (!ticket || !ticket.ticket || !ticket.sessionKey) {
    throw new Error("krb5: an AP-REQ needs a service ticket and its session key");
  }
  var profile = kcrypto.etypeById(ticket.etype);
  var mutual = opts.mutual !== false;
  var flags = (opts.gssFlags || [gss.GSS_FLAG.INTEG, gss.GSS_FLAG.CONF]).slice();
  if (mutual && flags.indexOf(gss.GSS_FLAG.MUTUAL) === -1) flags.push(gss.GSS_FLAG.MUTUAL);
  if (opts.delegation && flags.indexOf(gss.GSS_FLAG.DELEG) === -1) flags.push(gss.GSS_FLAG.DELEG);
  ...
  // sub-second part travels separately in cusec. A caller that later compares the
  // AP-REP's echo against a millisecond-precision Date will NEVER match: the echo is
  // correct and the comparison is wrong. So the truncated value is what this
  // function reports as `ctime`, and `cusec` is reported beside it.
  ...
  var sequenceNumber = opts.sequenceNumber === undefined ? randomSequenceNumber() : opts.sequenceNumber;
  ...
  var apReq = msgs.encApReq({
    ...
    authenticator: {
      etype: ticket.etype,
      // Key usage 11 for an AP-REQ Authenticator, under the ticket's session key.
      cipher: await profile.encrypt(ticket.sessionKey, kcrypto.KEY_USAGE.AP_REQ_AUTH, authenticator)
    }
  });

  log.debug("Leaving buildApReq(). mutual=" + mutual + ", seq=" + sequenceNumber);
  return {
    apReq: apReq,
...
// ctime and cusec, encrypted under the ticket's session key. Only something holding
// the service's long-term key could have decrypted the ticket to learn that session
// key, so a correct echo is the service's identity. A client that does not CHECK
// the echo has asked for mutual authentication and not performed it.
async function readApRep(options) {
  log.debug("Entering readApRep().");
  var opts = options || {};
  var bytes = opts.reply;

  // A service may answer with a GSS-wrapped AP-REP, or with a KRB-ERROR inside the
  // same wrapper. Unwrap when wrapped, and be explicit about which arrived.
  var wrapped = null;
  ...
  if (identified && identified.applicationNumber === msgs.APPLICATION.KRB_ERROR) {
  ...
  var part = msgs.readEncApRepPart(
    await profile.decrypt(opts.ticket.sessionKey, kcrypto.KEY_USAGE.AP_REP_ENCPART,
      apRep.encPart.cipher));

  // The echo, compared at the precision it was actually sent at.
  //
  // KerberosTime carries no fractional seconds, so ctime on the wire is truncated to
  // the second and the sub-second part is in cusec — RFC 4120 has the AP-REP echo
  // BOTH. Comparing a millisecond-precision Date here fails against a perfectly
  // correct service, which is a false accusation of the worst kind: it tells the
  // user their service is not who it says it is.
  ...
  if (!timeEchoed || !usecEchoed) {
    return {
      ok: false,
      mutualOk: false,
      reason: "the AP-REP echoes ctime " + part.ctime.toISOString() + "/" + part.cusec +
        " but the Authenticator sent " +
        (sentSecond ? sentSecond.toISOString() : "(unknown)") + "/" +
        (opts.sentCusec === undefined ? "(not checked)" : opts.sentCusec) +
        ". That echo is the whole of the service's proof of identity, so this context is NOT " +
  ...
  return {
    ok: true,
    mutualOk: true,
    // The acceptor's subkey, when it sent one, is what per-message tokens are then
    // keyed from — not the ticket's session key.
...
// acceptor's subkey if it offered one, else the initiator's, else the session key.
// Getting this wrong is a token the far end cannot verify.
function perMessageKey(context) {
  if (context.acceptorSubkey) {
    return { key: context.acceptorSubkey.key, etype: context.acceptorSubkey.etype,
             which: "the acceptor's subkey", acceptorSubkey: true };
  }
  if (context.subkey) {
    return { key: context.subkey.key, etype: context.subkey.etype,
             which: "the initiator's subkey", acceptorSubkey: false };
  }
  return { key: context.sessionKey, etype: context.etype,
           which: "the ticket's session key", acceptorSubkey: false };
}
...
// The S4UByteArray of [MS-SFU] section 2.2.1, whose exact composition is not guessable:
// the name TYPE as four little-endian bytes, then each name component, then the realm,
// then the auth-package — concatenated with no separators and no terminators.
function s4uByteArray(userName, userRealm, authPackage) {
  ...
  (userName.name || []).forEach(function (component) { parts.push(prim.utf8(component)); });
...
// **KERB_CHECKSUM_HMAC_MD5 always**, whatever the session key's own etype, so an AES
// session key is fed to an HMAC-MD5 here. Using the session key's own checksum type
// instead produces a well-formed request the KDC rejects, and [MS-SFU] section 2.2.1 is
// explicit about it.
async function forUserChecksum(sessionKey, userName, userRealm, authPackage) {
  ...
  return {
    ...
    checksum: await arcfour.checksum(sessionKey, kcrypto.KEY_USAGE.PA_FOR_USER_CKSUM,
...
async function buildS4u2SelfReq(options) {
  ...
  var user = opts.user;
  if (!user || !user.name) {
    throw new Error("krb5: S4U2Self needs the user to impersonate (a PrincipalName)");
  }
  ...
  var forUser = msgs.encPaForUser({
    userName: user,
    userRealm: userRealm,
    cksum: await forUserChecksum(opts.tgt.sessionKey, user, userRealm, authPackage),
  ...
  var built = await buildTgsReq(Object.assign({}, opts, {
    ...
    // Ask for a forwardable ticket: without that flag the result cannot be used as
    // evidence for classic S4U2Proxy, and the KDC only grants it to an account with
    // TRUSTED_TO_AUTHENTICATE_FOR_DELEGATION. A client that does not ask gets a ticket
    // that works perfectly and then fails at the next step for an unrelated-looking
    // reason.
    kdcOptions: opts.kdcOptions || [msgs.KDC_OPTION.FORWARDABLE, msgs.KDC_OPTION.RENEWABLE],
    extraPadata: [{ type: msgs.PA_TYPE.FOR_USER, value: forUser }].concat(opts.extraPadata || [])
  }));
  log.debug("Leaving buildS4u2SelfReq(). impersonating " + msgs.principalToString(user, userRealm));
  return Object.assign({ impersonating: msgs.principalToString(user, userRealm) }, built);
}

// S4U2Proxy. `evidence` is the ticket S4U2Self produced (or one the user forwarded);
// `sname` is the BACK-END service to reach on their behalf.
async function buildS4u2ProxyReq(options) {
  log.debug("Entering buildS4u2ProxyReq().");
  var opts = options || {};
  if (!opts.evidenceTicket) {
    throw new Error("krb5: S4U2Proxy needs the evidence ticket — the service ticket for the user, " +
      "which goes in additional-tickets and is what the KDC reads the client's identity out of");
  }
  var options4 = [msgs.KDC_OPTION.FORWARDABLE, msgs.KDC_OPTION.RENEWABLE,
                  msgs.KDC_OPTION.CNAME_IN_ADDL_TKT];
  var padata = [];
  // Resource-based constrained delegation additionally requires PA-PAC-OPTIONS with the
  // RBCD bit. [MS-SFU] makes a KDC answer KDC_ERR_BADOPTION without it, and that error
  // says nothing about padata — so it is offered explicitly rather than always sent.
  ...
  log.debug("Leaving buildS4u2ProxyReq(). resourceBased=" + !!opts.resourceBased);
  return built;
}

// A renewal. The same TGT is presented with the RENEW option and the same sname it was
// issued for; the KDC returns a new one with the ORIGINAL authtime preserved, which is
// the point — a renewed ticket does not become a freshly authenticated one, and a service
// relying on authtime would be deceived if it did.
async function buildRenewReq(options) {
  log.debug("Entering buildRenewReq().");
  var opts = options || {};
  return buildTgsReq(Object.assign({}, opts, {
    sname: opts.sname || { type: msgs.NAME_TYPE.SRV_INST, name: ["krbtgt", opts.tgt.realm] },
    kdcOptions: opts.kdcOptions || [msgs.KDC_OPTION.RENEW, msgs.KDC_OPTION.RENEWABLE]
  }));
}
...
// ticket-granting ticket, which is the whole point — a service ticket would only reach one
// service, and this is meant to reach all of them.
async function buildForwardedTgtReq(options) {
  var opts = options || {};
  return buildTgsReq(Object.assign({}, opts, {
    sname: opts.sname || { type: msgs.NAME_TYPE.SRV_INST, name: ["krbtgt", opts.tgt.realm] },
    // FORWARDED asks for the flag. FORWARDABLE is sent alongside it so the forwarded
    // ticket can itself be forwarded onward — though note this is belt and braces against
    // a STRICT KDC: RFC 4120 has the TGS set FORWARDABLE when it is requested *and* the
    // presented ticket carries it, while this project's mock KDC inherits the presented
    // ticket's flags unconditionally. So dropping the option changes nothing against the
    // mock and would matter against Active Directory — which is why it is stated here
    // rather than left to be discovered, and why no test asserts a difference it cannot
    // produce.
    kdcOptions: opts.kdcOptions || [msgs.KDC_OPTION.FORWARDED, msgs.KDC_OPTION.FORWARDABLE],
    // The address the ticket is being forwarded TO, when the client cares to bind it. AD
    // issues addressless tickets and so does this by default; a bound one is stricter and
    // breaks behind NAT, which is why almost nobody uses it.
    addresses: opts.addresses || null
  }));
}
...
//
// `key` is the AP exchange's subkey when there is one and the ticket's session key
// otherwise — NOT the forwarded ticket's own key. Key usage 14.
async function wrapDelegatedCredential(options) {
  ...
  if (!forwarded || !forwarded.ticket || !forwarded.sessionKey) {
    throw new Error("krb5: a KRB-CRED needs the forwarded ticket AND its session key — the ticket " +
      "alone is opaque to whoever receives it, so forwarding one without its key delegates nothing");
  }
  ...
  var cred = msgs.encKrbCred({
    ...
    encPart: {
      ...
      cipher: await profile.encrypt(opts.key.key, kcrypto.KEY_USAGE.KRB_CRED_ENCPART, part)
    }
  });
...
//
// The result is deliberately shaped like the return of readTgsRep(), so a service can hand
// it straight to buildTgsReq() and start acting as the client. That IS the capability, and
// the shape makes it obvious rather than clever.
async function readDelegatedCredential(options) {
  ...
  if (opts.key.etype !== cred.encPart.etype) {
    throw new Error("krb5: this KRB-CRED is encrypted with " + cred.encPart.etypeName +
      " (etype " + cred.encPart.etype + ") and the key supplied is etype " + opts.key.etype +
      ". The key is the AP exchange's subkey if one was sent and the ticket's session key " +
  ...
  if (part.ticketInfo.length !== cred.tickets.length) {
    throw new Error("krb5: this KRB-CRED carries " + cred.tickets.length + " ticket(s) and " +
      part.ticketInfo.length + " set(s) of ticket information. They are positional, so a " +
      "mismatch means no ticket can be paired with its key.");
  }
  var info = part.ticketInfo[0];
  log.debug("Leaving readDelegatedCredential(). " + msgs.principalToString(info.pname, info.prealm));
  return {
    ok: true,
