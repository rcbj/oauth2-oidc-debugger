...
// The authorization-data types this module has to walk through or name. The first two
// are RFC 4120's; the rest are MS-KILE's.
var AD_TYPE = {
  IF_RELEVANT: 1,
  KDC_ISSUED: 4,
  AND_OR: 5,
...
// `buildPac()` must fill them in, and the reason a verifier can zero the right ones.
...
// [MS-PAC] section 2.8's SignatureType table, and the etype whose crypto profile each
// one belongs to. Only these three appear from a Windows KDC; 19 and 20 are here
// because this project's own mock KDC can be driven with AES-SHA2, which Windows has
// never supported — so seeing one is a fact about the KDC, worth saying rather than
// rejecting.
var SIGNATURE_TYPE = {
...
// All four PAC signatures use this one. Named for what [MS-KILE] calls it, because
// the number alone reads like an arbitrary choice.
...
function profileForSignatureType(signatureType) {
  ...
  if (etype === undefined) {
    throw new Error("krb5: PAC signature type " + signatureType + " is not one this code can compute" +
...
//
// [MS-PAC] section 2.2.1 defines exactly these FIVE and says "all other bits MUST be
// set to zero and MUST be ignored on receipt" — note where RESOURCE sits: bit 2 of the
// diagram, which is 0x20000000, not 0x10 as counting from the left would suggest.
...
// The remaining SE_GROUP_* values from [SIDATT]. They are real flags in a Windows
// access token and NOT permitted in a PAC, so one appearing here is a finding rather
// than something to render as though the PAC defined it. USE_FOR_DENY_ONLY is the one
// to spot: the group is present and grants nothing, so a service can deny on it while
// the account genuinely "is a member".
...
//
// NOT the LDAP `userAccountControl` attribute's bits. The two tables share most of
// their NAMES and none of their VALUES — LDAP's USER_NORMAL_ACCOUNT is 0x0200 where
// SAMR's is 0x0010 — and [MS-PAC] cites SAMR. Mixing them up produces a flag list
// that is entirely wrong while looking entirely plausible, which is a lie a debugger
// has no business telling: "Trusted for delegation" against an account that is not is
// how someone spends a day chasing the wrong thing.
...
// KERB_VALIDATION_INFO's UserFlags. Only D and H are meaningful for Kerberos — the
// rest are set by NTLM and MUST be zero here, so seeing one IS the finding.
//
// The letters are [MS-PAC] section 2.5's own, kept in the names because the spec
// refers to these bits by letter and nothing else. Read that bit diagram carefully:
// the LEFTMOST cell is bit 0 and the RIGHTMOST is the LEAST significant bit, so A is
// 0x1 and L — eighteenth from the left — is 0x2000. Counting the other way puts D
// (EXTRA_SIDS) at 0x40 instead of 0x20, which makes the builder emit extra SIDs a real
// service then ignores, and makes the reader report a UserFlags value made of the
// wrong names. Cross-checked against Samba's netlogon.idl, which spells the same bits
// as NETLOGON_EXTRA_SIDS 0x20 / NETLOGON_RESOURCE_GROUPS 0x200.
var USER_FLAGS = [
  ...
  [0x00000200, "H RESOURCE_GROUPS"],                 // ResourceGroupIds is populated
...
// Well-known RIDs within a domain, and well-known SIDs that are not domain-relative.
// Rendering `S-1-5-21-…-512` without saying "Domain Admins" beside it is work the
// reader then has to do by hand, every time.
var WELL_KNOWN_RIDS = {
  500: "Administrator",
  501: "Guest",
  502: "krbtgt",
...
var WELL_KNOWN_SIDS = {
  ...
  // The two that matter most when debugging S4U: they record HOW the identity was
  // established, so a service can refuse an identity the KDC merely asserted.
...
function flagNames(table, value) {
  ...
  table.forEach(function (entry) {
    // >>> 0 because 0x80000000 is negative as a signed 32-bit int, and a bare & would
    // make SE_GROUP_LOGON_ID never match.
...
// Group attributes, naming the five [MS-PAC] defines and calling out any of the other
// SE_GROUP_* values as what they are: bits the spec requires to be zero here.
function groupAttributeNames(value) {
  ...
  GROUP_ATTRIBUTES_OUTSIDE_MS_PAC.forEach(function (entry) {
    // An exact mask match, because SE_GROUP_LOGON_ID is TWO bits — a bitwise-any test
    // would report it for either half alone. Note the SECOND `>>> 0`: `&` yields a
    // SIGNED int32, so for a mask with the top bit set the result is negative and
    // comparing it to the unsigned mask is false for the one value that should match.
    var mask = entry[0] >>> 0;
    if ((((value >>> 0) & mask) >>> 0) === mask) {
      names.push(entry[1] + " (not permitted in a PAC — [MS-PAC] requires all other bits zero)");
    }
  });
  return names;
}
...
// Walks AD-IF-RELEVANT (and the other container types) to whatever depth they nest,
// which is what makes ad-type 128 findable. Returns every PAC found — plural because
// a ticket with two is malformed in a way worth reporting rather than a case to
    ...
    entries.forEach(function (ad) {
      ...
      if (ad.type === AD_TYPE.IF_RELEVANT || ad.type === AD_TYPE.MANDATORY_FOR_KDC) {
        ...
        try {
          ...
          walk(msgs.readAuthorizationData(asn1.readTlv(prim.toBytes(ad.data), 0, 0)),
            here, depth + 1);
        } catch (e) {
          // A container this code cannot open is not a failure of the ticket: other
          // ad-types legitimately carry things we do not parse. Say so and move on.
          log.debug("krb5_pac: could not open " + here.join(" → ") + ": " + e.message);
        }
      }
    });
  }
...
// PACTYPE, [MS-PAC] section 2.3. NOT NDR — a plain little-endian struct, with every
// buffer offset a multiple of 8.
function parsePac(bytes) {
  ...
  var version = r.u32();
  if (version !== 0) {
    problems.push("The PACTYPE Version is " + version + "; [MS-PAC] section 2.3 requires 0.");
  }
  if (cBuffers === 0) {
    throw new Error("krb5: this PAC declares zero buffers");
  }
  if (cBuffers > MAX_BUFFERS) {
    throw new Error("krb5: this PAC declares " + cBuffers + " buffers (a real one has a handful) — " +
      "these bytes are probably not a PAC, or not being read little-endian");
  }
  if (8 + cBuffers * 16 > all.length) {
    throw new Error("krb5: this PAC declares " + cBuffers + " buffers, whose table alone would need " +
  ...
  for (var i = 0; i < cBuffers; i++) {
    ...
    if (off.high !== 0 || off.value + cbBufferSize > all.length) {
      entry.error = "This buffer claims " + cbBufferSize + " bytes at offset " + off.value +
        ", which is outside the " + all.length + "-byte PAC.";
      problems.push("Buffer " + i + " (" + entry.typeName + "): " + entry.error);
    } else {
      if (off.value % 8 !== 0) {
        problems.push("Buffer " + i + " (" + entry.typeName + ") starts at offset " + off.value +
          ", which is not a multiple of 8 — [MS-PAC] section 2.4 requires it. Windows will " +
          "reject this PAC.");
      }
  ...
  buffers.forEach(function (entry) {
    ...
    try {
      ...
      // against the buffer and carry on — the whole point of this page is looking at
      // something that is already wrong.
...
function bufferOfType(pac, type) {
  for (var i = 0; i < pac.buffers.length; i++) {
    if (pac.buffers[i].type === type) return pac.buffers[i];
  }
  return null;
}
...
// [MS-PAC] section 2.4 says which buffers MUST be present. Absence is worth naming
// because it changes what a Windows service will do with the ticket — and a duplicate
// is worth naming because the rule is "ignore the second", so a forged second buffer
...
function checkRequiredBuffers(pac) {
  [
    ...
    if (n === 0) {
      pac.problems.push("There is no " + pair[1] + " buffer. [MS-PAC] section 2.4 requires one.");
    } else if (n > 1) {
      pac.problems.push("There are " + n + " " + pair[1] + " buffers. Only the first is used; " +
        "the rest MUST be ignored, so a second one is a place to hide something.");
    }
  });
  [TYPE.SERVER_CHECKSUM, TYPE.KDC_CHECKSUM].forEach(function (t) {
    if (countOfType(pac, t) === 0) {
      pac.problems.push("There is no " + bufferTypeName(t) + " buffer. A PAC in a ticket always " +
...
function parseBuffer(type, bytes) {
  switch (type) {
    case TYPE.LOGON_INFO: return parseLogonInfo(bytes);
    case TYPE.CLIENT_INFO: return parseClientInfo(bytes);
    case TYPE.UPN_DNS_INFO: return parseUpnDnsInfo(bytes);
    ...
    default:
      // Claims, device info, credentials and delegation information are not decoded
      // here. Returning null rather than throwing keeps them listed with their sizes,
      // which is honest: "present, not decoded" is different from "absent".
      return null;
  }
}
...
function parseSignature(bytes) {
  ...
  if (b.length < 4) throw new Error("krb5: a signature buffer of " + b.length + " bytes");
  var r = ndr.createReader(b);
  var raw = r.u32();
  // SignatureType is a SIGNED 32-bit value: KERB_CHECKSUM_HMAC_MD5 is 0xFFFFFF76,
  // which is -138. Reading it unsigned gives 4294967158 and matches nothing.
  ...
  var signature = expected === null ? rest : rest.subarray(0, Math.min(expected, rest.length));
  // A trailing 2 bytes is an RODC identifier — the first 16 bits of the key version
  // number, present only when the issuing KDC is read-only. Anything else trailing is
  // a length that does not match the signature type.
  var trailing = expected === null ? new Uint8Array(0) : rest.subarray(signature.length);
  return {
    signatureType: signatureType,
    signatureTypeName: signatureTypeName(signatureType),
    signature: signature,
    expectedLength: expected,
    rodcIdentifier: trailing.length === 2 ? (trailing[0] | (trailing[1] << 8)) : null,
    trailing: trailing.length === 2 ? null : trailing,
    // Where in the buffer the signature bytes start, which is what the zeroing needs.
...
function parseClientInfo(bytes) {
  ...
  if (nameLength % 2 !== 0) {
    throw new Error("krb5: PAC_CLIENT_INFO NameLength is " + nameLength + " bytes, which is odd — " +
      "the name is UTF-16, so it must be even");
  }
  if (nameLength > r.remaining) {
    throw new Error("krb5: PAC_CLIENT_INFO says its name is " + nameLength + " bytes but only " +
...
// UPN_DNS_INFO, [MS-PAC] section 2.10. Offsets are from the START OF THIS BUFFER,
// not from the start of the PAC — a distinction that costs an afternoon if missed,
// because the small offsets land inside the PACTYPE header and decode as noise.
function parseUpnDnsInfo(bytes) {
  ...
  function utf16At(offset, length, what) {
    ...
    if (length % 2 !== 0) {
      throw new Error("krb5: UPN_DNS_INFO's " + what + " length is " + length + ", which is odd");
    }
    if (offset + length > b.length) {
      throw new Error("krb5: UPN_DNS_INFO's " + what + " claims " + length + " bytes at offset " +
        offset + ", past the end of its " + b.length + "-byte buffer");
    }
    var s = "";
    for (var i = 0; i < length; i += 2) s += String.fromCharCode(b[offset + i] | (b[offset + i + 1] << 8));
    return s;
  }

  var info = {
    flags: flags,
    flagNames: (flags & UPN_DNS_FLAG_NO_UPN_ATTRIBUTE ? ["U (no userPrincipalName attribute; " +
      "this UPN was constructed)"] : []).concat(
      flags & UPN_DNS_FLAG_EXTENDED ? ["S (extended with the SAM name and SID)"] : []),
  ...
  if (info.extended) {
    // The four extra fields exist only when S is set, so they are read only then —
    // reading them unconditionally would pick up the UPN's own characters and report
    // a SAM name made of them.
    r.seek(12);
    var samLength = r.u16();
    var samOffset = r.u16();
    var sidLength = r.u16();
    ...
    if (sidLength) {
      if (sidOffset + sidLength > b.length) {
        throw new Error("krb5: UPN_DNS_INFO's SID claims " + sidLength + " bytes at offset " +
...
// PAC_REQUESTOR_SID, [MS-PAC] section 2.15: "a single SID structure", with no NDR
// conformance ahead of it.
...
function parseRequestorGuid(bytes) {
  var b = prim.toBytes(bytes);
  if (b.length < 16) throw new Error("krb5: a GUID buffer of " + b.length + " bytes; 16 are needed");
  // A GUID's first three fields are little-endian and its last two are not, which is
  // why it cannot just be hex-dumped in order.
  function le(o, n) {
    var v = 0;
...
// carries every group membership. The field order below is the IDL's, verbatim; NDR is
// positional, so one field out of order silently misreads everything after it while
// keeping every value in range.
function parseLogonInfo(bytes) {
  var r = ndr.createReader(bytes);
  var headers = ndr.readTypeMarshallingHeaders(r);
  if (!r.pointer()) {
    throw new Error("krb5: the logon information's top-level pointer is NULL — there is a " +
  ...
  if (groupIdsPresent) {
    ...
    if (declared !== groupCount) {
      throw new Error("krb5: GroupCount is " + groupCount + " but the array's own conformant count " +
        "is " + declared);
    }
    ...
    throw new Error("krb5: GroupCount is " + groupCount + " but the GroupIds pointer is NULL");
  }
  ...
  // deferred past the END of the array — every element's fixed part first, then every
  // referent. Reading each SID as the loop reaches it is right for one element and
  // wrong for two, which is a bug that passes its first test.
  info.extraSids = [];
  if (extraSidsPresent) {
    var declaredSids = r.arrayCount("extra SID(s)");
    if (declaredSids !== sidCount) {
      throw new Error("krb5: SidCount is " + sidCount + " but the ExtraSids array's own conformant " +
    ...
    throw new Error("krb5: SidCount is " + sidCount + " but the ExtraSids pointer is NULL");
  }

  info.resourceGroupDomainSid = resourceGroupDomainSidPresent ? ndr.readConformantSid(r) : null;
  info.resourceGroups = [];
  if (resourceGroupIdsPresent) {
    var declaredRes = r.arrayCount("resource group(s)");
    if (declaredRes !== resourceGroupCount) {
      throw new Error("krb5: ResourceGroupCount is " + resourceGroupCount + " but the array's own " +
  ...
  info.userAccountControlNames = flagNames(USER_ACCOUNT_CONTROL, info.userAccountControl);
  info.userSid = info.logonDomainId ? ndr.sidWithRid(info.logonDomainId, info.userId) : null;
  ...
  // `undefined` for as long as these two lines were the other way round, so that
  // check silently never fired — the kind of defect a passing round-trip cannot see.)
...
// misconfiguration or a forgery tell, and none of them stops the structure parsing —
// which is exactly why they have to be checked deliberately.
function consistencyNotes(info, sidCount, resourceGroupCount) {
  var notes = [];
  if (sidCount !== 0 && !(info.userFlags & USER_FLAG_EXTRA_SIDS)) {
    notes.push("SidCount is " + sidCount + " but the EXTRA_SIDS flag (D) is not set. [MS-PAC] " +
      "section 2.5 requires it, and a service that trusts the flag will ignore these SIDs.");
  }
  if ((info.userFlags & USER_FLAG_EXTRA_SIDS) && sidCount === 0) {
    notes.push("The EXTRA_SIDS flag (D) is set but SidCount is 0.");
  }
  if (resourceGroupCount !== 0 && !(info.userFlags & USER_FLAG_RESOURCE_GROUPS)) {
    notes.push("ResourceGroupCount is " + resourceGroupCount + " but the RESOURCE_GROUPS flag (H) " +
      "is not set, which [MS-PAC] section 2.5 requires.");
  }
  if (info.userId === 0) {
    notes.push("UserId is 0, which means the account's own SID is the FIRST SID in ExtraSids " +
      "rather than LogonDomainId + UserId.");
  }
  var ntlmOnly = info.userFlagNames.filter(function (n) { return n.indexOf("NTLM only") !== -1; });
  if (ntlmOnly.length) {
    notes.push("UserFlags carries " + ntlmOnly.join(", ") + ". [MS-PAC] section 2.5 says these are " +
      "set only by NTLM and MUST be zero for Kerberos.");
  }
  if (info.reserved1[0] !== 0 || info.reserved1[1] !== 0) {
    notes.push("Reserved1 is not zero (" + info.reserved1.join(", ") + "); [MS-PAC] requires zero.");
  }
  var unknownUac = unknownFlagBits(USER_ACCOUNT_CONTROL, info.userAccountControl);
  if (unknownUac) {
    notes.push("UserAccountControl has undefined bits set (0x" + unknownUac.toString(16) + ").");
  }
  // NDR pads the marshalled object to a multiple of 8, so up to 7 trailing bytes are
  // expected and reporting them would be noise on every well-formed PAC. More than
  // that means a field was misread and the reader stopped early — which is worth
  // saying loudly, because every value above it will still look reasonable.
  if (info.bytesUnread > 7) {
    notes.push(info.bytesUnread + " bytes of this buffer were not consumed. NDR pads to a multiple " +
      "of 8, so at most 7 are expected; this many means a field was read at the wrong offset and " +
...
// Reproduces the PAC as it stood when a given signature was computed, by zeroing the
// signature fields that were still zero at that moment (see the file header — the
// order the four are generated in is what makes this well-defined).
function pacWithSignaturesZeroed(pac, types) {
  var copy = new Uint8Array(pac.raw);
  types.forEach(function (t) {
    pac.buffers.forEach(function (b) {
      if (b.type !== t || !b.bytes || !b.parsed) return;
...
//
// This table is the only statement of it, deliberately: the four signatures are the
// part of [MS-PAC] most easily got subtly wrong, and there are two callers — one with
// named keys (a service, a KDC) and one with a POOL of keys it can try (the decoder
// page, which has whatever the reader pasted in). Writing the covered bytes out twice
// is how the two come to disagree, and the disagreement would show up as a PAC that
// verifies on one screen and not on another.
  ...
  return [
    {
      ...
      role: "service key",
      data: pacWithSignaturesZeroed(pac, [TYPE.SERVER_CHECKSUM, TYPE.KDC_CHECKSUM]),
      note: "This is the one a SERVICE can check for itself — it is signed with the service's " +
        "own long-term key."
    },
    {
      type: TYPE.KDC_CHECKSUM,
      role: "krbtgt key",
      data: serverEntry && serverEntry.parsed ? serverEntry.parsed.signature : null,
      missing: "there is no server signature for it to counter-sign",
      note: "A counter-signature over the server signature ALONE, not over the PAC. Only a KDC " +
        "holds the key, which is what makes it the one a forged PAC cannot produce."
    },
    {
      ...
      note: "Added by the CVE-2022-37967 hardening: the KDC signature covers only the server " +
    ...
    },
    {
      ...
      missing: "the EncTicketPart it covers was not supplied. It is the DER of the ticket with " +
        "this PAC's ad-data replaced by a single zero byte, so it binds the PAC to ONE ticket",
      note: "Binds this PAC to one specific ticket, so a PAC cannot be lifted out of one and " +
        "dropped into another."
    }
  ];
}

// keys: { serverKey, kdcKey, ticketBytes }, each optional. A key is
// { etype, key } as elsewhere in this codec; `ticketBytes` is the DER of the
// EncTicketPart with the PAC's ad-data replaced by one zero byte, which only the
// caller can produce because only the caller has the ticket.
async function verifySignatures(pac, keys) {
  ...
  async function check(type, keyLabel, key, data, note) {
    ...
    if (!entry.parsed) {
      out.note = "This signature buffer did not parse" + (entry.error ? ": " + entry.error : ".");
      results.push(out);
      return;
    }
    if (!key) {
    ...
    if (data === null) {
      out.note = "Not checked: " + (note || "the data it covers was not available here.");
      results.push(out);
      return;
    }
    try {
      ...
      if (profile.id !== key.etype) {
        ...
        out.verified = false;
        out.note = "This signature is " + entry.parsed.signatureTypeName + ", which belongs to etype " +
          profile.id + ", but the " + keyLabel + " supplied is etype " + key.etype + ". " +
          "The key cannot verify this signature whatever its value.";
        results.push(out);
        return;
      }
      out.verified = await profile.verifyChecksum(key.key, KERB_NON_KERB_CKSUM_SALT, data,
        entry.parsed.signature);
      out.note = out.verified
        ? "Verified with the " + keyLabel + "."
        : "DOES NOT verify with the " + keyLabel + " supplied. Either the key is wrong or the PAC " +
  ...
  for (var i = 0; i < coverage.length; i++) {
    var c = coverage[i];
    await check(c.type, c.role, c.role === "service key" ? opts.serverKey : opts.kdcKey,
      c.data, c.data === null ? c.missing : c.note);
  }

  log.debug("Leaving verifySignatures(). " + results.length + " signature(s) considered.");
  return results;
}

// The same four checks against a POOL of keys — what the decoder page has, since a
// reader pastes in whatever keys they happen to hold and cannot be expected to label
// them. Reports WHICH key verified, by label, because "verified" without saying with
// what answers a different question: on a TGT the service key and the krbtgt key are
// the SAME key, and noticing that is often the point.
//
// THE IMPORTANT DISTINCTION, and it took a cried-wolf failure to get right: a
// signature that no key in the pool verifies is `verified: null` — NOT `false`. On a
// service ticket the reader has the service's key and no possible way to have the
// krbtgt key, so "the KDC signature does not verify" would appear on every correct
// PAC anyone ever pastes in, and a warning that fires always is a warning nobody
// reads. `false` is reserved for a key whose ROLE the caller has asserted — pass
// `opts.serverKey` when you know a key IS the service's (the decoder page does: the
// key that decrypted the ticket is the service's key by definition), and a failure
// against that is a real finding.
async function verifySignaturesWithAnyKey(pac, keys, opts) {
  log.debug("Entering verifySignaturesWithAnyKey(). keys=" + (keys || []).length);
  var pool = keys || [];
  var options = opts || {};
  var asserted = {
  ...
  for (var i = 0; i < coverage.length; i++) {
    ...
    // A key whose role the caller has asserted gives a definite verdict. Otherwise
    // only keys of the signature's own etype are even worth trying.
    var claimed = asserted[c.role];
    var eligible = claimed ? [claimed] : pool.filter(function (k) { return k.etype === profile.id; });
    ...
    if (claimed && claimed.etype !== profile.id) {
      ...
      out.verified = false;
      out.note = "This signature is " + entry.parsed.signatureTypeName + ", which belongs to " +
        "etype " + profile.id + ", but the " + c.role + " supplied is etype " + claimed.etype +
        ". That key cannot verify this signature whatever its value.";
      results.push(out);
      continue;
    }
    if (!eligible.length) {
      out.note = "Not checked: this is " + entry.parsed.signatureTypeName + ", which needs an " +
        "etype " + profile.id + " key, and none was supplied. " + c.note;
      results.push(out);
      continue;
    }
    for (var j = 0; j < eligible.length; j++) {
      var ok = await profile.verifyChecksum(eligible[j].key, KERB_NON_KERB_CKSUM_SALT, c.data,
        entry.parsed.signature);
      if (ok) {
        out.verified = true;
        out.verifiedBy = eligible[j].label || "a supplied key";
        out.note = "Verified with " + out.verifiedBy + ", acting as the " + c.role + ". " + c.note;
        break;
      }
    }
    if (out.verified === null) {
      if (claimed) {
        out.verified = false;
        out.note = "DOES NOT verify with the " + c.role + " supplied. Either that key is wrong or " +
          "this PAC has been altered since it was signed. " + c.note;
      } else {
        // Left as UNKNOWN on purpose — see this function's header.
        out.note = "Not confirmed: none of the " + eligible.length + " etype " + profile.id +
          " key(s) supplied verifies it, and none of them was identified as the " + c.role +
          ". On a service ticket you would not hold the krbtgt key at all, so this is the " +
  ...
  log.debug("Leaving verifySignaturesWithAnyKey(). " + results.length + " considered.");
  return results;
}
...
function encodeLogonInfo(spec) {
  ...
  w.fileTime(spec.passwordMustChange === undefined ? "never" : spec.passwordMustChange);

  ndr.writeUnicodeStringHeader(w, spec.effectiveName);
  ndr.writeUnicodeStringHeader(w, spec.fullName === undefined ? null : spec.fullName);
  ndr.writeUnicodeStringHeader(w, spec.logonScript === undefined ? null : spec.logonScript);
  ndr.writeUnicodeStringHeader(w, spec.profilePath === undefined ? null : spec.profilePath);
  ndr.writeUnicodeStringHeader(w, spec.homeDirectory === undefined ? null : spec.homeDirectory);
  ndr.writeUnicodeStringHeader(w, spec.homeDirectoryDrive === undefined ? null : spec.homeDirectoryDrive);
  ...
  // [MS-PAC] makes them a MUST and a builder that lets them drift emits a PAC whose
  // extra SIDs a real service ignores — which then reads as a KDC that did not add
  // them. `userFlags` in the spec is for tests that need exactly that defect.
  ...
  ndr.writeUnicodeStringHeader(w, spec.logonServer === undefined ? null : spec.logonServer);
  ndr.writeUnicodeStringHeader(w, spec.logonDomainName === undefined ? null : spec.logonDomainName);
  w.pointer(!!spec.logonDomainId);
  w.u32(0); w.u32(0);                                  // Reserved1[2]
  w.u32(spec.userAccountControl === undefined ? 0x00000010 : spec.userAccountControl);
  w.u32(spec.subAuthStatus || 0);
  w.fileTime(spec.lastSuccessfulILogon === undefined ? null : spec.lastSuccessfulILogon);
  w.fileTime(spec.lastFailedILogon === undefined ? null : spec.lastFailedILogon);
  ...
  ndr.writeUnicodeStringValue(w, spec.fullName === undefined ? null : spec.fullName);
  ndr.writeUnicodeStringValue(w, spec.logonScript === undefined ? null : spec.logonScript);
  ndr.writeUnicodeStringValue(w, spec.profilePath === undefined ? null : spec.profilePath);
  ndr.writeUnicodeStringValue(w, spec.homeDirectory === undefined ? null : spec.homeDirectory);
  ndr.writeUnicodeStringValue(w, spec.homeDirectoryDrive === undefined ? null : spec.homeDirectoryDrive);
  ...
  ndr.writeUnicodeStringValue(w, spec.logonServer === undefined ? null : spec.logonServer);
  ndr.writeUnicodeStringValue(w, spec.logonDomainName === undefined ? null : spec.logonDomainName);
  ...
  if (spec.resourceGroupDomainSid) ndr.writeConformantSid(w, spec.resourceGroupDomainSid);
  ...
  // NDR pads the marshalled object to a multiple of 8, and ObjectBufferLength counts
  // everything after the private header.
  w.align(8);
...
function encodeUpnDnsInfo(spec) {
  ...
  function place(text) {
    if (text === null || text === undefined) return { offset: 0, length: 0 };
...
function encodeSignaturePlaceholder(signatureType, length, rodcIdentifier) {
  ...
  w.zeros(length);
  if (rodcIdentifier !== undefined && rodcIdentifier !== null) w.u16(rodcIdentifier);
  return w.build();
}

// Assembles a PACTYPE from a list of { type, bytes }, padding each buffer's START to
// the 8-byte boundary [MS-PAC] section 2.4 requires. Returns the bytes plus, for each
// buffer, where it landed — which is what the signing pass needs in order to overwrite
...
async function buildPac(spec) {
  log.debug("Entering buildPac(). client=" + (spec.clientInfo && spec.clientInfo.name));
  ...
  var entries = [{ type: TYPE.LOGON_INFO, bytes: encodeLogonInfo(spec.logonInfo) }];
  entries.push({ type: TYPE.CLIENT_INFO, bytes: encodeClientInfo(spec.clientInfo) });
  if (spec.upnDns) entries.push({ type: TYPE.UPN_DNS_INFO, bytes: encodeUpnDnsInfo(spec.upnDns) });
  if (spec.attributes !== undefined) {
    entries.push({ type: TYPE.ATTRIBUTES_INFO, bytes: encodeAttributesInfo(spec.attributes) });
  }
  ...
  if (spec.includeTicketSignature) {
    entries.push({
      type: TYPE.TICKET_CHECKSUM,
      bytes: encodeSignaturePlaceholder(kdcSigType, kdcProfile.checksumBytes, spec.rodcIdentifier)
    });
  }
  ...
  entries.push({
    type: TYPE.SERVER_CHECKSUM,
    bytes: encodeSignaturePlaceholder(serverSigType, serverProfile.checksumBytes)
  });
  entries.push({
    type: TYPE.KDC_CHECKSUM,
    bytes: encodeSignaturePlaceholder(kdcSigType, kdcProfile.checksumBytes, spec.rodcIdentifier)
  ...
  log.debug("Leaving buildPac(). bytes=" + bytes.length + ", buffers=" + entries.length);
  return bytes;
}

// The signing pass, shared by buildPac() and resignPac().
//
// Separate because a KDC signs a PAC in two situations and they must not diverge: when
// it MINTS one, and when it re-signs one somebody else minted — a cross-realm referral
// arriving at the target realm, or a renewal, both of which [MS-PAC] requires to
// recompute all four. Two copies of this order would eventually disagree, and a PAC
// signed in the wrong order fails in a way indistinguishable from a wrong key.
async function signAssembled(assembled, spec) {
  ...
  function signatureAt(type) {
    for (var i = 0; i < assembled.placed.length; i++) {
      if (assembled.placed[i].type === type) return assembled.placed[i].offset + 4;
    }
    return -1;
  }
  ...
  function write(type, sig) {
    ...
    if (at < 0) throw new Error("krb5: no " + bufferTypeName(type) + " buffer to write into");
    bytes.set(sig, at);
  }

  // 1. The ticket signature, over bytes the caller supplies (only it has the ticket).
  if (has(TYPE.TICKET_CHECKSUM)) {
    if (!spec.ticketBytes) {
      throw new Error("krb5: a ticket signature was asked for but no EncTicketPart bytes were " +
        "supplied to compute it over. It covers the DER of the ticket with this PAC's ad-data " +
  ...
  var serverSig = await serverProfile.checksum(spec.serverKey.key, KERB_NON_KERB_CKSUM_SALT, bytes);
...
//
// This is what the target realm's KDC does to a cross-realm referral: the client's
// account lives in the other realm and this KDC has no copy of it, so it cannot
// rebuild the PAC — it carries the existing buffers across and re-signs them with its
// own keys, because the signatures it arrived with were made with a key the service
// being issued to does not have.
//
// Rebuilding instead of re-signing would look identical in a mock where both realms
// happen to share one principal table, and would be wrong everywhere else. It also
// quietly discards anything the issuing realm put in that this codec does not decode —
// claims, device info — which is the sort of loss nobody notices.
//
// **SID filtering is NOT applied here.** Windows strips SIDs belonging to domains the
// trust is not authorized to assert, and that control is what stops the other realm
// claiming membership of groups in this one. Saying so is better than a silence that
...
async function resignPac(pacBytes, spec) {
  ...
  if (spec.includeTicketSignature) {
    entries.push({
      type: TYPE.TICKET_CHECKSUM,
      bytes: encodeSignaturePlaceholder(kdcProfile.checksumType, kdcProfile.checksumBytes,
        spec.rodcIdentifier)
    });
  }
  if (spec.includeExtendedKdcSignature) {
  ...
  entries.push({
    type: TYPE.SERVER_CHECKSUM,
    bytes: encodeSignaturePlaceholder(serverProfile.checksumType, serverProfile.checksumBytes)
  });
  entries.push({
    type: TYPE.KDC_CHECKSUM,
    bytes: encodeSignaturePlaceholder(kdcProfile.checksumType, kdcProfile.checksumBytes,
  ...
  var bytes = await signAssembled(assemblePac(entries), spec);
  log.debug("Leaving resignPac(). bytes=" + bytes.length + ", carried " + entries.length +
    " buffer(s) across.");
  return bytes;
}

// Wraps a PAC as an EncTicketPart authorization-data element: ad-type 128 inside
// ad-type 1. The nesting is not optional — see the file header.
function wrapPacAsAuthorizationData(pacBytes) {
  return [{
    type: AD_TYPE.IF_RELEVANT,
    data: msgs.encAuthorizationData([{ type: AD_TYPE.WIN2K_PAC, data: pacBytes }])
  }];
}
