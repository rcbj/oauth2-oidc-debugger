...
function contextTag(n) {
  if (n < 0 || n > 30) throw new Error("krb5: context tag " + n + " out of range for this coder");
  return 0xa0 | n;
}

// [APPLICATION n], constructed.
function applicationTag(n) {
  if (n < 0 || n > 30) throw new Error("krb5: application tag " + n + " out of range for this coder");
  return 0x60 | n;
}
...
function encodeLength(n) {
  ...
  if (bytes.length > 4) throw new Error("krb5: refusing to encode a length of " + n + " bytes");
...
function encGeneralString(text) { return tlv(TAG.GENERAL_STRING, prim.utf8(text)); }
...
function encFlags(bitNumbers) {
  ...
  (bitNumbers || []).forEach(function (bit) {
    if (bit < 0 || bit > 31) throw new Error("krb5: flag bit " + bit + " out of range");
...
function flagsFromBits(bitNumbers) {
  var bytes = new Uint8Array(4);
  (bitNumbers || []).forEach(function (bit) { bytes[bit >> 3] |= 0x80 >> (bit & 7); });
  return bytes;
}
...
function formatKerberosTime(date) {
  ...
  if (isNaN(d.getTime())) throw new Error("krb5: not a date: " + date);
...
function parseKerberosTime(text) {
  ...
  if (!m) throw new Error("krb5: KerberosTime must be YYYYMMDDHHMMSSZ, got " + JSON.stringify(s));
...
// A SEQUENCE whose fields are context-tagged, skipping the ones that are absent.
// Every OPTIONAL field in this grammar is expressed by passing undefined or
// null, which is what keeps the message builders readable.
...
function readTlv(bytes, offset, depth) {
  var b = toBytes(bytes);
  var at = offset || 0;
  if ((depth || 0) > MAX_DEPTH) throw new Error("krb5: ASN.1 nested deeper than " + MAX_DEPTH);
  if (at + 2 > b.length) throw new Error("krb5: truncated ASN.1 at offset " + at + " (need a tag and a length)");
  var tag = b[at];
  var lenByte = b[at + 1];
  var headerLen = 2;
  var len;
  if (lenByte < 0x80) {
    ...
    throw new Error("krb5: indefinite ASN.1 length at offset " + at + " — DER requires a definite length");
  } else {
    var n = lenByte & 0x7f;
    if (n > 4) throw new Error("krb5: ASN.1 length field of " + n + " bytes at offset " + at);
    if (at + 2 + n > b.length) throw new Error("krb5: truncated ASN.1 length at offset " + at);
    len = 0;
  ...
  return {
    ...
    // exactly, and re-encoding from `tag` and `length` would not do it: a sender
    // that used a non-minimal long-form length would come back with a different
    // header and a checksum that no longer matches, for no visible reason.
...
function readTaggedSequence(value, depth) {
  ...
  readChildren(value, depth).forEach(function (child) {
    if ((child.tag & 0xc0) !== 0x80) {
      throw new Error("krb5: expected a context tag in this SEQUENCE, saw 0x" + child.tag.toString(16));
    }
    var n = child.tag & 0x1f;
    var inner = readChildren(child.value, (depth || 0) + 1);
    if (inner.length !== 1) {
      throw new Error("krb5: context tag [" + n + "] wraps " + inner.length + " elements, expected 1");
    }
    map[n] = inner[0];
  });
  return map;
...
function decInteger(t) {
  ...
  var b = t.value;
  if (b.length === 0) throw new Error("krb5: zero-length INTEGER");
  if (b.length > 6) throw new Error("krb5: INTEGER of " + b.length + " bytes is out of range for this coder");
...
function decFlags(t) {
  ...
  if (t.value.length < 1) throw new Error("krb5: empty BIT STRING");
...
function readApplication(bytes, expectedNumber) {
  ...
  if (b.length > MAX_INPUT_BYTES) {
    throw new Error("krb5: refusing to parse " + b.length + " bytes (limit " + MAX_INPUT_BYTES + ")");
  }
  var outer = readTlv(b, 0, 0);
  if ((outer.tag & 0xe0) !== 0x60) {
    throw new Error("krb5: not a Kerberos message — the outermost tag is 0x" +
      outer.tag.toString(16) + ", expected an [APPLICATION n] constructed tag (0x60-0x7e)");
  }
  var number = outer.tag & 0x1f;
  if (expectedNumber !== undefined && number !== expectedNumber) {
    throw new Error("krb5: expected [APPLICATION " + expectedNumber + "] but found [APPLICATION " + number + "]");
  }
  var inner = readChildren(outer.value, 1);
  if (inner.length !== 1) {
    throw new Error("krb5: [APPLICATION " + number + "] wraps " + inner.length + " elements, expected 1");
  }
  expectTag(inner[0], TAG.SEQUENCE, "the SEQUENCE inside [APPLICATION " + number + "]");
...
var TAG_NAMES = {
  ...
  0x05: "NULL", 0x06: "OBJECT IDENTIFIER", 0x0c: "UTF8String", 0x10: "SEQUENCE",
  0x11: "SET", 0x13: "PrintableString", 0x16: "IA5String", 0x17: "UTCTime",
  0x18: "GeneralizedTime", 0x1b: "GeneralString", 0x30: "SEQUENCE", 0x31: "SET"
};
...
function describeTag(tag) {
  ...
  var number = tag & 0x1f;
  if (cls === 0x00) return TAG_NAMES[tag] || TAG_NAMES[number] || ("universal " + number);
  if (cls === 0x40) return "[APPLICATION " + number + "]";
  if (cls === 0x80) return "[" + number + "]";
  return "[PRIVATE " + number + "]" + (constructed ? "" : " primitive");
}

// Is this value plausibly a nested structure rather than opaque bytes? Used only
// to decide whether to recurse for display; a wrong guess costs a prettier tree,
// not correctness.
function looksConstructed(t) {
  if ((t.tag & 0x20) === 0) return false;
  if (t.length === 0) return false;
  try {
    var children = readChildren(t.value, 0);
    return children.length > 0 && children[children.length - 1].end === t.value.length;
  } catch (e) {
  ...
  while (at < b.length) {
    ...
    if (looksConstructed(t)) {
      if (d < MAX_DEPTH) {
        ...
        // The depth limit is a display cap, not a parse error: bytes pasted into
        // the decoder page should still render as far as they go. But it must be
        // VISIBLE, because a constructed element rendered as a hex leaf reads as
        // "this is opaque data" — the opposite of the truth, and exactly the kind
        // of silent cap that makes a tool lie about what it examined.
        node.depthLimited = true;
        node.text = "(nesting deeper than " + MAX_DEPTH + " levels — not expanded; " +
                    t.length + " bytes)";
      }
    } else {
...
function renderPrimitive(t) {
  try {
    if (t.tag === TAG.INTEGER) return String(decInteger(t));
    if (t.tag === TAG.GENERAL_STRING || t.tag === TAG.IA5_STRING) return decLatin1(t.value);
    if (t.tag === TAG.GENERALIZED_TIME) return decLatin1(t.value);
    if (t.tag === TAG.BIT_STRING && t.value.length === 5) {
      return "bits set: " + (bitsFromFlags(t.value.subarray(1)).join(", ") || "(none)");
    }
  } catch (e) {
  ...
  return hex.length > 128 ? hex.slice(0, 128) + "… (" + t.value.length + " bytes)" : hex;
}
